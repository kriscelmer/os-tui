...
class Node(_common.Resource):
    ...
    def create(self, session, *args, **kwargs):
        ...
        if expected_provision_state is not None:
            ...
            try:
                ...
                raise ValueError(
                    "Node's provision_state must be one of {} for creation, got {}".format(
                ...
                error_message = (
                    f"Cannot create a node with initial provision state {expected_provision_state}"
                )
                # Nodes cannot be created as available using new API versions
                maximum = "1.10" if expected_provision_state == "available" else None
        ...
        if expected_provision_state == "manageable" and self.provision_state != "manageable":
    ...
    def commit(self, session, *args, **kwargs):
        ...
        if "maintenance_reason" in self._body.dirty or "maintenance" in self._body.dirty:
            if not self.is_maintenance and self.maintenance_reason:
                if "maintenance" in self._body.dirty:
                    self.maintenance_reason = None
                else:
                    raise ValueError("Maintenance reason cannot be set when maintenance is False")
            if self.is_maintenance:
                self._do_maintenance_action(session, "put", {"reason": self.maintenance_reason})
            else:
    ...
    def set_provision_state(
        ...
        body = {"target": target}
        if runbook:
            microversion = self._assert_microversion_for(session, _common.RUNBOOKS_VERSION)

            if clean_steps is not None:
                raise ValueError("Please provide either clean steps or a runbook, but not both.")
            if service_steps is not None:
                raise ValueError("Please provide either service steps or a runbook, but not both.")
        ...
        if config_drive:
            if target not in ("active", "rebuild"):
                raise ValueError(
                    'Config drive can only be provided with "active" and "rebuild" targets'
                )
            if isinstance(config_drive, bytes):
                try:
                    config_drive = config_drive.decode("utf-8")
                except UnicodeError:
                    raise ValueError("Config drive must be a dictionary or a base64 encoded string")
        ...
        if deploy_steps is not None:
            if target not in ("active", "rebuild"):
                raise ValueError(
                    'Deploy steps can only be provided with "deploy" and "rebuild" target'
                )
        ...
        if wait:
            try:
                ...
                raise ValueError(
                    f"For target {target} the expected state is not known, cannot wait for it"
                )
        ...
        msg = f"Failed to set provision state for bare metal node {self.id} to {target}"
    ...
    def wait_for_power_state(self, session, expected_state, timeout=None):
        ...
        for count in utils.iterate_timeout(
            timeout,
            f"Timeout waiting for node {self.id} to reach power state '{expected_state}'",
        ):
            self.fetch(session)
    ...
    def wait_for_provision_state(
        ...
        for count in utils.iterate_timeout(
            timeout,
            f"Timeout waiting for node {self.id} to reach target state '{expected_state}'",
        ):
            self.fetch(session)
    ...
    def _check_state_reached(self, session, expected_state, abort_on_failed_state=True):
        ...
        # "enroll"
            ...
            expected_state == "manageable" and self.provision_state == "enroll" and self.last_error
        ):
            raise exceptions.ResourceFailure(
    ...
    def inject_nmi(self, session):
        ...
        """
        session = self._get_session(session)
        version = self._assert_microversion_for(session, _common.INJECT_NMI_VERSION)
        request = self._prepare_request(requires_id=True)
    ...
    def set_power_state(self, session, target, wait=False, timeout=None):
        ...
        msg = f"Failed to set power state for bare metal node {self.id} to {target}"
        exceptions.raise_from_response(response, error_message=msg)
    ...
    def detach_vmedia(self, session, device_types=None):
        ...
        if device_types:
            delete_kwargs["json"] = {"device_types": _common.comma_separated_list(device_types)}
    ...
    def attach_vif(
        ...
        *,
        ...
        if port_id and port_group_id:
            msg = "Only one of vif_port_id and vif_portgroup_id can be provided"
        ...
        if required:
            ...
            if failed:
                raise exceptions.ValidationException(
                    "Validation failed for required interfaces of node {node}: {failures}".format(
    ...
    def set_boot_mode(self, session, target):
        ...
        if target not in ("uefi", "bios"):
            raise ValueError(
                f"Unrecognized boot mode {target}.Boot mode should be one of 'uefi' or 'bios'."
            )
        body = {"target": target}
    ...
    def call_vendor_passthru(self, session, verb, method, body=None):
        ...
        msg = f"Failed to call vendor_passthru for node {self.id}, verb {verb} and method {method}"
        exceptions.raise_from_response(response, error_message=msg)
    ...
    def get_node_inventory(self, session, node_id):
        ...
        if node_id is not None:
            warnings.warn(
                "The 'node_id' field is unnecessary and will be removed in a future release.",
    ...
    def patch(
        ...
        *,
        ...
        if reset_interfaces is not None:
            ...
            session = self._get_session(session)
            microversion = self._assert_microversion_for(session, _common.RESET_INTERFACES_VERSION)
            ...
            return super().patch(session, patch=patch, retry_on_conflict=retry_on_conflict)
