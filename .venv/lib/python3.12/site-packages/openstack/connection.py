...
if requestsexceptions.SubjectAltNameWarning:
    warnings.filterwarnings("ignore", category=requestsexceptions.SubjectAltNameWarning)
...
def from_config(
    ...
    **kwargs: ty.Any,
    ...
    if config is None:
        config = _config.OpenStackConfig().get_one(cloud=cloud, argparse=options, **kwargs)
    ...
    def __init__(
        ...
        **kwargs: ty.Any,
        ...
        if vendor_hook:
            try:
                ...
                try:
                    ...
                    self.log.warning("Hook should be in the entrypoint module:attribute format")
            except (ImportError, TypeError, AttributeError) as e:
                self.log.warning("Configured hook %s cannot be executed: %s", vendor_hook, e)
        ...
        if self.config._influxdb_config and "additional_metric_tags" in self.config.config:
            self.config._influxdb_config["additional_metric_tags"] = self.config.config[
                "additional_metric_tags"
            ]

    def add_service(self, service: service_description.ServiceDescription) -> None:
