...
class Queue(resource.Resource):
    ...
    def create(self, session, prepend_key=False, base_path=None, **kwargs):
        request = self._prepare_request(requires_id=True, prepend_key=prepend_key, base_path=None)
        headers = {
            "Client-ID": self.client_id or str(uuid.uuid4()),
            "X-PROJECT-ID": self.project_id or session.get_project_id(),
        }
        request.headers.update(headers)
        response = session.put(request.url, json=request.body, headers=request.headers)
    ...
    def list(cls, session, paginated=False, base_path=None, **params):
        ...
        headers = {
            ...
            "X-PROJECT-ID": params.get("project_id", None) or session.get_project_id(),
        }
    ...
    def fetch(
        ...
        **kwargs,
        ...
        request.headers.update(headers)
        response = session.get(request.url, headers=headers, skip_cache=skip_cache)
        self._translate_response(response)

        return self

    def delete(self, session, error_message=None, *, microversion=None, **kwargs):
