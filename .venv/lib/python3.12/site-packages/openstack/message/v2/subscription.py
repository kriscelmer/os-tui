...
class Subscription(resource.Resource):
    ...
    def create(self, session, prepend_key=False, base_path=None, **kwargs):
        ...
        response = session.post(request.url, json=request.body, headers=request.headers)
    ...
    def fetch(
        ...
        **kwargs,
        ...
        request.headers.update(headers)
        response = session.get(request.url, headers=request.headers, skip_cache=skip_cache)
        self._translate_response(response)
