...
class Proxy(proxy.Proxy):
        ...
        * Image tasks
        ...
        if validate_checksum and data and not isinstance(data, bytes):
            raise exceptions.SDKException(
                "Validating checksum is not possible when data is not a direct binary object"
            )
        ...
        if allow_duplicates:
            ...
            if current_image:
                ...
                if up_to_date:
                    ...
                    self.log.debug(
                        "image %(name)s exists, but contains different checksums. Updating.",
                        {"name": name},
                    )

        if disable_vendor_agent:
            kwargs.update(self._connection.config.config["disable_vendor_agent"])
    ...
    def import_image(
        ...
        *,
        ...
        if store is not None:
            if stores:
                raise exceptions.InvalidRequest("store and stores are mutually exclusive")
        ...
        if not all([image.container_format, image.disk_format]):
            raise exceptions.InvalidRequest(
                "Both container_format and disk_format are required for importing an image"
            )
    ...
    def upload_image(
        ...
        **attrs,
        ...
        if not all([container_format, disk_format]):
            raise exceptions.InvalidRequest("Both container_format and disk_format are required")
    ...
    def _upload_image(
        ...
        **kwargs,
        ...
        try:
            ...
            raise exceptions.SDKException(f"Image creation failed: {str(e)}") from e
    ...
    def _upload_image_put(
        ...
        **image_kwargs,
        ...
        if all_stores and stores:
            raise exceptions.InvalidRequest("all_stores is mutually exclusive with stores")
        ...
        supports_import = image.image_import_methods and import_method in image.image_import_methods
    ...
    def get_member(self, member, image):
        ...
        member_id = resource.Resource._get_id(member)
        image_id = resource.Resource._get_id(image)
        return self._get(_member.Member, member_id=member_id, image_id=image_id)
    ...
    def delete_all_metadef_objects(self, namespace):
        ...
        :raises: :class:`~openstack.exceptions.NotFoundException` when no
            resource can be found.
        """
        namespace = self._get_resource(_metadef_namespace.MetadefNamespace, namespace)
    ...
    def get_metadef_property(self, metadef_property, metadef_namespace, **query):
