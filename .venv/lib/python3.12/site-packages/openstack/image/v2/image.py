...
class Image(resource.Resource, tag.TagMixin, _download.DownloadMixin):
    ...
    def import_image(
        ...
        *,
        ...
        if store and stores:
            raise exceptions.InvalidRequest(
                "store and stores are mutually exclusive. stores should be preferred."
            )
        if store:
            stores = [store]
        else:
        ...
        if uri:
            if method != "web-download":
                raise exceptions.InvalidRequest('URI is only supported with method: "web-download"')
            data["method"]["uri"] = uri

        if remote_region and remote_image_id:
            if remote_service_interface:
                data["method"]["glance_service_interface"] = remote_service_interface
