...
def _verify_checksum(md5, checksum):
    if checksum:
        ...
        if digest != checksum:
            raise exceptions.InvalidResponse(f"checksum mismatch: {checksum} != {digest}")
...
class DownloadMixin:
    ...
    def download(
        ...
        if checksum is not None:
            _verify_checksum(hashlib.md5(resp.content, usedforsecurity=False), checksum)
        else:
            session.log.warning("Unable to verify the integrity of image %s", (self.id))
