...
class ComputeCloudMixin(_network_common.NetworkCommonCloudMixin):
    ...
    def get_flavor_by_ram(self, ram, include=None, get_extra=True):
        ...
        for flavor in sorted(flavors, key=operator.itemgetter("ram")):
            if flavor["ram"] >= ram and (not include or include in flavor["name"]):
                return flavor
        raise exceptions.SDKException(f"Could not find a flavor with {ram} and '{include}'")
    ...
    def remove_server_security_groups(self, server, security_groups):
        ...
        for sg in security_groups:
            try:
                ...
                self.log.debug(
                    "The security group %s was not present on server %s so no action was performed",
                    sg.name,
                    server.name,
                )
                ret = False
    ...
    def get_keypair(self, name_or_id, filters=None, *, user_id=None):
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_keypairs' instead",
    ...
    def get_flavor(self, name_or_id, filters=None, get_extra=True):
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_flavors' instead",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def get_server(
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_servers' instead",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def get_server_group(self, name_or_id=None, filters=None):
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_server_groups' instead",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def create_image_snapshot(
        ...
        **metadata,
        ...
        if not isinstance(server, dict):
            ...
            if not server_obj:
                raise exceptions.SDKException(
                    f"Server {server} could not be found and therefore could not be snapshotted."
                )
            server = server_obj
    ...
    def create_server(
        ...
        **kwargs,
        ...
        if not flavor:
            raise TypeError("create_server() missing 1 required argument: 'flavor'")
        if not image and not boot_volume:
            raise TypeError("create_server() requires either 'image' or 'boot_volume'")
        ...
        if group:
            if isinstance(group, dict):
                ...
                group_obj = self.compute.find_server_group(group, ignore_missing=False)
        ...
        if "nics" in kwargs and not isinstance(kwargs["nics"], list):
            if isinstance(kwargs["nics"], dict):
                ...
                raise exceptions.SDKException(
                    "nics parameter to create_server takes a list of dicts. Got: {nics}".format(
                        nics=kwargs["nics"]
                    )
                )
        ...
        if network and ("nics" not in kwargs or not kwargs["nics"]):
            ...
            for net in network:
                if isinstance(net, dict):
                    ...
                    network_obj = self.network.find_network(net, ignore_missing=False)
        ...
        for nic in kwargs.pop("nics", []):
            ...
            if "net-id" in nic:
                ...
                nic_net = self.network.find_network(net_name, ignore_missing=False)
                net["uuid"] = nic_net["id"]
            ...
            if nic:
                raise exceptions.SDKException(
                    f"Additional unsupported keys given for server network creation: {nic.keys()}"
                )
            networks.append(net)
        if networks:
        ...
        if not wait:
            ...
            if server["status"] == "ERROR":
                ...
                raise exceptions.SDKException(
                    "Error in creating the server (no further information available)",
                    extra_data=dict(server=server),
                )
    ...
    def _get_boot_from_volume_kwargs(
        ...
        if boot_volume:
            if isinstance(boot_volume, dict):
                ...
                volume = self.block_storage.find_volume(boot_volume, ignore_missing=False)
                volume_id = volume["id"]
            block_mapping = {
                "boot_index": "0",
        ...
        for volume in volumes:
            if isinstance(volume, dict):
                ...
                volume_obj = self.block_storage.find_volume(volume, ignore_missing=False)
    ...
    def delete_server_metadata(self, name_or_id, metadata_keys):
        ...
        self.compute.delete_server_metadata(server=server.id, keys=metadata_keys)
    ...
    def delete_server_group(self, name_or_id):
        ...
        if not server_group:
            self.log.debug("Server group %s not found for deleting", name_or_id)
            return False
    ...
    def delete_flavor(self, name_or_id):
        ...
        try:
            ...
            raise exceptions.SDKException(f"Unable to delete flavor {name_or_id}")
    ...
    def get_aggregate(self, name_or_id, filters=None):
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_aggregates' instead",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def create_aggregate(self, name, availability_zone=None):
        ...
        :raises: :class:`~openstack.exceptions.SDKException` on operation
            error.
        """
        return self.compute.create_aggregate(name=name, availability_zone=availability_zone)
    ...
    def set_aggregate_metadata(self, name_or_id, metadata):
        ...
        aggregate = self.get_aggregate(name_or_id)
        if not aggregate:
            raise exceptions.SDKException(f"Host aggregate {name_or_id} not found.")
