...
class CoeCloudMixin(openstackcloud._OpenStackCloudMixin):
    ...
    def update_coe_cluster(self, name_or_id, **kwargs):
        ...
        cluster = self.get_coe_cluster(name_or_id)
        if not cluster:
            raise exceptions.SDKException(f"COE cluster {name_or_id} not found.")
    ...
    def delete_cluster_template(self, name_or_id):
        ...
        self.container_infrastructure_management.delete_cluster_template(cluster_template)
        return True
    ...
    def update_cluster_template(self, name_or_id, **kwargs):
        ...
        cluster_template = self.get_cluster_template(name_or_id)
        if not cluster_template:
            raise exceptions.SDKException(f"Cluster template {name_or_id} not found.")

        cluster_template = self.container_infrastructure_management.update_cluster_template(
