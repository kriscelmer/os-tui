...
class BlockStorageCloudMixin(openstackcloud._OpenStackCloudMixin):
    ...
    def get_volume(self, name_or_id, filters=None):
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_volumes' instead",
    ...
    def get_volume_type(self, name_or_id, filters=None):
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_volume_types' instead",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def set_volume_bootable(self, name_or_id, bootable=True):
        ...
        if not volume:
            raise exceptions.SDKException(f"Volume {name_or_id} does not exist")
    ...
    def delete_volume(
        ...
        :raises: :class:`~openstack.exceptions.SDKException` on operation
            error.
        """
        volume = self.block_storage.find_volume(name_or_id, ignore_missing=True)
        if not volume:
            self.log.debug(
    ...
    def attach_volume(
        ...
        if volume["status"] != "available":
            raise exceptions.SDKException(
                "Volume {} is not available. Status is '{}'".format(volume["id"], volume["status"])
            )
    ...
    def get_volume_snapshot(self, name_or_id, filters=None):
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_volume_snapshots' instead",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def get_volume_backup(self, name_or_id, filters=None):
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_volume_backups' instead",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def get_volume_type_access(self, name_or_id):
        ...
        if not volume_type:
            raise exceptions.SDKException(f"VolumeType not found: {name_or_id}")
