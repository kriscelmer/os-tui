...
def _get_entity(cloud, resource, name_or_id, filters, **kwargs):
    ...
    if hasattr(name_or_id, "id") or (isinstance(name_or_id, dict) and "id" in name_or_id):
        return name_or_id
    ...
    search = resource if callable(resource) else getattr(cloud, f"search_{resource}s", None)
    if search:
        entities = search(name_or_id, filters, **kwargs)
        if entities:
            if len(entities) > 1:
                raise exceptions.SDKException(f"Multiple matches found for {name_or_id}")
            return entities[0]
    return None
    ...
    def func_wrapper(func, *args, **kwargs):
        ...
        for k in kwargs:
            if k not in argspec.args[1:] and k not in valid_args:
                raise TypeError(
                    f"{inspect.stack()[1][3]}() got an unexpected keyword argument '{k}'"
                )
        ...
        if (key in d) and (d[key] is not None):
            try:
                ...
                raise exceptions.SDKException(
                    f"Search for minimum value failed. Value for {key} is not an integer: {d[key]}"
                )
    ...
    for d in data:
        if (key in d) and (d[key] is not None):
            try:
                ...
                raise exceptions.SDKException(
                    f"Search for maximum value failed. Value for {key} is not an integer: {d[key]}"
                )
