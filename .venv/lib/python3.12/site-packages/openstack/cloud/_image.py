...
class ImageCloudMixin(openstackcloud._OpenStackCloudMixin):
    ...
    def get_image(self, name_or_id, filters=None):
        ...
        if filters is not None:
            warnings.warn(
                "The 'filters' argument is deprecated; use 'search_images' instead",
    ...
    def wait_for_image(self, image, timeout=3600):
        ...
        for count in utils.iterate_timeout(timeout, "Timeout waiting for image to snapshot"):
            ...
            if image["status"] == "active":
                ...
                raise exceptions.SDKException(f"Image {image_id} hit error state")
    ...
    def create_image(
        ...
        **kwargs,
        ...
        try:
            for count in utils.iterate_timeout(timeout, "Timeout waiting for the image to finish."):
                image_obj = self.get_image(image.id)
                if image_obj and image_obj.status not in ("queued", "saving"):
                    return image_obj
        except exceptions.ResourceTimeout:
            self.log.debug("Timeout waiting for image to become ready. Deleting.")
            self.delete_image(image.id, wait=True)
            raise

    def update_image_properties(self, image=None, name_or_id=None, meta=None, **properties):
        image = image or name_or_id
        return self.image.update_image_properties(image=image, meta=meta, **properties)
