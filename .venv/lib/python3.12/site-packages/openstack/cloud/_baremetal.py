    ...
    for row in nics:
        if isinstance(row, str):
            ...
            try:
                ...
                raise TypeError(f"Either 'address' or 'mac' must be provided for port {row}")
...
class BaremetalCloudMixin(openstackcloud._OpenStackCloudMixin):
    ...
    def _delete_node_on_error(self, node):
        try:
            ...
            self.log.debug("cleaning up node %s because of an error: %s", node.id, exc)
            tb = sys.exc_info()[2]
            try:
                self.baremetal.delete_node(node)
            except Exception:
                self.log.debug("could not remove node %s", node.id, exc_info=True)
    ...
    def unregister_machine(self, nics, uuid, wait=None, timeout=600):
        ...
        if machine["provision_state"] in invalid_states:
            raise exceptions.SDKException(
                "Error unregistering node '{}' due to current provision state '{}'".format(
                    uuid, machine["provision_state"]
                )
            )
    ...
    def update_machine(self, name_or_id, **attrs):
        ...
        if not machine:
            raise exceptions.SDKException(f"Machine update failed to find Machine: {name_or_id}. ")

        new_config = dict(machine._to_munch(), **attrs)

        try:
            patch = jsonpatch.JsonPatch.from_diff(machine._to_munch(), new_config)
    ...
    def list_ports_attached_to_machine(self, name_or_id):
        ...
        return [self.network.find_port(vif, ignore_missing=False) for vif in vif_ids]
    ...
    def activate_node(self, uuid, configdrive=None, wait=False, timeout=1200):
        self.node_set_provision_state(uuid, "active", configdrive, wait=wait, timeout=timeout)
