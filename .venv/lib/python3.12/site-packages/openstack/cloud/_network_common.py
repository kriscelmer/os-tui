...
class NetworkCommonCloudMixin(openstackcloud._OpenStackCloudMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._use_external_network = self.config.config.get("use_external_network", True)
        self._use_internal_network = self.config.config.get("use_internal_network", True)
    ...
    def _set_interesting_networks(self):
        ...
        for network in all_networks:
            ...
            if (
                ...
                (network.is_router_external or network.provider_physical_network)
    ...
    def get_internal_networks(self):
        ...
        """
        self._find_interesting_networks()
        return list(self._internal_ipv4_networks) + list(self._internal_ipv6_networks)
    ...
    def list_floating_ips(self, filters=None):
        ...
        if self._use_neutron_floating():
            try:
                ...
                self.log.debug(
                    "Something went wrong talking to neutron API: '%(msg)s'. Trying with Nova.",
                    {"msg": str(e)},
                )
    ...
    def _neutron_available_floating_ips(self, network=None, project_id=None, server=None):
        ...
        if network:
            ...
            if floating_network_id is None:
                raise exceptions.NotFoundException(f"unable to find external network {network}")
        else:
    ...
    def _nova_available_floating_ips(self, pool=None):
        ...
        with _utils.openstacksdk_exceptions(f"Unable to create floating IP in pool {pool}"):
            if pool is None:
                pools = self.list_floating_ip_pools()
                if not pools:
    ...
    def create_floating_ip(
        ...
        f_ips = self._normalize_floating_ips([self._nova_create_floating_ip(pool=network)])
        return f_ips[0]
    ...
    def _neutron_create_floating_ip(
        ...
        if not network_id:
            if network_name_or_id:
                try:
                    network = self.network.find_network(network_name_or_id, ignore_missing=False)
                except exceptions.NotFoundException:
                    raise exceptions.NotFoundException(
                        f"unable to find network for floating ips with ID {network_name_or_id}"
                    )
        ...
        if port:
            ...
            if wait:
                try:
                    ...
                    self.log.error(
                        "Timed out on floating ip %(fip)s becoming active. Deleting",
                        {"fip": fip_id},
                    )
                    try:
            ...
            if fip["port_id"] != port:
                if server:
                    ...
                    raise exceptions.SDKException(
                        f"Attempted to create FIP on port {port} but something went wrong"
                    )
        return fip
    ...
    def _nova_create_floating_ip(self, pool=None):
        with _utils.openstacksdk_exceptions(f"Unable to create floating IP in pool {pool}"):
            ...
            data = proxy._json_response(
                self.compute.get("/os-floating-ips/{id}".format(id=pool_ip["id"]))
            )
    ...
    def _nova_attach_ip_to_server(self, server_id, floating_ip_id, fixed_address=None):
        f_ip = self.get_floating_ip(id=floating_ip_id)
        if f_ip is None:
            raise exceptions.SDKException(f"unable to find floating IP {floating_ip_id}")
        error_message = f"Error attaching IP {floating_ip_id} to instance {server_id}"
    ...
    def _nova_detach_ip_from_server(self, server_id, floating_ip_id):
        ...
        if f_ip is None:
            raise exceptions.SDKException(f"unable to find floating IP {floating_ip_id}")
        error_message = f"Error detaching IP {floating_ip_id} from instance {server_id}"
        return proxy._json_response(
            self.compute.post(
                f"/servers/{server_id}/action",
                json=dict(removeFloatingIp=dict(address=f_ip["floating_ip_address"])),
            ),
            error_message=error_message,
        )
    ...
    def add_auto_ip(self, server, wait=False, timeout=60, reuse=True):
        ...
        """
        server = self._add_auto_ip(server, wait=wait, timeout=timeout, reuse=reuse)
    ...
    def _needs_floating_ip(self, server, nat_destination):
        ...
        if server["public_v4"] or any(
            [
                any([address["OS-EXT-IPS:type"] == "floating" for address in addresses])
        ...
        if not server["private_v4"] and not any(
            [
                any([address["OS-EXT-IPS:type"] == "fixed" for address in addresses])
                for addresses in (server["addresses"] or {}).values()
            ]
        ):
            return False
    ...
    def _nat_destination_port(self, server, fixed_address=None, nat_destination=None):
        ...
        if not fixed_address:
            if len(ports) > 1:
                if nat_destination:
                    nat_network = self.network.find_network(nat_destination, ignore_missing=True)
                    if not nat_network:
                        raise exceptions.SDKException(
            ...
            raise exceptions.SDKException(
                "unable to find a free fixed IPv4 address for server {}".format(server["id"])
            )
    ...
    def _use_neutron_floating(self):
        return self.has_service("network") and self._floating_ip_source == "neutron"
    ...
    def create_security_group(self, name, description, project_id=None, stateful=None):
    ...
    def delete_security_group(self, name_or_id):
        ...
        if secgroup is None:
            self.log.debug("Security group %s not found for deleting", name_or_id)
            return False

        if self._use_neutron_secgroups():
            self.network.delete_security_group(secgroup["id"], ignore_missing=False)
            return True

        else:
            proxy._json_response(
                self.compute.delete("/os-security-groups/{id}".format(id=secgroup["id"]))
            )
            return True
    ...
    def update_security_group(self, name_or_id, **kwargs):
        ...
        if group is None:
            raise exceptions.SDKException(f"Security group {name_or_id} not found.")
    ...
    def create_security_group_rule(
        ...
        if not secgroup:
            raise exceptions.SDKException(f"Security group {secgroup_name_or_id} not found.")
        ...
        if self._use_neutron_secgroups():
            ...
            rule_def = {
                ...
                "port_range_min": (None if port_range_min == -1 else port_range_min),
                "port_range_max": (None if port_range_max == -1 else port_range_max),
                "protocol": protocol,
            ...
            if direction == "egress":
                self.log.debug("Rule creation failed: Nova does not support egress rules")
    ...
    def delete_security_group_rule(self, rule_id):
        ...
        if self._use_neutron_secgroups():
            self.network.delete_security_group_rule(rule_id, ignore_missing=False)
            return True
