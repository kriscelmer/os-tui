...
class IdentityCloudMixin(openstackcloud._OpenStackCloudMixin):
        ...
        if utils.supports_version(self.identity, "3"):
            if not domain_id:
                raise exceptions.SDKException(
                    "User or project creation requires an explicit domain_id argument."
                )
            else:
    ...
    def get_project(self, name_or_id, filters=None, domain_id=None):
        ...
        if filters is not None:
            warnings.warn(
                "the 'filters' argument is deprecated; use 'search_projects' instead",
                os_warnings.RemovedInSDK60Warning,
            )
            entities = self.search_projects(name_or_id, filters, domain_id=domain_id)
            if not entities:
                return None
        ...
        return self.identity.find_project(name_or_id=name_or_id, domain_id=domain_id)
    ...
    def get_user(self, name_or_id, filters=None, domain_id=None):
        ...
        if filters is not None:
            warnings.warn(
                "the 'filters' argument is deprecated; use 'search_user' instead",
                os_warnings.RemovedInSDK60Warning,
            )
            entities = self.search_users(name_or_id, filters, domain_id=domain_id)
            if not entities:
                return None
    ...
    def get_user_by_id(self, user_id, normalize=None):
        ...
        if normalize is not None:
            warnings.warn(
                "The 'normalize' field is unnecessary and will be removed in a future release.",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def _get_user_and_group(self, user_name_or_id, group_name_or_id):
        ...
        if not group:
            raise exceptions.SDKException(f"Group {group_name_or_id} not found")
    ...
    @_utils.valid_kwargs("name", "enabled", "type", "service_type", "description")
    ...
    def get_service(self, name_or_id, filters=None):
        ...
        if filters is not None:
            warnings.warn(
                "the 'filters' argument is deprecated; use 'search_services' instead",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def delete_service(self, name_or_id):
        ...
        try:
            ...
            self.log.exception("Failed to delete service {id}".format(id=service["id"]))
            return False
    ...
    def create_endpoint(
        ...
        **kwargs,
        ...
        if service is None:
            raise exceptions.SDKException(f"service {service_name_or_id} not found")
        ...
        if url:
            ...
            if public_url:
                endpoint_args.update({"url": public_url, "interface": "public"})
                endpoints_args.append(endpoint_args.copy())
            if internal_url:
                endpoint_args.update({"url": internal_url, "interface": "internal"})
                endpoints_args.append(endpoint_args.copy())
            if admin_url:
    ...
    @_utils.valid_kwargs("enabled", "service_name_or_id", "url", "interface", "region")
    ...
    def get_endpoint(self, id, filters=None):
        ...
        if filters is not None:
            warnings.warn(
                "the 'filters' argument is deprecated; use 'search_endpoints' instead",
                os_warnings.RemovedInSDK60Warning,
            )
    ...
    def update_domain(
        ...
        if domain_id is None:
            if name_or_id is None:
                raise exceptions.SDKException("You must pass either domain_id or name_or_id value")
            dom = self.get_domain(None, name_or_id)
            if dom is None:
                raise exceptions.SDKException(f"Domain {name_or_id} not found for updating")
            domain_id = dom["id"]
    ...
    def delete_domain(self, domain_id=None, name_or_id=None):
        ...
        try:
            if domain_id is None:
                ...
                if dom is None:
                    self.log.debug("Domain %s not found for deleting", name_or_id)
                    return False
                domain_id = dom["id"]
    ...
    def get_group(self, name_or_id, filters=None, domain_id=None):
        ...
        if filters is not None:
            ...
            entities = self.search_groups(name_or_id, filters, domain_id=domain_id)
            if not entities:
                return None
        ...
        return self.identity.find_group(name_or_id=name_or_id, domain_id=domain_id)
    ...
    def update_group(
        ...
        **kwargs,
        ...
        :raises: :class:`~openstack.exceptions.SDKException` if something goes
            wrong during the OpenStack API call.
        """
        group = self.identity.find_group(name_or_id, ignore_missing=False, **kwargs)
    ...
    def get_role(self, name_or_id, filters=None, domain_id=None):
        ...
        if filters is not None:
            warnings.warn(
                "the 'filters' argument is deprecated; use 'search_roles' instead",
                os_warnings.RemovedInSDK60Warning,
            )
            entities = self.search_roles(name_or_id, filters, domain_id=domain_id)
            if not entities:
                return None
        ...
        return self.identity.find_role(name_or_id=name_or_id, domain_id=domain_id)
    ...
    def list_role_assignments(self, filters=None):
        ...
        if "os_inherit_extension_inherited_to" in filters:
            ...
            filters["inherited_to"] = filters.pop("os_inherit_extension_inherited_to")
    ...
    def _get_grant_revoke_params(
        ...
        if user:
            data["user"] = self.identity.find_user(user, ignore_missing=False, **search_args)
        if group:
        ...
        if project:
            ...
            if user:
                ...
                if has_role:
                    ...
                    return False
                self.identity.assign_project_role_to_user(project, user, role, inherited=inherited)
            else:
            ...
            if user:
                ...
                if has_role:
                    self.log.debug("Assignment already exists")
                    return False
                self.identity.assign_domain_role_to_user(domain, user, role, inherited=inherited)
            else:
                ...
                if has_role:
                    self.log.debug("Assignment already exists")
                    return False
                self.identity.assign_domain_role_to_group(domain, group, role, inherited=inherited)
        else:
            # Proceed with system
