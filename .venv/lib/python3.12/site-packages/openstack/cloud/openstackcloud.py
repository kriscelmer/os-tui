...
class _OpenStackCloudMixin(_services_mixin.ServicesMixin):
    ...
    def __init__(
        ...
        extra_services: list["service_description.ServiceDescription"] | None = None,
        strict: bool = False,
        ...
        **kwargs: ty.Any,
        ...
        # cert verification
        if not self.verify:
            self.log.debug("Turning off Insecure SSL warnings since verify=False")
        ...
        self._cache = self._make_cache(cache_class, cache_expiration_time, cache_arguments)
        ...
        self._local_ipv6 = _utils.localhost_supports_ipv6() if not self.force_ipv4 else False
    ...
    def _pool_executor(self) -> concurrent.futures.Executor:
        if not self.__pool_executor:
            self.__pool_executor = concurrent.futures.ThreadPoolExecutor(max_workers=5)
    ...
    def _make_cache(
        ...
        return dogpile.cache.make_region(function_key_generator=self._make_cache_key).configure(
    ...
    def _make_cache_key(
        ...
        def generate_key(*args, **kwargs):
            ...
            kwargs_key = ",".join([f"{k}:{kwargs[k]}" for k in kw_keys if k != "cache"])
    ...
    def _keystone_catalog(self) -> "ks_service_catalog.ServiceCatalog":
        if self.session.auth is None:
            raise exceptions.ConfigException("session has no auth information attached")

        if not isinstance(self.session.auth, ks_plugin_base.BaseIdentityPlugin):
            raise exceptions.ConfigException("cannot fetch catalog for non-keystone auth plugin")
    ...
    def get_session_endpoint(
        ...
        try:
            ...
            self.log.debug("Endpoint not found in %s cloud: %s", self.name, str(e))
            endpoint = None
    ...
    def has_service(self, service_key: str, version: str | None = None) -> bool:
        if not self.config.has_service(service_key):
            # TODO(mordred) add a stamp here so that we only report this once
            if not (service_key in self._disable_warnings and self._disable_warnings[service_key]):
                self.log.debug(
    ...
    def search_resources(
        ...
        **filters,
        ...
        if not hasattr(self, service_name):
            raise exceptions.SDKException(f"service {service_name} is not existing/enabled")
