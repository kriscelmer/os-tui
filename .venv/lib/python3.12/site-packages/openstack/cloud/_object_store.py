...
class ObjectStoreCloudMixin(openstackcloud._OpenStackCloudMixin):
    def list_containers(self, full_listing=None, prefix=None):
        ...
        if full_listing is not None:
            warnings.warn(
                "The 'full_listing' field is unnecessary and will be removed in a future release.",
    ...
    def update_container(self, name, headers):
        ...
        """
        self.object_store.set_container_metadata(name, refresh=False, **headers)
    ...
    def get_container_access(self, name):
        ...
        for key, value in OBJECT_CONTAINER_ACLS.items():
            ...
            if str(acl) == str(value):
                return key
        raise exceptions.SDKException(f"Could not determine container access for ACL: {acl}.")
    ...
    def list_objects(self, container, full_listing=True, prefix=None):
        ...
        :raises: :class:`~openstack.exceptions.SDKException` on operation
            error.
        """
        return list(self.object_store.objects(container=container, prefix=prefix))
    ...
    def stream_object(
        ...
        """
        try:
            yield from self.object_store.stream_object(obj, container, chunk_size=resp_chunk_size)
        except exceptions.NotFoundException:
            return
