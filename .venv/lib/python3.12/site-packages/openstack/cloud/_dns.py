...
class DnsCloudMixin(openstackcloud._OpenStackCloudMixin):
    ...
    def get_zone(self, name_or_id, filters=None):
        ...
        if not filters:
            filters = {}
        return self.dns.find_zone(name_or_id=name_or_id, ignore_missing=True, **filters)
    ...
    def get_recordset(self, zone, name_or_id):
        ...
        if not zone_obj:
            raise exceptions.SDKException(f"Zone {name_or_id} not found.")
        return self.dns.find_recordset(zone=zone_obj, name_or_id=name_or_id, ignore_missing=True)
    ...
    def create_recordset(self, zone, name, recordset_type, records, description=None, ttl=None):
        """Create a recordset.
