...
def get_string_format_keys(fmt_string: str, old_style: bool = True) -> list[str]:
...
def supports_microversion(
    ...
    if (
        ...
        if adapter.default_microversion is not None:
            ...
            candidate = discover.normalize_version_number(adapter.default_microversion)
    ...
    if raise_exception:
        raise exceptions.SDKException(
            f"Required microversion {microversion} is not supported by the server side"
        )
...
def pick_microversion(session: ks_adapter.Adapter, required: str) -> str | None:
...
def maximum_supported_microversion(
    ...
    server_max = discover.normalize_version_number(endpoint_data.max_microversion)

    if endpoint_data.min_microversion:
        server_min = discover.normalize_version_number(endpoint_data.min_microversion)
...
def _calculate_data_hashes(
    ...
    if isinstance(data, io.BufferedIOBase):
        ...
        raise TypeError("unsupported type for data; expected IO stream or bytes; got {type(data)}")
