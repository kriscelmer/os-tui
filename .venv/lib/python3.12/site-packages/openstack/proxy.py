...
class Proxy(adapter.Adapter):
        ...
        **kwargs: ty.Any,
        ...
        try:
            if conn.cache_enabled and not skip_cache and method == "GET":
                ...
                expiration_time = int(conn._cache_expirations.get(key_prefix, 0))
    ...
    def _extract_name(
        ...
        url_parts = [
            ...
            if (x != project_id and (not project_id or (project_id and x != "AUTH_" + project_id)))
        ]
    ...
    def _extract_name_consume_url_parts(self, url_parts: list[str]) -> list[str]:
    ...
    def _report_stats_statsd(
        ...
        try:
            ...
            name_parts = [
                ...
                for f in self._extract_name(url, self.service_type, self.session.get_project_id())
            ]
            key = ".".join(
                [
                    self._statsd_prefix,
    ...
    def _report_stats_prometheus(
        ...
        endpoint = f"{parsed_url.scheme}://{parsed_url.netloc}{parsed_url.path}"  # type: ignore[str-bytes-safe]
        if response is not None:
            labels = {
                "method": method,
                "endpoint": endpoint,
    ...
    def _get_connection(self) -> connection.Connection | None:
        ...
        """
        return getattr(self, "_connection", getattr(self.session, "_sdk_connection", None))
    ...
    def _get_resource(
        ...
        **attrs: ty.Any,
        ...
        if value is None:
        ...
        elif isinstance(value, dict) and not isinstance(value, resource.Resource):
            ...
            if not isinstance(value, resource_type):
                raise ValueError(
                    f"Expected {resource_type.__name__} but received {value.__class__.__name__}"
                )
            res = value
            res._update(**attrs)
    ...
    def _find(
        ...
        **attrs: ty.Any,
        ...
        """
        return resource_type.find(self, name_or_id, ignore_missing=ignore_missing, **attrs)
    ...
    def _list(
        ...
        **attrs: ty.Any,
        ...
        data = resource_type.list(self, paginated=paginated, base_path=base_path, **attrs)
    ...
    def _service_cleanup_del_res(
        ...
        try:
            if resource_evaluation_fn and callable(resource_evaluation_fn):
                ...
                need_delete = self._service_cleanup_resource_filters_evaluation(
                    obj, filters=filters
                )
            ...
            for k, v in filters.items():
                try:
                    ...
                    if res_val:
                        ...
                        self.log.debug(
                            f"Requested cleanup attribute {k} is not available on the resource"
                        )
                        part_cond.append(False)
                except Exception:
