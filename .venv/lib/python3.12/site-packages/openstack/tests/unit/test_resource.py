...
class TestResource(base.TestCase):
    ...
    def test_repr(self):
        ...
        class Test(resource.Resource):
            def __init__(self):
                ...
                self._header.attributes.items = mock.Mock(return_value=b.items())

                self._uri = mock.Mock()
                self._uri.attributes.items = mock.Mock(return_value=c.items())

                self._computed = mock.Mock()
                self._computed.attributes.items = mock.Mock(return_value=d.items())
    ...
    def test__attributes(self):
        ...
        self.assertEqual(
            sorted(["foo", "bar", "_bar", "bar_local", "id", "name", "location"]),
        ...
        self.assertEqual(
            sorted(["foo", "bar", "_bar", "bar_remote", "id", "name", "location"]),
    ...
    def test__attributes_iterator(self):
        ...
        for attr, component in sot._attributes_iterator(components=(fields.Header,)):
            if attr in expected:
                expected.remove(attr)
        self.assertEqual([], expected)
    ...
    def test__prepare_request_with_patch_params(self):
        ...
        result = sot._prepare_request(requires_id=True, patch=True, params=params)
    ...
    def test_unknown_attrs_in_body_not_allowed_to_set(self):
        ...
        try:
            ...
            self.assertNotIn("unknown_param", sot)
            return
        self.fail("Parameter 'unknown_param' unexpectedly set through the dict interface")
...
class TestResourceActions(base.TestCase):
    ...
    def test_list_response_paginated_with_max_items(self):
        ...
        results = self.sot.list(self.session, limit=3, paginated=True, max_items=2)
        ...
        results = self.sot.list(self.session, limit=1, paginated=True, max_items=3)
        result0 = next(results)
        self.assertEqual(result0.id, ids[0])
        result1 = next(results)
        self.assertEqual(result1.id, ids[1])
    ...
    def test_list_with_injected_headers(self):
        ...
        _ = list(self.test_class.list(self.session, headers={"X-Test": "value"}))

        expected = {"Accept": "application/json", "X-Test": "value"}
        self.assertEqual(expected, self.session.get.call_args.kwargs["headers"])
    ...
    def test_list_dns_with_headers(self, mock_resource_list):
        ...
        self.assertEqual(expected, mock_resource_list.call_args.kwargs["headers"])
