...
class TestNode(base.TestCase):
    ...
    def test_instantiate(self):
        ...
        self.assertEqual(FAKE["automated_clean"], sot.is_automated_clean_enabled)
        ...
        self.assertEqual(FAKE["driver_internal_info"], sot.driver_internal_info)
        ...
        self.assertEqual(FAKE["target_provision_state"], sot.target_provision_state)
...
class TestNodeSetProvisionState(base.TestCase):
    ...
    def test_deploy_with_configdrive_as_bytestring(self):
        ...
        result = self.node.set_provision_state(self.session, "active", config_drive=config_drive)
        self.assertIs(result, self.node)
        self.session.put.assert_called_once_with(
            f"nodes/{self.node.id}/states/provision",
    ...
    def test_set_provision_state_clean_runbook(self):
        ...
        result = self.node.set_provision_state(self.session, "clean", runbook=runbook)
    ...
    def test_set_provision_state_service_runbook(self):
        runbook = "CUSTOM_AWESOME"
        result = self.node.set_provision_state(self.session, "service", runbook=runbook)
...
class TestNodeVif(base.TestCase):
    ...
    def test_attach_vif_with_port_uuid(self):
        self.assertIsNone(
            self.node.attach_vif(self.session, self.vif_id, port_id=self.vif_port_uuid)
        )
        self.session.post.assert_called_once_with(
            f"nodes/{self.node.id}/vifs",
...
class TestNodeVmedia(base.TestCase):
    ...
    def test_attach_vmedia(self):
        self.assertIsNone(self.node.attach_vmedia(self.session, self.device_type, self.image_url))
        self.session.post.assert_called_once_with(
