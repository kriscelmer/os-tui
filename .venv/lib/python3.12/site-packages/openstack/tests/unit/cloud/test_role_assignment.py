...
class TestRoleAssignment(base.TestCase):
    ...
    def setUp(self, cloud_config_fixture="clouds.yaml"):
        ...
        self.user_data = self._get_user_data(domain_id=self.domain_data.domain_id)
        self.project_data = self._get_project_data(domain_id=self.domain_data.domain_id)
        ...
        self.group_data = self._get_group_data(domain_id=self.domain_data.domain_id)
        ...
        if not use_name:
            ...
            return [
                ...
                dict(
                    ...
                    uri=self.get_mock_url(
                        resource=resource + "s",
                        qs_elements=["name=" + getattr(data, attr)] + qs_elements,
                    ),
                    status_code=200,
    ...
    def __user_mocks(self, user_data, use_name, is_found=True, domain_data=None):
        qs_elements = []
        if domain_data:
        ...
        if not use_name:
            ...
            uri_mocks += [
                ...
                dict(
                    ...
                    status_code=200,
                    json={"users": ([user_data.json_response["user"]] if is_found else [])},
                ),
            ]
        return uri_mocks
    ...
    def test_grant_bad_domain_exception(self):
        self.register_uris(
            [
                dict(
                    method="GET",
                    uri=self.get_mock_url(resource="domains", append=["baddomain"]),
                    status_code=404,
                ),
                dict(
                    method="GET",
                    uri=self.get_mock_url(resource="domains", qs_elements=["name=baddomain"]),
                    status_code=404,
                ),
            ]
        )
