...
class TestObject(BaseTestObject):
    ...
    def test_update_container(self):
        headers = {"x-container-read": _object_store.OBJECT_CONTAINER_ACLS["public"]}
        self.register_uris(
            [
                dict(
                    method="POST",
        ...
        self.register_uris(
            [
                dict(
                    ...
                    status_code=204,
                    validate=dict(
                        headers={"x-container-read": _object_store.OBJECT_CONTAINER_ACLS["private"]}
                    ),
                )
            ]
        )
        ...
        self.register_uris(
            [
                dict(
                    ...
                    uri=self.container_endpoint,
                    headers={
                        "x-container-read": str(_object_store.OBJECT_CONTAINER_ACLS["public"])
                    },
                )
            ]
        )
        ...
        self.register_uris(
            [
                ...
                dict(
                    method="HEAD",
                    uri=self.endpoint + "/",
                    headers={"X-Account-Meta-Temp-Url-Key": "amazingly-secure-key"},
                ),
            ]
        )
        self.assertEqual(
    ...
    def test_get_object_not_found(self):
        self.register_uris([dict(method="GET", uri=self.object_endpoint, status_code=404)])
    ...
    def test_create_dynamic_large_object(self):
        ...
        uris_to_mock.extend(
            [
                ...
                for index, offset in enumerate(range(0, len(self.content), max_file_size))
            ]
        )
