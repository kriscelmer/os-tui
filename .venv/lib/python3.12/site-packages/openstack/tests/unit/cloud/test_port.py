...
class TestPort(base.TestCase):
    mock_neutron_port_create_rep = {
        "port": {
            ...
            "binding:profile": {},
            "fixed_ips": [{"subnet_id": "test-subnet-id", "ip_address": "29.29.29.29"}],
            "id": "test-port-id",
            "security_groups": [],
            "device_id": "",
        }
    ...
    def test_create_port(self):
        self.register_uris(
            [
                dict(
                    method="POST",
                    uri=self.get_mock_url("network", "public", append=["v2.0", "ports"]),
    ...
    def test_update_port(self):
        ...
        self.register_uris(
            [
                dict(
                    method="GET",
                    uri=self.get_mock_url("network", "public", append=["v2.0", "ports", port_id]),
                ...
                dict(
                    ...
                    validate=dict(json={"port": {"name": "test-port-name-updated"}}),
                ),
            ]
        )
        port = self.cloud.update_port(name_or_id=port_id, name="test-port-name-updated")
    ...
    def test_delete_subnet_multiple_found(self):
        ...
        self.assertRaises(exceptions.SDKException, self.cloud.delete_port, port_name)
        self.assert_calls()
