...
class TestCloudEndpoints(base.TestCase):
    def get_mock_url(
        ...
        return super().get_mock_url(service_type, interface, resource, append, base_url_append)
    ...
    def test_create_endpoint(self):
        ...
        self.register_uris(
            [
                dict(
                    method="GET",
                    uri=self.get_mock_url(resource="services", append=[service_data.service_id]),
                    status_code=200,
                ...
                dict(
                    ...
                    validate=dict(json=public_endpoint_data_disabled.json_request),
                ),
                dict(
                    method="GET",
                    uri=self.get_mock_url(resource="services", append=[service_data.service_id]),
        ...
        self.register_uris(
            [
                dict(
                    ...
                    uri=self.get_mock_url(),
                    status_code=200,
                    json={"endpoints": [e.json_response["endpoint"] for e in endpoints_data]},
                )
            ]
        )
