...
standard_fake_server = fakes.make_fake_server(
    ...
    status="ACTIVE",
    addresses={
        "private": [{"OS-EXT-IPS:type": "fixed", "addr": PRIVATE_V4, "version": 4}],
        "public": [{"OS-EXT-IPS:type": "floating", "addr": PUBLIC_V4, "version": 4}],
    },
    flavor={"id": "101"},
...
OSIC_SUBNETS = [
    {
        "allocation_pools": [{"end": "172.99.106.254", "start": "172.99.106.5"}],
...
class TestMeta(base.TestCase):
    ...
    def test_find_nova_addresses_key_name_and_ext_tag(self):
        ...
        self.assertEqual(
            ["198.51.100.2"],
            meta.find_nova_addresses(addrs, key_name="public", ext_tag="fixed"),
        )
        self.assertEqual(
            [],
    ...
    def test_find_nova_addresses_all(self):
        ...
        self.assertEqual(
            ["198.51.100.2"],
            meta.find_nova_addresses(addrs, key_name="public", ext_tag="fixed", version=4),
        )
        self.assertEqual(
            [],
            meta.find_nova_addresses(addrs, key_name="public", ext_tag="fixed", version=6),
        )

    def test_find_nova_addresses_floating_first(self):
        # Note 198.51.100.0/24 is TEST-NET-2 from rfc5737
        addrs = {
            "private": [{"addr": "192.0.2.5", "version": 4, "OS-EXT-IPS:type": "fixed"}],
            "public": [
                {
                    "addr": "198.51.100.1",
                    "version": 4,
        ...
        self.register_uris(
            [
                dict(
                    ...
                    json={"networks": [{"id": "test-net-id", "name": "test-net-name"}]},
                ),
                dict(
                    method="GET",
        ...
        self.register_uris(
            [
                dict(
                    method="GET",
                    uri="https://network.example.com/v2.0/networks",
                    json={"networks": [{"id": "test-net-id", "name": "test-net"}]},
                ),
                dict(
                    method="GET",
                    uri="https://network.example.com/v2.0/subnets",
    ...
    def test_get_server_private_ip_devstack(
        ...
        self.register_uris(
            [
                dict(
                    method="GET",
                    uri=("https://network.example.com/v2.0/ports?device_id=test-id"),
                    json={
                        "ports": [
                            {
                                "id": "test_port_id",
                ...
                dict(
                    method="GET",
                    uri=("https://network.example.com/v2.0/floatingips?port_id=test_port_id"),
                    json={"floatingips": []},
                ),
                dict(
                    method="GET",
    ...
    def test_get_security_groups(self, mock_list_server_security_groups):
        ...
        mock_list_server_security_groups.return_value = [{"name": "testgroup", "id": "1"}]
    ...
    def test_basic_hostvars(self, mock_get_server_external_ipv4, mock_get_server_external_ipv6):
        ...
        hostvars = meta.get_hostvars_from_server(
            FakeCloud(),
            self.cloud._normalize_server(meta.obj_to_munch(standard_fake_server)),
        )
