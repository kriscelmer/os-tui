...
class TestAgent(base.TestCase):
    ...
    def test_get_bgp_speakers_hosted_by_dragent(self, mock_list):
        ...
        response = mock.Mock()
        speaker_body = {
            "bgp_speakers": [{"name": "bgp_speaker_1", "ip_version": 4, "id": IDENTIFIER}]
        }
        response.body = speaker_body
        mock_list.return_value = [bgp_speaker.BgpSpeaker(**speaker_body["bgp_speakers"][0])]
        response.json = mock.Mock(return_value=response.body)
        response.status_code = 200
        sess.get = mock.Mock(return_value=response)
        resp = sot.get_bgp_speakers_hosted_by_dragent(sess)

        self.assertEqual(resp, [bgp_speaker.BgpSpeaker(**response.body["bgp_speakers"][0])])
