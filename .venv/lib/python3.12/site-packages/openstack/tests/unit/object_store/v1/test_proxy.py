...
class TestObjectStoreProxy(test_proxy_base.TestProxyBase):
        ...
        self.register_uris(
            [
                dict(
                    ...
                    uri=self.endpoint,
                    status_code=204,
                    validate=dict(headers={"x-account-meta-temp-url-key": key}),
                ),
                dict(
                    method="HEAD",
                    uri=self.endpoint,
        ...
        self.register_uris(
            [
                dict(
                    ...
                    uri=self.endpoint,
                    status_code=204,
                    validate=dict(headers={"x-account-meta-temp-url-key-2": key}),
                ),
                dict(
                    method="HEAD",
                    uri=self.endpoint,
        ...
        self.register_uris(
            [
                dict(
                    ...
                    status_code=204,
                    validate=dict(headers={"x-container-meta-temp-url-key": key}),
                ),
                dict(
                    method="HEAD",
                    uri=self.container_endpoint,
        ...
        self.register_uris(
            [
                dict(
                    ...
                    uri=self.container_endpoint,
                    status_code=204,
                    validate=dict(headers={"x-container-meta-temp-url-key-2": key}),
                ),
                dict(
                    method="HEAD",
                    uri=self.container_endpoint,
    ...
    def test_file_segment(self):
        file_size = 4200
        content = "".join(
            random.choice(string.ascii_uppercase + string.digits) for _ in range(file_size)
        ).encode("latin-1")
...
class TestExtractName(TestObjectStoreProxy):
    scenarios = [
        ...
        (
            "object_long",
            dict(url="/v1/AUTH_123/cnt/path/deep/object_name", parts=["object"]),
        ),
    ]
...
class TestTempURL(TestObjectStoreProxy):
    ...
    def test_generate_temp_url_ip_range(self, time_mock, hmac_mock):
        ...
        expected_url = path + (
            "?temp_url_sig=temp_url_signature&temp_url_expires=1400003600&temp_url_ip_range="
        )
    ...
    def test_generate_temp_url_prefix(self, time_mock, hmac_mock):
        ...
        for p in prefixes:
            ...
            expected_url = path + (
                "?temp_url_sig=temp_url_signature&temp_url_expires=1400003600&temp_url_prefix=" + p
            )
            expected_body = "\n".join(
                [
                    self.method,
    ...
    def test_generate_absolute_expiry_temp_url(self, hmac_mock):
        if isinstance(self.expected_url, bytes):
            ...
            expected_url = self.expected_url.replace("1400003600", "2146636800")
        url = self.proxy.generate_temp_url(
            self.url,
            2146636800,
            self.method,
...
class TestTempURLUnicodePathAndKey(TestTempURL):
    url = "/v1/\u00e4/c/\u00f3"
    key = "k\u00e9y"
    expected_url = f"{url}?temp_url_sig=temp_url_signature&temp_url_expires=1400003600"
    expected_body = "\n".join(
        [
            "GET",
            "1400003600",
