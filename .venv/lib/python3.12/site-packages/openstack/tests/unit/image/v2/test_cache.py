...
class TestCache(base.TestCase):
    ...
    def test_queue(self):
        ...
        sess.put.assert_called_with("cache/image_id", microversion=sess.default_microversion)
    ...
    def test_clear(self):
        ...
        session.delete.assert_called_with("/cache", headers={"x-image-cache-clear-target": "cache"})

        sot.clear(session, "queue")
        session.delete.assert_called_with("/cache", headers={"x-image-cache-clear-target": "queue"})

        self.assertRaises(exceptions.InvalidRequest, sot.clear, session, "invalid")
