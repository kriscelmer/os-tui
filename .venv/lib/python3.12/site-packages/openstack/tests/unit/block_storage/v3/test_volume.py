...
class TestVolume(base.TestCase):
    ...
    def test_create(self):
        ...
        self.assertEqual(VOLUME["os-vol-tenant-attr:tenant_id"], sot.project_id)
        self.assertEqual(VOLUME["os-vol-mig-status-attr:migstat"], sot.migration_status)
        self.assertEqual(VOLUME["os-vol-mig-status-attr:name_id"], sot.migration_id)
        ...
        self.assertEqual(VOLUME["consistencygroup_id"], sot.consistency_group_id)
        self.assertEqual(
            VOLUME["os-volume-replication:driver_data"],
            sot.replication_driver_data,
        )
        self.assertFalse(sot.is_encrypted)
        self.assertDictEqual(VOLUME["OS-SCH-HNT:scheduler_hints"], sot.scheduler_hints)
    ...
    def test_delete_image_metadata(self):
        ...
        self.sess.post.assert_has_calls(
            [
                mock.call(url, json=body_a, microversion=sot._max_microversion),
                mock.call(url, json=body_b, microversion=sot._max_microversion),
            ]
        )
    ...
    def test_detach_force(self):
        ...
        self.assertIsNone(sot.detach(self.sess, "1", force=True, connector={"a": "b"}))

        url = f"volumes/{FAKE_ID}/action"
        body = {"os-force_detach": {"attachment_id": "1", "connector": {"a": "b"}}}
        self.sess.post.assert_called_with(url, json=body, microversion=sot._max_microversion)
    ...
    def test_migrate_cluster(self, mv_mock):
        ...
        self.assertIsNone(
            sot.migrate(self.sess, cluster="1", force_host_copy=True, lock_volume=True)
        )
    ...
    def test_complete_migration(self):
        ...
        url = f"volumes/{FAKE_ID}/action"
        body = {"os-migrate_volume_completion": {"new_volume": "1", "error": False}}
        self.sess.post.assert_called_with(url, json=body, microversion=sot._max_microversion)

    def test_complete_migration_error(self):
        sot = volume.Volume(**VOLUME)

        self.assertIsNone(sot.complete_migration(self.sess, new_volume_id="1", error=True))

        url = f"volumes/{FAKE_ID}/action"
        body = {"os-migrate_volume_completion": {"new_volume": "1", "error": True}}
        self.sess.post.assert_called_with(url, json=body, microversion=sot._max_microversion)
    ...
    def test_init_attachment(self):
        ...
        self.resp.json = mock.Mock(return_value=self.resp.body)
        self.sess.post = mock.Mock(return_value=self.resp)
        self.assertEqual({"c": "d"}, sot.init_attachment(self.sess, {"a": "b"}))
