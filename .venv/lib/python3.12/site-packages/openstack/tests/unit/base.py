...
_UserData = collections.namedtuple(
    "UserData",
    "user_id, password, name, email, description, domain_id, enabled, json_response, json_request",
)


_GroupData = collections.namedtuple(
    "GroupData",
    "group_id, group_name, domain_id, description, json_response, json_request",
)
...
_EndpointDataV3 = collections.namedtuple(
    "EndpointData",
    "endpoint_id, service_id, interface, region_id, url, enabled, json_response, json_request",
)
...
_RoleData = collections.namedtuple("RoleData", "role_id, role_name, json_response, json_request")
...
class TestCase(base.TestCase):
    ...
    def setUp(self, cloud_config_fixture="clouds.yaml"):
        ...
        self.os_fixture = self.useFixture(os_fixture.ConnectionFixture(project_id=fakes.PROJECT_ID))
        ...
        self.discovery_json = os.path.join(self.fixtures_directory, "discovery.json")
    ...
    def get_mock_url(
        ...
        endpoint_url = self.cloud.endpoint_for(service_type=service_type, interface=interface)
        ...
        if list_get:
            uri_mock_list.append(
                dict(
                    ...
                    status_code=200,
                    json={"projects": [p.json_response["project"] for p in project_list]},
                )
            )
        if id_get:
    ...
    def get_placement_discovery_mock_dict(self, discovery_fixture="placement.json"):
        discovery_fixture = os.path.join(self.fixtures_directory, discovery_fixture)
        return dict(
            method="GET",
    ...
    def get_ironic_discovery_mock_dict(self):
        discovery_fixture = os.path.join(self.fixtures_directory, "baremetal.json")
        return dict(
            method="GET",
    ...
    def get_senlin_discovery_mock_dict(self):
        discovery_fixture = os.path.join(self.fixtures_directory, "clustering.json")
        return dict(
            method="GET",
    ...
    def use_compute_discovery(
        ...
        self.__do_register_uris(
            [
                self.get_nova_discovery_mock_dict(compute_version_json, compute_discovery_url),
            ]
        )

    def get_cyborg_discovery_mock_dict(self):
        discovery_fixture = os.path.join(self.fixtures_directory, "accelerator.json")
        return dict(
            method="GET",
    ...
    def use_glance(
        ...
        self.__do_register_uris(
            [self.get_glance_discovery_mock_dict(image_version_json, image_discovery_url)]
        )
    ...
    def use_placement(self, **kwargs):
        self.__do_register_uris([self.get_placement_discovery_mock_dict(**kwargs)])
    ...
    def __do_register_uris(self, uri_mock_list=None):
        for to_mock in uri_mock_list:
            ...
            if invalid_keys:
                raise TypeError(f"Invalid values passed to validate: {invalid_keys}")
        ...
        if do_count:
            self.assertEqual(
                ...
                "Expected:\n{}'\nGot:\n{}".format(
                    ...
                    "\n".join([f"{h.method} {h.url}" for h in self.adapter.request_history]),
                ),
            )
