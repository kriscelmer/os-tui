...
class TestServer(base.TestCase):
    ...
    def test_make_it(self):
        ...
        self.assertEqual(EXAMPLE["OS-EXT-AZ:availability_zone"], sot.availability_zone)
        ...
        self.assertEqual(EXAMPLE["OS-SRV-USG:terminated_at"], sot.terminated_at)
        self.assertEqual(EXAMPLE["security_groups"], sot.security_groups)
        self.assertEqual(EXAMPLE["adminPass"], sot.admin_password)
        self.assertEqual(EXAMPLE["block_device_mapping_v2"], sot.block_device_mapping)
        ...
        self.assertEqual(EXAMPLE["OS-EXT-SRV-ATTR:instance_name"], sot.instance_name)
        self.assertEqual(EXAMPLE["OS-EXT-SRV-ATTR:kernel_id"], sot.kernel_id)
        self.assertEqual(EXAMPLE["OS-EXT-SRV-ATTR:launch_index"], sot.launch_index)
        self.assertEqual(EXAMPLE["OS-EXT-SRV-ATTR:ramdisk_id"], sot.ramdisk_id)
        self.assertEqual(EXAMPLE["OS-EXT-SRV-ATTR:reservation_id"], sot.reservation_id)
        self.assertEqual(EXAMPLE["OS-EXT-SRV-ATTR:root_device_name"], sot.root_device_name)
        self.assertEqual(EXAMPLE["OS-SCH-HNT:scheduler_hints"], sot.scheduler_hints)
    ...
    def test__prepare_server(self):
        ...
        self.assertNotIn("OS-EXT-AZ:availability_zone", request.body[sot.resource_key])
        self.assertEqual(request.body[sot.resource_key]["availability_zone"], zone)

        self.assertNotIn("OS-EXT-SRV-ATTR:user_data", request.body[sot.resource_key])
        self.assertEqual(request.body[sot.resource_key]["user_data"], data)

        self.assertNotIn("OS-SCH-HNT:scheduler_hints", request.body[sot.resource_key])
        self.assertEqual(request.body["OS-SCH-HNT:scheduler_hints"], hints)

        self.assertNotIn("OS-EXT-SRV-ATTR:hostname", request.body[sot.resource_key])
    ...
    def test_get_password(self):
        sot = server.Server(**EXAMPLE)
        self.sess.get.return_value = fakes.FakeResponse(data={"password": "foo"})

        result = sot.get_password(self.sess)
        self.assertEqual("foo", result)

        url = "servers/IDENTIFIER/os-server-password"
        self.sess.get.assert_called_with(url, microversion=self.sess.default_microversion)
    ...
    def test_clear_password(self):
        ...
        url = "servers/IDENTIFIER/os-server-password"
        self.sess.delete.assert_called_with(url, microversion=self.sess.default_microversion)
    ...
    def test_live_migrate_25(self):
        ...
        res = sot.live_migrate(self.sess, host="HOST2", force=True, block_migration=False)
        ...
        headers = {"Accept": ""}
        self.sess.post.assert_called_with(url, json=body, headers=headers, microversion="2.25")
    ...
    def test_live_migrate_30(self):
        ...
        res = sot.live_migrate(self.sess, host="HOST2", force=False, block_migration=False)
