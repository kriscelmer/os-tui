...
class TestProjectCleanup(base.BaseFunctionalTest):
    ...
    def _create_network_resources(self):
        ...
        self.router = self.user_cloud_alt.network.create_router(name=self.getUniqueString("router"))
    ...
    def test_block_storage_cleanup(self):
        ...
        vol = self.user_cloud_alt.block_storage.create_volume(name="vol1", size="1")
        self.user_cloud_alt.block_storage.wait_for_status(vol)
        s1 = self.user_cloud_alt.block_storage.create_snapshot(volume_id=vol.id)
        ...
        self.assertEqual(0, len(list(self.user_cloud_alt.block_storage.backups())))
        # Ensure no snapshots remain
        self.assertEqual(0, len(list(self.user_cloud_alt.block_storage.snapshots())))
    ...
    def test_cleanup_vpnaas(self):
        if not list(self.user_cloud_alt.network.service_providers(service_type="VPN")):
        ...
        network_left = self.user_cloud_alt.network.create_network(name="network_left")
        ...
        router_left = self.user_cloud_alt.network.create_router(name="router_left")
        self.user_cloud_alt.network.add_interface_to_router(
        ...
        network_right = self.user_cloud_alt.network.create_network(name="network_right")
        ...
        router_right = self.user_cloud_alt.network.create_router(name="router_right")
        self.user_cloud_alt.network.add_interface_to_router(
        ...
        ike_policy = self.user_cloud_alt.network.create_vpn_ike_policy(name="ike_policy")
        ipsec_policy = self.user_cloud_alt.network.create_vpn_ipsec_policy(name="ipsec_policy")
        ...
        router_right_ip = router_right.external_gateway_info["external_fixed_ips"][0]["ip_address"]
        ipsec_site_conn = self.user_cloud_alt.network.create_vpn_ipsec_site_connection(
        ...
        site_conn_check = self.user_cloud_alt.network.get_vpn_ipsec_site_connection(
            ipsec_site_conn.id
        )
        ...
        self.assertEqual(0, len(list(self.user_cloud_alt.network.vpn_ike_policies())))
        self.assertEqual(0, len(list(self.user_cloud_alt.network.vpn_ipsec_policies())))
        self.assertEqual(0, len(list(self.user_cloud_alt.network.vpn_services())))
        self.assertEqual(0, len(list(self.user_cloud_alt.network.vpn_endpoint_groups())))
        self.assertEqual(
            0,
            len(list(self.user_cloud_alt.network.vpn_ipsec_site_connections())),
        )
