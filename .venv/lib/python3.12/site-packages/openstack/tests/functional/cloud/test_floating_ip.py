...
class TestFloatingIP(base.BaseFunctionalTest):
    ...
    def _cleanup_ips(self, server):
        ...
        for ip in self.user_cloud.list_floating_ips():
            if ip.get("fixed_ip", None) == fixed_ip or ip.get("fixed_ip_address", None) == fixed_ip:
                try:
    ...
    def _setup_networks(self):
        if self.user_cloud.has_service("network"):
            ...
            self.addDetail(
                "networks-neutron",
                content.text_content(pprint.pformat(self.user_cloud.list_networks())),
            )
        else:
            # Find network names for nova-net
            data = proxy._json_response(self.user_cloud.compute.get("/os-tenant-networks"))
    ...
    def test_private_ip(self):
        ...
        self.addDetail("server", content.text_content(pprint.pformat(new_server)))
    ...
    def test_detach_ip_from_server(self):
        ...
        f_ip = self.user_cloud.get_floating_ip(id=None, filters={"floating_ip_address": ip})
        self.user_cloud.detach_ip_from_server(server_id=new_server.id, floating_ip_id=f_ip["id"])
    ...
    def test_list_floating_ips(self):
        ...
        if self.operator_cloud:
            fip_op_id_list = [fip.id for fip in self.operator_cloud.list_floating_ips()]
        fip_user_id_list = [fip.id for fip in self.user_cloud.list_floating_ips()]
