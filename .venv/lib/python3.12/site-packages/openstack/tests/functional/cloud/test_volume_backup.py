...
class TestVolume(base.BaseFunctionalTest):
    ...
    def test_list_volume_backups(self):
        vol1 = self.user_cloud.create_volume(display_name=self.getUniqueString(), size=1)
        ...
        # here.
        vol2 = self.user_cloud.create_volume(display_name=self.getUniqueString(), size=1)
        self.addCleanup(self.user_cloud.delete_volume, vol2["id"])

        backup_name_1 = self.getUniqueString()
        backup = self.user_cloud.create_volume_backup(volume_id=vol1["id"], name=backup_name_1)
        ...
        backups = self.user_cloud.list_volume_backups(filters={"name": backup_name_1})
