...
class TestCompute(base.BaseFunctionalTest):
    ...
    def test_create_and_delete_server_auto_ip_delete_ips(self):
        ...
        self.assertEqual(self.flavor.name, server["flavor"]["original_name"])
        self.assertIsNotNone(server["adminPass"])
        self.assertTrue(self.user_cloud.delete_server(self.server_name, wait=True, delete_ips=True))
        srv = self.user_cloud.get_server(self.server_name)
        self.assertTrue(srv is None or srv.status.lower() == "deleted")
    ...
    def test_list_availability_zone_names(self):
        self.assertEqual(["nova"], self.user_cloud.list_availability_zone_names())
    ...
    def test_get_image_id(self):
        self.assertEqual(self.image.id, self.user_cloud.get_image_id(self.image.id))
        self.assertEqual(self.image.id, self.user_cloud.get_image_id(self.image.name))

    def test_get_image_name(self):
        self.assertEqual(self.image.name, self.user_cloud.get_image_name(self.image.id))
        self.assertEqual(self.image.name, self.user_cloud.get_image_name(self.image.name))
    ...
    def test_create_image_snapshot_wait_active(self):
        ...
        image = self.user_cloud.create_image_snapshot("test-snapshot", server, wait=True)
