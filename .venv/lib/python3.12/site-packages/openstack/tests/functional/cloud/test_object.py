...
class TestObject(base.BaseFunctionalTest):
    ...
    def test_create_object(self):
        ...
        self.assertEqual([], self.user_cloud.list_containers(prefix="somethin"))
        sizes = (
        ...
        for size, nseg in sizes:
            ...
            with tempfile.NamedTemporaryFile() as fake_file:
                ...
                fake_file.flush()
                name = f"test-{size}"
                self.addCleanup(self.user_cloud.delete_object, container_name, name)
                ...
                self.assertFalse(
                    self.user_cloud.is_object_stale(container_name, name, fake_file.name)
                )
            self.assertEqual(
                "bar",
                self.user_cloud.get_object_metadata(container_name, name)["foo"],
            )
            ...
            self.assertEqual(
                "testv",
                self.user_cloud.get_object_metadata(container_name, name)["testk"],
            )
            self.assertIsNotNone(self.user_cloud.get_object(container_name, name))
            self.assertEqual(name, self.user_cloud.list_objects(container_name)[0]["name"])
            self.assertEqual([], self.user_cloud.list_objects(container_name, prefix="abc"))
            self.assertTrue(self.user_cloud.delete_object(container_name, name))
        self.assertEqual([], self.user_cloud.list_objects(container_name))
        self.assertEqual(container_name, self.user_cloud.get_container(container_name).name)
    ...
    def test_download_object_to_file(self):
        ...
        self.addCleanup(self.user_cloud.delete_container, container_name)
        self.user_cloud.create_container(container_name)
        self.assertEqual(container_name, self.user_cloud.list_containers()[0]["name"])
        sizes = (
            (64 * 1024, 1),  # 64K, one segment
            (64 * 1024, 5),  # 64MB, 5 segments
        )
        for size, nseg in sizes:
            ...
            with tempfile.NamedTemporaryFile() as fake_file:
                self.user_cloud.get_object(container_name, name, outfile=fake_file.name)
