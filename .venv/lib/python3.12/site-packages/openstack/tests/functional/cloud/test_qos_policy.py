...
class TestQosPolicy(base.BaseFunctionalTest):
    ...
    def test_create_qos_policy_shared(self):
        policy = self.operator_cloud.create_qos_policy(name=self.policy_name, shared=True)
        self.assertIn("id", policy)
        self.assertEqual(self.policy_name, policy["name"])
    ...
    def test_list_qos_policies_filtered(self):
        ...
        policy2 = self.operator_cloud.create_qos_policy(name=self.policy_name + "other")
        self.assertIsNotNone(policy2)
        match = self.operator_cloud.list_qos_policies(filters=dict(name=self.policy_name))
