...
class TestType(base.BaseBlockStorageTest):
    def setUp(self):
        ...
        sot = self.operator_cloud.block_storage.create_type(name=self.TYPE_NAME)
        assert isinstance(sot, _type.Type)
        self.assertEqual(self.TYPE_NAME, sot.name)
        self.TYPE_ID = sot.id

    def tearDown(self):
        sot = self.operator_cloud.block_storage.delete_type(self.TYPE_ID, ignore_missing=False)
