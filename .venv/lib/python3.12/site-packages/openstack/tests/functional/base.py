...
class BaseFunctionalTest(base.TestCase):
    ...
    def setUp(self):
        ...
        if not self._system_admin_name:
            raise self.failureException(
                "OPENSTACKSDK_SYSTEM_ADMIN_CLOUD must be set to a non-empty value"
            )

        self.config = openstack.config.OpenStackConfig()

        self._user_cloud_name = os.environ.get("OPENSTACKSDK_DEMO_CLOUD", "devstack")
        if not self._user_cloud_name:
            raise self.failureException("OPENSTACKSDK_DEMO_CLOUD must be set to a non-empty value")

        self._user_alt_cloud_name = os.environ.get("OPENSTACKSDK_DEMO_CLOUD_ALT", "devstack-alt")
        ...
        self._operator_cloud_name = os.environ.get("OPENSTACKSDK_OPERATOR_CLOUD", "devstack-admin")
    ...
    def _set_user_cloud(self, **kwargs):
        user_config = self.config.get_one(cloud=self._user_cloud_name, **kwargs)
        self.user_cloud = connection.Connection(config=user_config)
        _disable_keep_alive(self.user_cloud)

        user_config_alt = self.config.get_one(cloud=self._user_alt_cloud_name, **kwargs)
    ...
    def _set_operator_cloud(self, **kwargs):
        operator_config = self.config.get_one(cloud=self._operator_cloud_name, **kwargs)
        self.operator_cloud = connection.Connection(config=operator_config)
        _disable_keep_alive(self.operator_cloud)

        system_admin_config = self.config.get_one(cloud=self._system_admin_name, **kwargs)
        self.system_admin_cloud = connection.Connection(config=system_admin_config)
    ...
    def require_service(self, service_type, min_microversion=None, **kwargs):
        ...
        data = self.operator_cloud.session.get_endpoint_data(service_type=service_type, **kwargs)
        ...
        if not data or not (
            ...
            self.skipTest(
                f"Service {service_type} does not provide microversion {min_microversion}"
            )
    ...
    def getUniqueString(self, prefix=None):
        ...
        return (prefix if prefix else "") + f"{int(time.time())}-{uuid.uuid4().hex}"
    ...
    def create_temporary_project(self):
        ...
        if not role_assignment:
            self.operator_cloud.grant_role("member", user=user_id, project=project["id"], wait=True)
