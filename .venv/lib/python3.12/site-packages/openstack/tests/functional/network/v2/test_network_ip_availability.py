...
class TestNetworkIPAvailability(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        if not self.operator_cloud._has_neutron_extension("network-ip-availability"):
            self.skipTest("Neutron network-ip-availability extension is required for this test")
        ...
        self.SUB_ID = sub.id
        prt = self.operator_cloud.network.create_port(name=self.PORT_NAME, network_id=self.NET_ID)
    ...
    def test_find(self):
        sot = self.operator_cloud.network.find_network_ip_availability(self.NET_ID)
        self.assertEqual(self.NET_ID, sot.network_id)

    def test_get(self):
        sot = self.operator_cloud.network.get_network_ip_availability(self.NET_ID)
        self.assertEqual(self.NET_ID, sot.network_id)
        self.assertEqual(self.NET_NAME, sot.network_name)

    def test_list(self):
        ids = [o.network_id for o in self.operator_cloud.network.network_ip_availabilities()]
