...
class TestNetworkSegmentRange(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        if not self.operator_cloud.network.find_extension("network-segment-range"):
            self.skipTest("Network Segment Range extension disabled")

        test_seg_range = self.operator_cloud.network.create_network_segment_range(
            name=self.NAME,
        ...
        self.assertIsInstance(test_seg_range, network_segment_range.NetworkSegmentRange)
        ...
        self.assertEqual(self.PHYSICAL_NETWORK, test_seg_range.physical_network)
    ...
    def test_create_delete(self):
        del_test_seg_range = self.operator_cloud.network.delete_network_segment_range(
            self.NETWORK_SEGMENT_RANGE_ID
        )
        self.assertIsNone(del_test_seg_range)

    def test_find(self):
        test_seg_range = self.operator_cloud.network.find_network_segment_range(
            self.NETWORK_SEGMENT_RANGE_ID
        )
    ...
    def test_list(self):
        ids = [o.id for o in self.operator_cloud.network.network_segment_ranges(name=None)]
