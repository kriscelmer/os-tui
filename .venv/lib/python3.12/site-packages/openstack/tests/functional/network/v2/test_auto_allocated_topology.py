...
class TestAutoAllocatedTopology(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        if not self.operator_cloud._has_neutron_extension("auto-allocated-topology"):
            self.skipTest("Neutron auto-allocated-topology extension is required for this test")
    ...
    def tearDown(self):
        res = self.test_cloud.network.delete_auto_allocated_topology(self.PROJECT_ID)
    ...
    def _create_project(self):
        ...
        if not project:
            params = {
                "name": project_name,
                "description": (
                    "test project used only for the TestAutoAllocatedTopology tests class"
                ),
    ...
    def test_auto_allocated_topology(self):
        ...
        top = self.test_cloud.network.validate_auto_allocated_topology(self.PROJECT_ID)
        ...
        top = self.test_cloud.network.get_auto_allocated_topology(self.PROJECT_ID)
        network = self.test_cloud.network.get_network(top.id)
    ...
    def _set_network_external(self):
        ...
        for network in networks:
            if network.name == "public":
                self.test_cloud.network.update_network(network, is_default=True)
