...
class TestRBACPolicy(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        if not self.user_cloud._has_neutron_extension("rbac-policies"):
            self.skipTest("Neutron rbac-policies extension is required for this test")
    ...
    def tearDown(self):
        if self.operator_cloud:
            sot = self.operator_cloud.network.delete_rbac_policy(self.ID, ignore_missing=False)
        else:
            sot = self.user_cloud.network.delete_rbac_policy(self.ID, ignore_missing=False)
