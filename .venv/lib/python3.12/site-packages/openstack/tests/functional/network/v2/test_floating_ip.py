...
class TestFloatingIP(common.TestTagNeutron):
    ...
    def setUp(self):
        ...
        if self.user_cloud._has_neutron_extension(
            ...
            fip_args.update(dict(dns_domain=self.DNS_DOMAIN, dns_name=self.DNS_NAME))
    ...
    def tearDown(self):
        sot = self.user_cloud.network.delete_ip(self.FIP.id, ignore_missing=False)
        self.assertIsNone(sot)
        sot = self.user_cloud.network.delete_port(self.PORT_ID, ignore_missing=False)
        self.assertIsNone(sot)
    ...
    def test_update(self):
        sot = self.user_cloud.network.update_ip(self.FIP.id, port_id=self.PORT_ID)
