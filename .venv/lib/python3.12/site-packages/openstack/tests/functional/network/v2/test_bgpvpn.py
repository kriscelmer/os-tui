...
class TestBGPVPN(base.BaseFunctionalTest):
    def setUp(self):
        ...
        router = self.operator_cloud.network.create_router(name=self.ROUTER_NAME)
        assert isinstance(router, _router.Router)
        self.ROUTER = router

        net_assoc = self.operator_cloud.network.create_bgpvpn_network_association(
            self.BGPVPN, network_id=self.NETWORK.id
        )
        assert isinstance(net_assoc, _bgpvpn_net_assoc.BgpVpnNetworkAssociation)
        self.NET_ASSOC = net_assoc

        port_assoc = self.operator_cloud.network.create_bgpvpn_port_association(
        ...
        router_assoc = self.operator_cloud.network.create_bgpvpn_router_association(
            self.BGPVPN, router_id=self.ROUTER.id
        )
        assert isinstance(router_assoc, _bgpvpn_router_assoc.BgpVpnRouterAssociation)
    ...
    def test_list_bgpvpn_network_associations(self):
        net_assoc_ids = [
            net_assoc.id
            for net_assoc in (
                self.operator_cloud.network.bgpvpn_network_associations(self.BGPVPN.id)
            )
        ]
    ...
    def test_list_bgpvpn_port_associations(self):
        port_assoc_ids = [
            port_assoc.id
            for port_assoc in (self.operator_cloud.network.bgpvpn_port_associations(self.BGPVPN.id))
        ]
    ...
    def test_list_bgpvpn_router_associations(self):
        router_assoc_ids = [
            router_assoc.id
            for router_assoc in (
                self.operator_cloud.network.bgpvpn_router_associations(self.BGPVPN.id)
            )
        ]
