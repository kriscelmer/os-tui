...
class TestVpnIkePolicy(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        policy = self.user_cloud.network.create_vpn_ike_policy(name=self.IKEPOLICY_NAME)
        assert isinstance(policy, vpn_ike_policy.VpnIkePolicy)
        self.assertEqual(self.IKEPOLICY_NAME, policy.name)
        self.ID = policy.id

    def tearDown(self):
        ikepolicy = self.user_cloud.network.delete_vpn_ike_policy(self.ID, ignore_missing=True)
    ...
    def test_find(self):
        policy = self.user_cloud.network.find_vpn_ike_policy(self.IKEPOLICY_NAME)
    ...
    def test_update(self):
        policy = self.user_cloud.network.update_vpn_ike_policy(self.ID, name=self.UPDATE_NAME)
