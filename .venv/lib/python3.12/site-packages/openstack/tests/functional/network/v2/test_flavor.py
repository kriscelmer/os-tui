...
class TestFlavor(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        if self.operator_cloud:
            ...
            self.service_profiles = self.operator_cloud.network.create_service_profile(
    ...
    def tearDown(self):
        if self.operator_cloud and self.ID:
            flavors = self.operator_cloud.network.delete_flavor(self.ID, ignore_missing=True)
    ...
    def test_update(self):
        if not self.operator_cloud:
            self.skipTest("Operator cloud required for this test")
        flavor = self.operator_cloud.network.update_flavor(self.ID, name=self.UPDATE_NAME)
        self.assertEqual(self.UPDATE_NAME, flavor.name)

    def test_associate_disassociate_flavor_with_service_profile(self):
        if not self.operator_cloud:
            self.skipTest("Operator cloud required for this test")
        response = self.operator_cloud.network.associate_flavor_with_service_profile(
