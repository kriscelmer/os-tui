...
class TestQoSPacketRateLimitRule(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        qos_rule = self.operator_cloud.network.create_qos_packet_rate_limit_rule(
            self.QOS_POLICY_ID,
            max_kpps=self.RULE_MAX_KPPS,
            max_burst_kpps=self.RULE_MAX_BURST_KPPS,
            direction=self.RULE_DIRECTION,
        )
        assert isinstance(qos_rule, _qos_packet_rate_limit_rule.QoSPacketRateLimitRule)
    ...
    def test_list(self):
        rule_ids = [
            o.id
            for o in self.operator_cloud.network.qos_packet_rate_limit_rules(self.QOS_POLICY_ID)
        ]
