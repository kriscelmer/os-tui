...
class TestServiceProfile(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        if self.operator_cloud:
            service_profiles = self.operator_cloud.network.create_service_profile(
                description=self.SERVICE_PROFILE_DESCRIPTION,
                metainfo=self.METAINFO,
            )
            assert isinstance(service_profiles, _service_profile.ServiceProfile)
            self.assertEqual(self.SERVICE_PROFILE_DESCRIPTION, service_profiles.description)
    ...
    def tearDown(self):
        if self.ID:
            service_profiles = self.operator_cloud.network.delete_service_profile(
    ...
    def test_find(self):
        self.user_cloud.network.find_service_profile(name_or_id="not_existing", ignore_missing=True)
        if self.operator_cloud and self.ID:
            service_profiles = self.operator_cloud.network.find_service_profile(self.ID)
            self.assertEqual(self.METAINFO, service_profiles.meta_info)

    def test_get(self):
        if not self.ID:
            self.skipTest("ServiceProfile was not created")
        service_profiles = self.operator_cloud.network.get_service_profile(self.ID)
        self.assertEqual(self.METAINFO, service_profiles.meta_info)
        self.assertEqual(self.SERVICE_PROFILE_DESCRIPTION, service_profiles.description)
    ...
    def test_list(self):
        ...
        # Test as operator
        if self.operator_cloud:
            metainfos = [f.meta_info for f in self.operator_cloud.network.service_profiles()]
            if self.ID:
