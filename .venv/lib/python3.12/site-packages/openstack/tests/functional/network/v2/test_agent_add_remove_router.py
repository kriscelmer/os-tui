...
class TestAgentRouters(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        self.ROUTER = self.user_cloud.network.create_router(name=self.ROUTER_NAME)
        ...
        agents = [agent for agent in agent_list if agent.agent_type == "L3 agent"]
        if len(agent_list) == 0:
    ...
    def test_remove_router_from_agent(self):
        self.user_cloud.network.remove_router_from_agent(self.AGENT, self.ROUTER)
        rots = self.user_cloud.network.agent_hosted_routers(self.AGENT)
        routers = [router.id for router in rots]
