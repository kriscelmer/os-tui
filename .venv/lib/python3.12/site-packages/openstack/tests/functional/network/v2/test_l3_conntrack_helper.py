...
class TestL3ConntrackHelper(base.BaseFunctionalTest):
    ...
    def test_get(self):
        sot = self.user_cloud.network.get_conntrack_helper(self.CT_HELPER, self.ROT_ID)
        self.assertEqual(self.PROTOCOL, sot.protocol)
        self.assertEqual(self.HELPER, sot.helper)
        self.assertEqual(self.PORT, sot.port)

    def test_list(self):
        helper_ids = [o.id for o in self.user_cloud.network.conntrack_helpers(self.ROT_ID)]
