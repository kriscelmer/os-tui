...
class TestTapMirror(base.BaseFunctionalTest):
    def setUp(self):
        ...
        port = self.user_cloud.network.create_port(network_id=self.MIRROR_NET_ID)
    ...
    def tearDown(self):
        sot = self.user_cloud.network.delete_tap_mirror(self.TAP_MIRROR.id, ignore_missing=False)
    ...
    def test_list_tap_mirrors(self):
        tap_mirror_ids = [tm.id for tm in self.user_cloud.network.tap_mirrors()]
        self.assertIn(self.TAP_MIRROR.id, tap_mirror_ids)

    def test_update_tap_mirror(self):
        description = "My Tap Mirror"
        sot = self.user_cloud.network.update_tap_mirror(self.TAP_MIRROR.id, description=description)
