...
class TestPortForwarding(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        if not self.user_cloud._has_neutron_extension("external-net"):
            self.skipTest("Neutron external-net extension is required for this test")
        if not self.user_cloud.network.find_extension("floating-ip-port-forwarding"):
        ...
        if not self.EXT_NET_ID and self.operator_cloud:
            ...
            net = self._create_network(self.EXT_NET_NAME, **{"router:external": True})
            self.EXT_NET_ID = net.id
            sub = self._create_subnet(self.EXT_SUB_NAME, self.EXT_NET_ID, self.EXT_CIDR)
            self.EXT_SUB_ID = sub.id

        # Create Internal Network
        net = self._create_network(self.INT_NET_NAME)
        self.INT_NET_ID = net.id
        sub = self._create_subnet(self.INT_SUB_NAME, self.INT_NET_ID, self.INT_CIDR)
        ...
        sot = self.ROT.add_interface(self.user_cloud.network, subnet_id=self.INT_SUB_ID)
        ...
        # Create Floating IP.
        fip = self.user_cloud.network.create_ip(floating_network_id=self.EXT_NET_ID)
    ...
    def tearDown(self):
        ...
        sot = self.user_cloud.network.delete_ip(self.FIP_ID, ignore_missing=False)
        self.assertIsNone(sot)
        sot = self.user_cloud.network.delete_port(self.INTERNAL_PORT_ID, ignore_missing=False)
        self.assertIsNone(sot)
        sot = self.ROT.remove_interface(self.user_cloud.network, subnet_id=self.INT_SUB_ID)
        self.assertEqual(sot["subnet_id"], self.INT_SUB_ID)
        sot = self.user_cloud.network.delete_router(self.ROT_ID, ignore_missing=False)
        self.assertIsNone(sot)
        sot = self.user_cloud.network.delete_subnet(self.INT_SUB_ID, ignore_missing=False)
        self.assertIsNone(sot)
        sot = self.user_cloud.network.delete_network(self.INT_NET_ID, ignore_missing=False)
        self.assertIsNone(sot)
        super().tearDown()
    ...
    def test_find(self):
        sot = self.user_cloud.network.find_port_forwarding(self.PF.id, self.FIP_ID)
    ...
    def test_list(self):
        pf_ids = [o.id for o in self.user_cloud.network.port_forwardings(self.FIP_ID)]
