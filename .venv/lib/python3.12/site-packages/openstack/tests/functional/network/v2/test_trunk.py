...
class TestTrunk(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        trunk = self.user_cloud.network.create_trunk(name=self.TRUNK_NAME, port_id=self.PORT_ID)
        assert isinstance(trunk, _trunk.Trunk)
        self.TRUNK_ID = trunk.id

    def tearDown(self):
        self.user_cloud.network.delete_trunk(self.TRUNK_ID, ignore_missing=False)
        for port_id in self.ports_to_clean:
            self.user_cloud.network.delete_port(port_id, ignore_missing=False)
        self.user_cloud.network.delete_network(self.NET_ID, ignore_missing=False)
        super().tearDown()
    ...
    def test_update(self):
        sot = self.user_cloud.network.update_trunk(self.TRUNK_ID, name=self.TRUNK_NAME_UPDATED)
