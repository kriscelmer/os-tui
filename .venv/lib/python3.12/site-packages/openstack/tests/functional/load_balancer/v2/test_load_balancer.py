...
class TestLoadBalancer(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        test_flavor_profile = self.operator_cloud.load_balancer.create_flavor_profile(
        ...
        test_flavor = self.operator_cloud.load_balancer.create_flavor(
        ...
        test_az_profile = self.operator_cloud.load_balancer.create_availability_zone_profile(
        ...
        self.assertEqual(self.AVAILABILITY_ZONE_PROFILE_NAME, test_az_profile.name)
        self.AVAILABILITY_ZONE_PROFILE_ID = test_az_profile.id

        test_az = self.operator_cloud.load_balancer.create_availability_zone(
        ...
        test_lb = self.operator_cloud.load_balancer.create_load_balancer(
            name=self.LB_NAME,
        ...
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            test_lb.id, interval=1, wait=self._wait_for_timeout
        )
        self.LB_ID = test_lb.id

        amphorae = self.operator_cloud.load_balancer.amphorae(loadbalancer_id=self.LB_ID)
        for amp in amphorae:
            self.AMPHORA_ID = amp.id

        test_listener = self.operator_cloud.load_balancer.create_listener(
        ...
        test_pool = self.operator_cloud.load_balancer.create_pool(
            name=self.POOL_NAME,
            protocol=self.PROTOCOL,
        ...
        test_member = self.operator_cloud.load_balancer.create_member(
            pool=self.POOL_ID,
        ...
        test_hm = self.operator_cloud.load_balancer.create_health_monitor(
        ...
        test_l7policy = self.operator_cloud.load_balancer.create_l7_policy(
            listener_id=self.LISTENER_ID,
        ...
        test_l7rule = self.operator_cloud.load_balancer.create_l7_rule(
    ...
    def tearDown(self):
        self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )

        self.operator_cloud.load_balancer.delete_quota(self.PROJECT_ID, ignore_missing=False)

        self.operator_cloud.load_balancer.delete_l7_rule(
        ...
        self.operator_cloud.load_balancer.delete_l7_policy(self.L7POLICY_ID, ignore_missing=False)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )

        self.operator_cloud.load_balancer.delete_health_monitor(self.HM_ID, ignore_missing=False)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )

        self.operator_cloud.load_balancer.delete_member(
        ...
        self.operator_cloud.load_balancer.delete_pool(self.POOL_ID, ignore_missing=False)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )

        self.operator_cloud.load_balancer.delete_listener(self.LISTENER_ID, ignore_missing=False)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )

        self.operator_cloud.load_balancer.delete_load_balancer(self.LB_ID, ignore_missing=False)
        super().tearDown()

        self.operator_cloud.load_balancer.delete_flavor(self.FLAVOR_ID, ignore_missing=False)

        self.operator_cloud.load_balancer.delete_flavor_profile(
            self.FLAVOR_PROFILE_ID, ignore_missing=False
        )

        self.operator_cloud.load_balancer.delete_availability_zone(
            self.AVAILABILITY_ZONE_NAME, ignore_missing=False
        )

        self.operator_cloud.load_balancer.delete_availability_zone_profile(
            self.AVAILABILITY_ZONE_PROFILE_ID, ignore_missing=False
        )

    def test_lb_find(self):
        test_lb = self.operator_cloud.load_balancer.find_load_balancer(self.LB_NAME)
        self.assertEqual(self.LB_ID, test_lb.id)

    def test_lb_get(self):
        test_lb = self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)
        self.assertEqual(self.LB_NAME, test_lb.name)
        self.assertEqual(self.LB_ID, test_lb.id)
        self.assertEqual(self.VIP_SUBNET_ID, test_lb.vip_subnet_id)

    def test_lb_get_stats(self):
        test_lb_stats = self.operator_cloud.load_balancer.get_load_balancer_statistics(self.LB_ID)
    ...
    def test_lb_list(self):
        names = [lb.name for lb in self.operator_cloud.load_balancer.load_balancers()]
        self.assertIn(self.LB_NAME, names)

    def test_lb_update(self):
        self.operator_cloud.load_balancer.update_load_balancer(self.LB_ID, name=self.UPDATE_NAME)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_lb = self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)
        ...
        self.operator_cloud.load_balancer.update_load_balancer(self.LB_ID, name=self.LB_NAME)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_lb = self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)
    ...
    def test_lb_failover(self):
        self.operator_cloud.load_balancer.failover_load_balancer(self.LB_ID)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_lb = self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)
    ...
    def test_listener_find(self):
        test_listener = self.operator_cloud.load_balancer.find_listener(self.LISTENER_NAME)
        self.assertEqual(self.LISTENER_ID, test_listener.id)

    def test_listener_get(self):
        test_listener = self.operator_cloud.load_balancer.get_listener(self.LISTENER_ID)
        self.assertEqual(self.LISTENER_NAME, test_listener.name)
        self.assertEqual(self.LISTENER_ID, test_listener.id)
    ...
    def test_listener_get_stats(self):
        test_listener_stats = self.operator_cloud.load_balancer.get_listener_statistics(
            self.LISTENER_ID
        )
    ...
    def test_listener_list(self):
        names = [ls.name for ls in self.operator_cloud.load_balancer.listeners()]
        self.assertIn(self.LISTENER_NAME, names)

    def test_listener_update(self):
        self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)

        self.operator_cloud.load_balancer.update_listener(self.LISTENER_ID, name=self.UPDATE_NAME)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_listener = self.operator_cloud.load_balancer.get_listener(self.LISTENER_ID)
        ...
        self.operator_cloud.load_balancer.update_listener(self.LISTENER_ID, name=self.LISTENER_NAME)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_listener = self.operator_cloud.load_balancer.get_listener(self.LISTENER_ID)
    ...
    def test_pool_get(self):
        test_pool = self.operator_cloud.load_balancer.get_pool(self.POOL_ID)
        self.assertEqual(self.POOL_NAME, test_pool.name)
        self.assertEqual(self.POOL_ID, test_pool.id)
        self.assertEqual(self.PROTOCOL, test_pool.protocol)

    def test_pool_list(self):
        names = [pool.name for pool in self.operator_cloud.load_balancer.pools()]
        self.assertIn(self.POOL_NAME, names)

    def test_pool_update(self):
        self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)

        self.operator_cloud.load_balancer.update_pool(self.POOL_ID, name=self.UPDATE_NAME)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_pool = self.operator_cloud.load_balancer.get_pool(self.POOL_ID)
        ...
        self.operator_cloud.load_balancer.update_pool(self.POOL_ID, name=self.POOL_NAME)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_pool = self.operator_cloud.load_balancer.get_pool(self.POOL_ID)
    ...
    def test_member_find(self):
        test_member = self.operator_cloud.load_balancer.find_member(self.MEMBER_NAME, self.POOL_ID)
        self.assertEqual(self.MEMBER_ID, test_member.id)

    def test_member_get(self):
        test_member = self.operator_cloud.load_balancer.get_member(self.MEMBER_ID, self.POOL_ID)
        self.assertEqual(self.MEMBER_NAME, test_member.name)
        self.assertEqual(self.MEMBER_ID, test_member.id)
    ...
    def test_member_list(self):
        names = [mb.name for mb in self.operator_cloud.load_balancer.members(self.POOL_ID)]
        self.assertIn(self.MEMBER_NAME, names)

    def test_member_update(self):
        self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)

        self.operator_cloud.load_balancer.update_member(
    ...
    def test_health_monitor_find(self):
        test_hm = self.operator_cloud.load_balancer.find_health_monitor(self.HM_NAME)
        self.assertEqual(self.HM_ID, test_hm.id)

    def test_health_monitor_get(self):
        test_hm = self.operator_cloud.load_balancer.get_health_monitor(self.HM_ID)
        self.assertEqual(self.HM_NAME, test_hm.name)
        self.assertEqual(self.HM_ID, test_hm.id)
    ...
    def test_health_monitor_list(self):
        names = [hm.name for hm in self.operator_cloud.load_balancer.health_monitors()]
        self.assertIn(self.HM_NAME, names)

    def test_health_monitor_update(self):
        self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)

        self.operator_cloud.load_balancer.update_health_monitor(self.HM_ID, name=self.UPDATE_NAME)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_hm = self.operator_cloud.load_balancer.get_health_monitor(self.HM_ID)
        ...
        self.operator_cloud.load_balancer.update_health_monitor(self.HM_ID, name=self.HM_NAME)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_hm = self.operator_cloud.load_balancer.get_health_monitor(self.HM_ID)
    ...
    def test_l7_policy_find(self):
        test_l7_policy = self.operator_cloud.load_balancer.find_l7_policy(self.L7POLICY_NAME)
        self.assertEqual(self.L7POLICY_ID, test_l7_policy.id)

    def test_l7_policy_get(self):
        test_l7_policy = self.operator_cloud.load_balancer.get_l7_policy(self.L7POLICY_ID)
    ...
    def test_l7_policy_list(self):
        names = [l7.name for l7 in self.operator_cloud.load_balancer.l7_policies()]
        self.assertIn(self.L7POLICY_NAME, names)

    def test_l7_policy_update(self):
        self.operator_cloud.load_balancer.get_load_balancer(self.LB_ID)

        self.operator_cloud.load_balancer.update_l7_policy(self.L7POLICY_ID, name=self.UPDATE_NAME)
        self.operator_cloud.load_balancer.wait_for_load_balancer(
            self.LB_ID, wait=self._wait_for_timeout
        )
        test_l7_policy = self.operator_cloud.load_balancer.get_l7_policy(self.L7POLICY_ID)
        ...
        self.operator_cloud.load_balancer.update_l7_policy(
    ...
    def test_l7_rule_get(self):
        test_l7_rule = self.operator_cloud.load_balancer.get_l7_rule(
    ...
    def test_l7_rule_list(self):
        ids = [
            l7.id for l7 in self.operator_cloud.load_balancer.l7_rules(l7_policy=self.L7POLICY_ID)
        ]
    ...
    def test_l7_rule_update(self):
        ...
        self.operator_cloud.load_balancer.update_l7_rule(
            self.L7RULE_ID,
            l7_policy=self.L7POLICY_ID,
    ...
    def test_quota_list(self):
        for qot in self.operator_cloud.load_balancer.quotas():
            self.assertIsNotNone(qot.project_id)

    def test_quota_get(self):
        test_quota = self.operator_cloud.load_balancer.get_quota(self.PROJECT_ID)
        self.assertEqual(self.PROJECT_ID, test_quota.id)

    def test_quota_update(self):
        attrs = {"load_balancer": 12345, "pool": 67890}
        for project_quota in self.operator_cloud.load_balancer.quotas():
            self.operator_cloud.load_balancer.update_quota(project_quota, **attrs)
            new_quota = self.operator_cloud.load_balancer.get_quota(project_quota.project_id)
            self.assertEqual(12345, new_quota.load_balancers)
            self.assertEqual(67890, new_quota.pools)

    def test_default_quota(self):
        self.operator_cloud.load_balancer.get_quota_default()

    def test_providers(self):
        providers = self.operator_cloud.load_balancer.providers()
        # Make sure our default provider is in the list
        self.assertTrue(any(prov["name"] == self.AMPHORA for prov in providers))

    def test_provider_flavor_capabilities(self):
        capabilities = self.operator_cloud.load_balancer.provider_flavor_capabilities(self.AMPHORA)
        # Make sure a known capability is in the default provider
        self.assertTrue(any(cap["name"] == "loadbalancer_topology" for cap in capabilities))
    ...
    def test_flavor_profile_get(self):
        test_flavor_profile = self.operator_cloud.load_balancer.get_flavor_profile(
    ...
    def test_flavor_profile_list(self):
        names = [fv.name for fv in self.operator_cloud.load_balancer.flavor_profiles()]
        self.assertIn(self.FLAVOR_PROFILE_NAME, names)

    def test_flavor_profile_update(self):
        self.operator_cloud.load_balancer.update_flavor_profile(
    ...
    def test_flavor_find(self):
        test_flavor = self.operator_cloud.load_balancer.find_flavor(self.FLAVOR_NAME)
        self.assertEqual(self.FLAVOR_ID, test_flavor.id)

    def test_flavor_get(self):
        test_flavor = self.operator_cloud.load_balancer.get_flavor(self.FLAVOR_ID)
        self.assertEqual(self.FLAVOR_NAME, test_flavor.name)
        self.assertEqual(self.FLAVOR_ID, test_flavor.id)
    ...
    def test_flavor_list(self):
        names = [fv.name for fv in self.operator_cloud.load_balancer.flavors()]
        self.assertIn(self.FLAVOR_NAME, names)

    def test_flavor_update(self):
        self.operator_cloud.load_balancer.update_flavor(self.FLAVOR_ID, name=self.UPDATE_NAME)
        test_flavor = self.operator_cloud.load_balancer.get_flavor(self.FLAVOR_ID)
        self.assertEqual(self.UPDATE_NAME, test_flavor.name)

        self.operator_cloud.load_balancer.update_flavor(self.FLAVOR_ID, name=self.FLAVOR_NAME)
        test_flavor = self.operator_cloud.load_balancer.get_flavor(self.FLAVOR_ID)
        self.assertEqual(self.FLAVOR_NAME, test_flavor.name)

    def test_amphora_list(self):
        amp_ids = [amp.id for amp in self.operator_cloud.load_balancer.amphorae()]
        self.assertIn(self.AMPHORA_ID, amp_ids)

    def test_amphora_find(self):
        test_amphora = self.operator_cloud.load_balancer.find_amphora(self.AMPHORA_ID)
        self.assertEqual(self.AMPHORA_ID, test_amphora.id)

    def test_amphora_get(self):
        test_amphora = self.operator_cloud.load_balancer.get_amphora(self.AMPHORA_ID)
        self.assertEqual(self.AMPHORA_ID, test_amphora.id)

    def test_amphora_configure(self):
        self.operator_cloud.load_balancer.configure_amphora(self.AMPHORA_ID)
        test_amp = self.operator_cloud.load_balancer.get_amphora(self.AMPHORA_ID)
        self.assertEqual(self.AMPHORA_ID, test_amp.id)

    def test_amphora_failover(self):
        self.operator_cloud.load_balancer.failover_amphora(self.AMPHORA_ID)
        test_amp = self.operator_cloud.load_balancer.get_amphora(self.AMPHORA_ID)
        self.assertEqual(self.AMPHORA_ID, test_amp.id)

    def test_availability_zone_profile_find(self):
        test_profile = self.operator_cloud.load_balancer.find_availability_zone_profile(
    ...
    def test_availability_zone_profile_get(self):
        test_availability_zone_profile = (
            self.operator_cloud.load_balancer.get_availability_zone_profile(
        ...
        self.assertEqual(self.AMPHORA, test_availability_zone_profile.provider_name)
        self.assertEqual(
    ...
    def test_availability_zone_profile_list(self):
        names = [az.name for az in self.operator_cloud.load_balancer.availability_zone_profiles()]
        self.assertIn(self.AVAILABILITY_ZONE_PROFILE_NAME, names)

    def test_availability_zone_profile_update(self):
        self.operator_cloud.load_balancer.update_availability_zone_profile(
    ...
    def test_availability_zone_find(self):
        test_availability_zone = self.operator_cloud.load_balancer.find_availability_zone(
            self.AVAILABILITY_ZONE_NAME
        )
        self.assertEqual(self.AVAILABILITY_ZONE_NAME, test_availability_zone.name)

    def test_availability_zone_get(self):
        test_availability_zone = self.operator_cloud.load_balancer.get_availability_zone(
            self.AVAILABILITY_ZONE_NAME
        )
        self.assertEqual(self.AVAILABILITY_ZONE_NAME, test_availability_zone.name)
    ...
    def test_availability_zone_list(self):
        names = [az.name for az in self.operator_cloud.load_balancer.availability_zones()]
        self.assertIn(self.AVAILABILITY_ZONE_NAME, names)

    def test_availability_zone_update(self):
        self.operator_cloud.load_balancer.update_availability_zone(
            self.AVAILABILITY_ZONE_NAME, description=self.UPDATE_DESCRIPTION
        )
        test_availability_zone = self.operator_cloud.load_balancer.get_availability_zone(
            self.AVAILABILITY_ZONE_NAME
        )
        self.assertEqual(self.UPDATE_DESCRIPTION, test_availability_zone.description)
