...
class TestRegisteredLimit(base.BaseFunctionalTest):
    def setUp(self):
        ...
        self.addCleanup(self.system_admin_cloud.identity.delete_region, self.region)
        ...
        self.addCleanup(self.system_admin_cloud.identity.delete_service, self.service)

        self.resource_name = self.getUniqueString("resource")
        self.registered_limit_description = self.getUniqueString("registered_limit")

    def _delete_registered_limit(self, registered_limit):
        ret = self.system_admin_cloud.identity.delete_registered_limit(registered_limit)
        self.assertIsNone(ret)

    def test_registered_limit(self):
        # create the registered limit

        registered_limit = self.system_admin_cloud.identity.create_registered_limit(
        ...
        self.assertIsInstance(registered_limit, _registered_limit.RegisteredLimit)
        ...
        registered_limit = self.system_admin_cloud.identity.update_registered_limit(
            registered_limit, description=self.registered_limit_description
        )
        self.assertIsInstance(registered_limit, _registered_limit.RegisteredLimit)
        self.assertEqual(self.registered_limit_description, registered_limit.description)

        # retrieve details of the (updated) registered limit by ID

        registered_limit = self.system_admin_cloud.identity.get_registered_limit(
            registered_limit.id
        )
        self.assertIsInstance(registered_limit, _registered_limit.RegisteredLimit)
        self.assertEqual(self.registered_limit_description, registered_limit.description)
        ...
        registered_limits = list(self.system_admin_cloud.identity.registered_limits())
        self.assertIsInstance(registered_limits[0], _registered_limit.RegisteredLimit)
        self.assertIn(
            self.resource_name,
