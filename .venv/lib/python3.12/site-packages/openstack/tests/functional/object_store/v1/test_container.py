...
class TestContainer(base.BaseFunctionalTest):
    def setUp(self):
        ...
        container = self.operator_cloud.object_store.create_container(name=self.NAME)
        self.addEmptyCleanup(
            self.operator_cloud.object_store.delete_container,
            self.NAME,
    ...
    def test_list(self):
        names = [o.name for o in self.operator_cloud.object_store.containers()]
        self.assertIn(self.NAME, names)

    def test_system_metadata(self):
        # get system metadata
        container = self.operator_cloud.object_store.get_container_metadata(self.NAME)
        ...
        self.operator_cloud.object_store.set_container_metadata(
        ...
        self.operator_cloud.object_store.set_container_metadata(container, read_ACL=".r:demo")
        container = self.operator_cloud.object_store.get_container_metadata(self.NAME)
        self.assertEqual(".r:demo", container.read_ACL)
        self.assertEqual("demo:demo", container.write_ACL)

        # set system metadata and custom metadata
        self.operator_cloud.object_store.set_container_metadata(container, k0="v0", sync_key="1234")
        container = self.operator_cloud.object_store.get_container_metadata(self.NAME)
        ...
        self.operator_cloud.object_store.delete_container_metadata(container, ["sync_key"])
        container = self.operator_cloud.object_store.get_container_metadata(self.NAME)
        self.assertTrue(container.metadata)
        self.assertIn("k0", container.metadata)
        self.assertEqual("v0", container.metadata["k0"])
    ...
    def test_custom_metadata(self):
        ...
        self.operator_cloud.object_store.set_container_metadata(container)
        container = self.operator_cloud.object_store.get_container_metadata(container)
        self.assertFalse(container.metadata)

        # set empty custom metadata
        self.operator_cloud.object_store.set_container_metadata(container, k0="")
        container = self.operator_cloud.object_store.get_container_metadata(container)
        self.assertFalse(container.metadata)

        # set custom metadata
        self.operator_cloud.object_store.set_container_metadata(container, k1="v1")
        container = self.operator_cloud.object_store.get_container_metadata(container)
        self.assertTrue(container.metadata)
        ...
        self.operator_cloud.object_store.set_container_metadata(self.NAME, k2="v2")
        container = self.operator_cloud.object_store.get_container_metadata(container)
        self.assertTrue(container.metadata)
        ...
        # update metadata
        self.operator_cloud.object_store.set_container_metadata(container, k1="v1.1")
        container = self.operator_cloud.object_store.get_container_metadata(self.NAME)
        self.assertTrue(container.metadata)
        self.assertEqual(2, len(container.metadata))
        self.assertIn("k1", container.metadata)
        ...
        # delete metadata
        self.operator_cloud.object_store.delete_container_metadata(container, ["k1"])
        container = self.operator_cloud.object_store.get_container_metadata(self.NAME)
        self.assertTrue(container.metadata)
        self.assertEqual(1, len(container.metadata))
        self.assertIn("k2", container.metadata)
