...
class TestFlavor(base.BaseFunctionalTest):
    ...
    def test_flavor_access(self):
        ...
        flavor = self.user_cloud.compute.find_flavor(flavor_name, ignore_missing=True)
        self.assertIsNone(flavor)

        # validate we can see the new flavor ourselves

        flavor = self.operator_cloud.compute.find_flavor(flavor_name, ignore_missing=True)
        ...
        self.operator_cloud.compute.flavor_add_tenant_access(flavor.id, project["id"])
        ...
        self.operator_cloud.compute.flavor_remove_tenant_access(flavor.id, project["id"])
    ...
    def test_flavor_extra_specs(self):
        ...
        specs = {"a": "b"}
        self.operator_cloud.compute.create_flavor_extra_specs(flavor, extra_specs=specs)

        # verify specs were created correctly

        flavor_with_specs = self.operator_cloud.compute.fetch_flavor_extra_specs(flavor)
        self.assertDictEqual(specs, flavor_with_specs.extra_specs)

        # update/add a single extra spec property

        self.operator_cloud.compute.update_flavor_extra_specs_property(flavor, "c", "d")

        # fetch single property value

        prop_value = self.operator_cloud.compute.get_flavor_extra_specs_property(flavor, "c")
        self.assertEqual("d", prop_value)

        # delete the new property

        self.operator_cloud.compute.delete_flavor_extra_specs_property(flavor, "c")
