...
class TestBareMetalVolumetarget(base.BaseBaremetalTest):
    ...
    def test_volume_target_create_get_delete(self):
        self.operator_cloud.baremetal.set_node_provision_state(self.node, "manage", wait=True)
        self.operator_cloud.baremetal.set_node_power_state(self.node, "power off")
        ...
        loaded = self.operator_cloud.baremetal.get_volume_target(volume_target.id)
        self.assertEqual(loaded.id, volume_target.id)
        self.assertIsNotNone(loaded.node_id)

        with_fields = self.operator_cloud.baremetal.get_volume_target(
            volume_target.id, fields=["uuid", "extra"]
        )
        self.assertEqual(volume_target.id, with_fields.id)
        self.assertIsNone(with_fields.node_id)

        self.operator_cloud.baremetal.delete_volume_target(volume_target, ignore_missing=False)
        self.assertRaises(
            exceptions.NotFoundException,
            self.operator_cloud.baremetal.get_volume_target,
            volume_target.id,
        )

    def test_volume_target_list(self):
        node2 = self.create_node(name="test-node")
        self.operator_cloud.baremetal.set_node_provision_state(node2, "manage", wait=True)
        ...
        vts = self.operator_cloud.baremetal.volume_targets(node=self.node.id)
        self.assertEqual([v.id for v in vts], [vt2.id])

        vts = self.operator_cloud.baremetal.volume_targets(node=node2.id)
        self.assertEqual([v.id for v in vts], [vt1.id])

        vts = self.operator_cloud.baremetal.volume_targets(node="test-node")
        self.assertEqual([v.id for v in vts], [vt1.id])

        vts_with_details = self.operator_cloud.baremetal.volume_targets(details=True)
        for i in vts_with_details:
            self.assertIsNotNone(i.id)
            self.assertIsNotNone(i.volume_type)

        vts_with_fields = self.operator_cloud.baremetal.volume_targets(fields=["uuid", "node_uuid"])
    ...
    def test_volume_target_list_update_delete(self):
        ...
        volume_target = next(
            self.operator_cloud.baremetal.volume_targets(details=True, node=self.node.id)
        )
        self.assertEqual(volume_target.extra, {"foo": "bar"})

        # This test checks that resources returned from listing are usable
        self.operator_cloud.baremetal.update_volume_target(volume_target, extra={"foo": 42})
        self.operator_cloud.baremetal.delete_volume_target(volume_target, ignore_missing=False)
    ...
    def test_volume_target_update(self):
        ...
        volume_target = self.operator_cloud.baremetal.update_volume_target(volume_target)
        self.assertEqual({"answer": 42}, volume_target.extra)

        volume_target = self.operator_cloud.baremetal.get_volume_target(volume_target.id)
        self.assertEqual({"answer": 42}, volume_target.extra)
    ...
    def test_volume_target_negative_non_existing(self):
        ...
        self.assertRaises(
            exceptions.NotFoundException,
            self.operator_cloud.baremetal.find_volume_target,
            uuid,
            ignore_missing=False,
        )
        self.assertRaises(
            exceptions.NotFoundException,
            self.operator_cloud.baremetal.delete_volume_target,
            uuid,
            ignore_missing=False,
        )
        self.assertIsNone(self.operator_cloud.baremetal.find_volume_target(uuid))
        self.assertIsNone(self.operator_cloud.baremetal.delete_volume_target(uuid))
    ...
    def test_volume_target_fields(self):
        ...
        result = self.operator_cloud.baremetal.volume_targets(fields=["uuid", "node_id"])
        for item in result:
