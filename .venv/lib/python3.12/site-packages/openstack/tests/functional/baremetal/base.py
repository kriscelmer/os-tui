...
class BaseBaremetalTest(base.BaseFunctionalTest):
    ...
    def create_node(self, driver="fake-hardware", **kwargs):
        node = self.operator_cloud.baremetal.create_node(driver=driver, **kwargs)
        self.node_id = node.id
        self.addCleanup(
            lambda: self.operator_cloud.baremetal.delete_node(self.node_id, ignore_missing=True)
        )
    ...
    def create_port(self, node_id=None, **kwargs):
        ...
        port = self.operator_cloud.baremetal.create_port(node_uuid=node_id, **kwargs)
        self.addCleanup(
            lambda: self.operator_cloud.baremetal.delete_port(port.id, ignore_missing=True)
        )
        return port

    def create_port_group(self, node_id=None, **kwargs):
        node_id = node_id or self.node_id
        port_group = self.operator_cloud.baremetal.create_port_group(node_uuid=node_id, **kwargs)
        self.addCleanup(
    ...
    def create_volume_connector(self, node_id=None, **kwargs):
        node_id = node_id or self.node_id
        volume_connector = self.operator_cloud.baremetal.create_volume_connector(
    ...
    def create_deploy_template(self, **kwargs):
        ...
        deploy_template = self.system_admin_cloud.baremetal.create_deploy_template(**kwargs)
    ...
    def create_runbook(self, **kwargs):
        ...
        self.addCleanup(
            lambda: self.operator_cloud.baremetal.delete_runbook(runbook.id, ignore_missing=True)
        )
        return runbook
    ...
    def create_inspection_rule(self, **kwargs):
        ...
        inspection_rule = self.system_admin_cloud.baremetal.create_inspection_rule(**kwargs)
