...
class Base(base.BaseBaremetalTest):
    ...
    def _create_available_node(self):
        ...
        self.addCleanup(
            lambda: self.operator_cloud.baremetal.update_node(node.id, instance_id=None)
        )
        return node
...
class TestBareMetalAllocation(Base):
    ...
    def test_allocation_create_get_delete(self):
        ...
        self.operator_cloud.baremetal.delete_allocation(allocation, ignore_missing=False)
        self.assertRaises(
    ...
    def test_allocation_list(self):
        ...
        self.operator_cloud.baremetal.wait_for_allocation(allocation2, ignore_error=True)
        ...
        allocations = self.operator_cloud.baremetal.allocations(node=self.node.id)
        self.assertEqual([p.id for p in allocations], [allocation1.id])
    ...
    def test_allocation_negative_failure(self):
        ...
        allocation = self.operator_cloud.baremetal.get_allocation(allocation.id)
    ...
    def test_allocation_negative_non_existing(self):
        ...
        self.assertIsNone(self.operator_cloud.baremetal.delete_allocation(uuid))
...
class TestBareMetalAllocationUpdate(Base):
    ...
    def test_allocation_update(self):
        ...
        allocation = self.create_allocation(resource_class=self.resource_class)
        allocation = self.operator_cloud.baremetal.wait_for_allocation(allocation)
        self.assertEqual("active", allocation.state)
        self.assertIsNone(allocation.last_error)
