...
class TestBareMetalVolumeconnector(base.BaseBaremetalTest):
    ...
    def test_volume_connector_create_get_delete(self):
        ...
        loaded = self.operator_cloud.baremetal.get_volume_connector(volume_connector.id)
    ...
    def test_volume_connector_list(self):
        ...
        vcs = self.operator_cloud.baremetal.volume_connectors(node=self.node.id)
    ...
    def test_volume_connector_list_update_delete(self):
        ...
        volume_connector = next(
            self.operator_cloud.baremetal.volume_connectors(details=True, node=self.node.id)
        )
        self.assertEqual(volume_connector.extra, {"foo": "bar"})

        # This test checks that resources returned from listing are usable
        self.operator_cloud.baremetal.update_volume_connector(volume_connector, extra={"foo": 42})
        self.operator_cloud.baremetal.delete_volume_connector(
            volume_connector, ignore_missing=False
        )
    ...
    def test_volume_connector_update(self):
        ...
        volume_connector = self.operator_cloud.baremetal.update_volume_connector(volume_connector)
        self.assertEqual({"answer": 42}, volume_connector.extra)

        volume_connector = self.operator_cloud.baremetal.get_volume_connector(volume_connector.id)
        self.assertEqual({"answer": 42}, volume_connector.extra)
    ...
    def test_volume_connector_patch(self):
        ...
        volume_connector = self.operator_cloud.baremetal.patch_volume_connector(
            volume_connector,
    ...
    def test_volume_connector_negative_non_existing(self):
        ...
        self.assertIsNone(self.operator_cloud.baremetal.find_volume_connector(uuid))
        self.assertIsNone(self.operator_cloud.baremetal.delete_volume_connector(uuid))
    ...
    def test_volume_connector_fields(self):
        ...
        result = self.operator_cloud.baremetal.volume_connectors(fields=["uuid", "node_id"])
        for item in result:
