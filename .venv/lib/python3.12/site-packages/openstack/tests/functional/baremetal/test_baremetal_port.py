...
class TestBareMetalPort(base.BaseBaremetalTest):
    ...
    def test_port_create_get_delete(self):
        ...
        loaded = self.operator_cloud.baremetal.get_port(port.id)
        self.assertEqual(loaded.id, port.id)
        self.assertIsNotNone(loaded.address)

        with_fields = self.operator_cloud.baremetal.get_port(
            port.id, fields=["uuid", "extra", "node_id"]
        )
        self.assertEqual(port.id, with_fields.id)
        self.assertIsNone(with_fields.address)

        self.operator_cloud.baremetal.delete_port(port, ignore_missing=False)
        self.assertRaises(
            exceptions.NotFoundException,
            self.operator_cloud.baremetal.get_port,
            port.id,
        )
    ...
    def test_port_list(self):
        ...
        ports = self.operator_cloud.baremetal.ports(address="11:22:33:44:55:77")
        self.assertEqual([p.id for p in ports], [port2.id])

        ports = self.operator_cloud.baremetal.ports(node=node2.id)
        self.assertEqual([p.id for p in ports], [port1.id])

        ports = self.operator_cloud.baremetal.ports(node="test-node")
        self.assertEqual([p.id for p in ports], [port1.id])
    ...
    def test_port_list_update_delete(self):
        ...
        port = next(self.operator_cloud.baremetal.ports(details=True, address="11:22:33:44:55:66"))
        self.assertEqual(port.extra, {"foo": "bar"})

        # This test checks that resources returned from listing are usable
        self.operator_cloud.baremetal.update_port(port, extra={"foo": 42})
        self.operator_cloud.baremetal.delete_port(port, ignore_missing=False)
    ...
    def test_port_update(self):
        ...
        port = self.operator_cloud.baremetal.update_port(port)
        self.assertEqual("66:55:44:33:22:11", port.address)
        self.assertEqual({"answer": 42}, port.extra)

        port = self.operator_cloud.baremetal.get_port(port.id)
        self.assertEqual("66:55:44:33:22:11", port.address)
        self.assertEqual({"answer": 42}, port.extra)
    ...
    def test_port_negative_non_existing(self):
        ...
        self.assertRaises(
            exceptions.NotFoundException,
            self.operator_cloud.baremetal.find_port,
            uuid,
            ignore_missing=False,
        )
        self.assertRaises(
            exceptions.NotFoundException,
            self.operator_cloud.baremetal.delete_port,
            uuid,
            ignore_missing=False,
        )
        self.assertRaises(
            exceptions.NotFoundException,
            self.operator_cloud.baremetal.update_port,
            uuid,
            pxe_enabled=True,
        )
        self.assertIsNone(self.operator_cloud.baremetal.find_port(uuid))
        self.assertIsNone(self.operator_cloud.baremetal.delete_port(uuid))
...
class TestBareMetalPortFields(base.BaseBaremetalTest):
    ...
    def test_port_fields(self):
        ...
        result = self.operator_cloud.baremetal.ports(fields=["uuid", "node_id"])
        for item in result:
