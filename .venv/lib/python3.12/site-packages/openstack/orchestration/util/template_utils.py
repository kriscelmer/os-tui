...
def get_template_contents(
    ...
    if template_url:
        ...
        raise exceptions.SDKException(
            "Must provide one of template_file, template_url or template_object"
        )

    if not tpl:
        raise exceptions.SDKException(f"Could not fetch template from {template_url}")
    ...
    try:
        ...
        raise exceptions.SDKException(f"Error parsing template {template_url} {e}")

    tmpl_base_url = utils.base_url_for_url(template_url)
    if files is None:
        files = {}
    resolve_template_get_files(template, files, tmpl_base_url, is_object, object_request)
            ...
            if str_url not in files:
                ...
                if is_template(file_content):
                    if is_object:
                        ...
                        template = get_template_contents(template_url=str_url, files=files)[1]
