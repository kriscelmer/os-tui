...
class CloudRegion:
    ...
    def __init__(
        ...
        collector_registry: ty.Optional["prometheus_client.CollectorRegistry"] = None,
    ...
    def name(self) -> str:
        ...
        if auth and isinstance(auth, ks_identity_base.BaseIdentityPlugin) and auth.auth_url:
    ...
    def set_service_value(self, key: str, service_type: str, value: ty.Any) -> None:
        key = _make_key(key, service_type)
        self.config[key] = value

    def set_session_constructor(self, session_constructor: type[ks_session.Session]) -> None:
    ...
    def _get_service_config(self, key: str, service_type: str) -> ty.Any | None:
        config_dict = self.config.get(key)
        if not config_dict:
            return None

        if not isinstance(config_dict, dict):
            raise RuntimeError(f"invalid configuration for service type {service_type!r}")
    ...
    def get_region_name(self, service_type: str | None = None) -> str | None:
        ...
        value = self._get_config("region_name", service_type, fallback_to_unprefixed=True)
        return str(value) if value is not None else value

    def get_interface(self, service_type: str | None = None) -> list[str] | str | None:
        value = self._get_config("interface", service_type, fallback_to_unprefixed=True)
        if value is None:
            return value
        if isinstance(value, str):
            return value
        if isinstance(value, list) and all({isinstance(x, str) or x is None for x in value}):
            return value

        raise exceptions.ConfigException(
            f"interface should be str, list of str or None but is {type(value)}"
        )
    ...
    def get_service_type(self, service_type: str) -> str:
        ...
        if self._service_type_manager.is_known(service_type):
            service_type = self._service_type_manager.get_service_type(service_type)
        value = self._get_config("service_type", service_type, default=service_type)
        return str(value) if value is not None else value
    ...
    def get_endpoint(self, service_type: str) -> str | None:
        ...
        if not value and service_type == "identity" and SCOPE_KEYS.isdisjoint(set(auth.keys())):
        ...
        service_type = self._service_type_manager.get_service_type(service_type)
        if value and self.config.get("profile") == "rackspace" and service_type == "block-storage":
    ...
    def get_connect_retries(self, service_type: str) -> int | None:
        value = self._get_config("connect_retries", service_type, fallback_to_unprefixed=True)
        return int(value) if value is not None else value

    def get_status_code_retries(self, service_type: str) -> int | None:
        value = self._get_config("status_code_retries", service_type, fallback_to_unprefixed=True)
        return int(value) if value is not None else value
    ...
    def get_session(self) -> ks_session.Session:
        ...
        if self._keystone_session is None:
            ...
            # cert verification
            if not verify:
                self.log.debug(f"Turning off SSL warnings for {self.full_name} since verify=False")
    ...
    def _get_version_request(self, service_type: str, version: str | None) -> _util.VersionRequest:
    ...
    def get_all_version_data(self, service_type: str) -> list[discover.VersionData]:
    ...
    def _get_hardcoded_endpoint(self, service_type: str, constructor: type[proxy.Proxy]) -> str:
    ...
    def get_external_networks(self) -> list[str]:
        ...
        return [net["name"] for net in self.config.get("networks", []) if net["routes_externally"]]
    ...
    def get_client_config(
        ...
        """
        return self._get_extra_config(name, self._get_extra_config("client", defaults))
    ...
    def get_rate_limit(self, service_type: str) -> float | None:
        return self._get_service_config("rate_limit", service_type=service_type)

    def get_concurrency(self, service_type: str) -> int | None:
        return self._get_service_config("concurrency", service_type=service_type)
    ...
    def has_service(self, service_type: str) -> bool:
        service_type = service_type.lower().replace("-", "_")
        key = f"has_{service_type}"
        value = self.config.get(key, self._service_type_manager.is_official(service_type))
        assert isinstance(value, bool)
        return value

    def disable_service(self, service_type: str, reason: str | None = None) -> None:
