...
PLATFORMDIRS = platformdirs.PlatformDirs("openstack", "OpenStack", multipath=True)
...
if SNAP_REAL_HOME:
    UNIX_CONFIG_HOME = os.path.join(os.path.join(SNAP_REAL_HOME, ".config"), "openstack")
else:
    UNIX_CONFIG_HOME = os.path.join(os.path.expanduser(os.path.join("~", ".config")), "openstack")
...
JSON_SUFFIXES = (".json",)
CONFIG_FILES = [
    os.path.join(d, "clouds" + s) for d in CONFIG_SEARCH_PATH for s in YAML_SUFFIXES + JSON_SUFFIXES
]
SECURE_FILES = [
    os.path.join(d, "secure" + s) for d in CONFIG_SEARCH_PATH for s in YAML_SUFFIXES + JSON_SUFFIXES
]
VENDOR_FILES = [
...
class OpenStackConfig:
    ...
    def __init__(
        ...
        if secure_config_filename and secure_config:
            ...
            cloud_config = _util.merge_clouds(cloud_config or {}, secure_config)
        ...
        if force_ipv4 is not None:
            ...
            prefer_ipv6 = get_boolean(
                self._get_envvar(
                    "OS_PREFER_IPV6",
                    client_config.get("prefer_ipv6", client_config.get("prefer-ipv6", True)),
                )
            )
            force_ipv4 = get_boolean(
                self._get_envvar(
                    "OS_FORCE_IPV4",
                    client_config.get("force_ipv4", client_config.get("broken-ipv6", False)),
                )
            )
    ...
    def _get_os_environ(self, envvar_prefix: str | None = None) -> dict[str, ty.Any] | None:
    ...
    def _validate_config_file(self, path: str, data: ty.Any) -> bool:
        ...
        if "clouds" not in data:
            ...
            self.log.warning("Configuration file %s does not contain a 'clouds' key", path)
            return False
    ...
    def _expand_regions(self, regions: list[str | dict[str, ty.Any]]) -> list[dict[str, ty.Any]]:
        ret = []
        for region in regions:
            if isinstance(region, dict):
                # i.e. must have name key, and only name,values keys
                if "name" not in region or not {"name", "values"} >= set(region):
                    raise exceptions.ConfigException(f"Invalid region entry at: {region}")
    ...
    def _get_region(self, cloud: str | None = None, region_name: str = "") -> dict[str, ty.Any]:
        ...
        if (
            ...
            if profile_data:
                ...
                if status == "deprecated":
                    ...
                    raise exceptions.ConfigException(
                        f"{profile_name} references a cloud that no longer exists: {message}"
                    )
                _auth_update(cloud, profile_data)
            else:
                # Can't find the requested vendor config, go about business
                warnings.warn(
                    f"Couldn't find the vendor profile {profile_name} for the cloud {name}",
    ...
    def _validate_networks(self, networks: list[dict[str, ty.Any]], key: str) -> None:
        value = None
        for net in networks:
    ...
    def _fix_backwards_networks(self, cloud: dict[str, ty.Any]) -> dict[str, ty.Any]:
    ...
    def _fix_backwards_auth(self, cloud: dict[str, ty.Any]) -> dict[str, ty.Any]:
        mappings = {
    ...
    def _fix_backwards_auth_plugin(self, cloud: dict[str, ty.Any]) -> dict[str, ty.Any]:
    ...
    def register_argparse_arguments(
        ...
        for service_key in service_keys:
            ...
            parser.add_argument(
                "--{service_key}-api-version".format(service_key=service_key.replace("_", "-")),
    ...
    def _fix_backwards_interface(self, cloud: dict[str, ty.Any]) -> dict[str, ty.Any]:
        new_cloud = {}
    ...
    def _fix_backwards_api_timeout(self, cloud: dict[str, ty.Any]) -> dict[str, ty.Any]:
        new_cloud = {}
    ...
    def _get_auth_loader(self, config: dict[str, ty.Any]) -> loading.BaseLoader[ty.Any]:
        ...
        loader: loading.BaseLoader[ty.Any] = loading.get_plugin_loader(config["auth_type"])
    ...
    def option_prompt(self, config: dict[str, ty.Any], p_opt: "opts.Opt") -> dict[str, ty.Any]:
    ...
    def _handle_value_types(self, config: dict[str, ty.Any]) -> dict[str, ty.Any]:
...
if __name__ == "__main__":
    ...
    for cloud in config:
        ...
        if len(sys.argv) == 1:
            print_cloud = True
        elif len(sys.argv) == 3 and (sys.argv[1] == cloud.name and sys.argv[2] == cloud.region):
            print_cloud = True
