        ...
        if not invalid_keys:
            ...
            if not allow_unknown_params:
                raise exceptions.InvalidResourceQuery(
                    message="Invalid query params: {}".format(",".join(invalid_keys)),
...
class Resource(dict):
    ...
    def __init__(self, _synchronized=False, connection=None, **attrs):
        ...
        self._body = _ComponentManager(attributes=body, synchronized=_synchronized)
        self._header = _ComponentManager(attributes=header, synchronized=_synchronized)
        self._uri = _ComponentManager(attributes=uri, synchronized=_synchronized)
        self._computed = _ComponentManager(attributes=computed, synchronized=_synchronized)
        ...
        if self._store_unknown_attrs_as_properties:
            ...
            self._store_unknown_attrs_as_properties = (
                hasattr(self.__class__, "properties") and self.__class__.properties.type is None
            )
    ...
    @classmethod
    def _attributes_iterator(cls, components=tuple([fields.Body, fields.Header])):
        ...
        if name == "id":
            ...
            try:
                ...
                if name in self._attr_aliases:
                    ...
                    return object.__getattribute__(self, self._attr_aliases[name])
                if self._allow_unknown_attrs_in_body:
    ...
    def _attributes(self, remote_names=False, components=None, include_aliases=True):
    ...
    def _update_from_body_attrs(self, attrs: collections.abc.MutableMapping[str, ty.Any]) -> None:
        body = self._consume_body_attrs(attrs)
        self._body.attributes.update(body)
        self._body.clean()

    def _update_from_header_attrs(self, attrs: collections.abc.MutableMapping[str, ty.Any]) -> None:
        headers = self._consume_header_attrs(attrs)
        self._header.attributes.update(headers)
        self._header.clean()

    def _update_uri_from_attrs(self, attrs: collections.abc.MutableMapping[str, ty.Any]) -> None:
        uri = self._consume_uri_attrs(attrs)
    ...
    @classmethod
    def _get_mapping(cls, component: type[fields._BaseComponent]) -> ty.MutableMapping[str, ty.Any]:
    ...
    def to_dict(
        ...
        if not components:
            raise ValueError("At least one of `body`, `headers` or `computed` must be True")
        ...
        *,
        ...
        if patch:
            if not self._store_unknown_attrs_as_properties:
                ...
                new = self._unpack_properties_to_resource_root(self._body.attributes)
                original_body = self._unpack_properties_to_resource_root(self._original_body)
    ...
    def _prepare_request(
        ...
        **kwargs,
        ...
        if requires_id:
            if self.id is None:
                raise exceptions.InvalidRequest("Request requires an ID but none was found")
    ...
    def _assert_microversion_for(
        ...
        *,
        ...
        if actual is None:
            message = f"API version {expected} is required, but the default version will be used."
            _raise(message)
        ...
        if expected is not None:
            ...
            if actual_n < expected_n:
                message = f"API version {expected} is required, but {actual} will be used."
                _raise(message)
    ...
    def create(
        ...
        **params: ty.Any,
        ...
        if self.create_method == "PUT":
            ...
            raise exceptions.ResourceFailure(f"Invalid create method: {self.create_method}")
    ...
    def bulk_create(
        ...
        **params: ty.Any,
        ...
        if not (data and isinstance(data, list) and all([isinstance(x, dict) for x in data])):
        ...
        if cls.create_method == "PUT":
            ...
            raise exceptions.ResourceFailure(f"Invalid create method: {cls.create_method}")
        ...
        for attrs in data:
            ...
            request = resource._prepare_request(requires_id=requires_id, base_path=base_path)
        ...
        has_body = cls.has_body if cls.create_returns_body is None else cls.create_returns_body
    ...
    def _commit(
        ...
        try:
            ...
            raise exceptions.ResourceFailure(f"Invalid commit method: {method}")
    ...
    def delete(
        ...
        **kwargs: ty.Any,
        ...
        response = self._raw_delete(session, microversion=microversion, **kwargs)
        kwargs = {}
        if error_message:
            kwargs["error_message"] = error_message

        self._translate_response(response, has_body=False, error_message=error_message)
        return self
            ...
            if resources and paginated:
                ...
                try:
                    if next_params["marker"] == last_marker:
                        ...
                        # services had bugs.
                        raise exceptions.SDKException("Endless pagination loop detected, aborting")
                except KeyError:
    ...
    def find(
        ...
        **params: ty.Any,
        ...
        raise exceptions.NotFoundException(f"No {cls.__name__} found for {name_or_id}")
...
def wait_for_status(
    ...
    for count in utils.iterate_timeout(timeout=wait, message=msg, wait=interval):
        ...
        if not resource:
            raise exceptions.ResourceFailure(f"{name} went away while waiting for {status}")
        ...
        if normalized_status == _normalize_status(status):
            ...
            raise exceptions.ResourceFailure(f"{name} transitioned to failure state {new_status}")

        LOG.debug(
            "Still waiting for resource %s to reach state %s, current state is %s",
            name,
            status,
            new_status,
        )
