...
class Object(_base.BaseResource):
    ...
    #: the object, in bytes.
    content_length = resource.Header("content-length", type=int, alias="_bytes")
    ...
    #: *Type: bool*
    is_static_large_object = resource.Header("x-static-large-object", type=bool)
    ...
    last_modified_at = resource.Header("last-modified", alias="_last_modified", aka="updated_at")
    ...
    is_content_type_detected = resource.Header("x-detect-content-type", type=bool)
    ...
    access_control_allow_origin = resource.Header("access-control-allow-origin")
    ...
    def set_metadata(self, session, metadata, refresh=True):
        ...
        filtered_metadata = {key: value for key, value in metadata.items() if value}
    ...
    def _download(self, session, error_message=None, stream=False):
        ...
        response = session.get(request.url, headers=request.headers, stream=stream)
    ...
    def stream(self, session, error_message=None, chunk_size=1024):
        response = self._download(session, error_message=error_message, stream=True)
    ...
    def create(self, session, prepend_key=True, base_path=None, **kwargs):
        ...
        response = session.put(request.url, data=self.data, headers=request.headers)
        self._translate_response(response, has_body=False)
        return self
