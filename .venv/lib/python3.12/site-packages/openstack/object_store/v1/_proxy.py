...
class Proxy(proxy.Proxy):
    ...
    def delete_container(self, container, ignore_missing=True):
        ...
        :returns: ``None``
        """
        self._delete(_container.Container, container, ignore_missing=ignore_missing)
    ...
    def get_object(
        ...
        _object = self._get_resource(_obj.Object, obj, container=container_name)
        request = _object._prepare_request()

        get_stream = outfile is not None

        response = self.get(request.url, headers=request.headers, stream=get_stream)
        ...
        if outfile:
            ...
            for chunk in response.iter_content(resp_chunk_size, decode_unicode=False):
    ...
    def create_object(
        ...
        **headers,
        ...
        if data is not None:
            self.log.debug("swift uploading data to %(endpoint)s", {"endpoint": endpoint})
            return self._create(
                _obj.Object,
    ...
    def is_object_stale(self, container, name, filename, file_md5=None, file_sha256=None):
        ...
        try:
            ...
            self._connection.log.debug(f"swift stale check, no object: {container}/{name}")
            return True
    ...
    def _upload_large_object(self, endpoint, filename, headers, file_size, segment_size, use_slo):
        ...
        try:
            ...
            try:
                ...
                self.log.debug(
                    "Failed to upload large object manifest for %s. Removing segment uploads.",
                    segment_prefix,
                )
    ...
    def get_object_segment_size(self, segment_size):
        ...
        try:
            ...
            self._connection.log.info(
                "Swift capabilities not supported. Using default max file size."
            )
    ...
    def _delete_autocreated_image_objects(self, container=None, segment_prefix=None):
    ...
    def _service_cleanup(
        ...
        try:
            ...
            if bulk_delete is not None:
                ...
                bulk_delete_max_per_request = bulk_delete.get("max_deletes_per_request", 10000)
