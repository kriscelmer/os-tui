...
class ServiceDescription:
    ...
    def __init__(
        ...
        self.supported_versions = supported_versions or self.supported_versions or {}
    ...
    def _validate_proxy(self, proxy, endpoint):
        ...
        if exc or not endpoint or not service_url:
            raise exceptions.ServiceDiscoveryException(
                "Failed to create a working proxy for service {service_type}: {message}".format(
        ...
        if version_string:
            ...
            if getattr(
                ...
                return config.get_session_client(
                    self.service_type,
                    allow_version_hack=True,
                    constructor=self.supported_versions[str(supported_versions[0])],
                    version=version_string,
                )
        else:
            version_kwargs["min_version"] = str(supported_versions[0])
            version_kwargs["max_version"] = f"{str(supported_versions[-1])}.latest"
