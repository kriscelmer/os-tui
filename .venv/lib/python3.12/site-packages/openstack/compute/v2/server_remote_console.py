...
class ServerRemoteConsole(resource.Resource):
    ...
    def create(self, session, prepend_key=True, base_path=None, **params):
        ...
        if not utils.supports_microversion(session, "2.99") and self.type == "spice-direct":
            raise ValueError("Console type spice-direct is not supported on server side")
        return super().create(session, prepend_key=prepend_key, base_path=base_path, **params)
