...
class CatalogHackTests(utils.TestCase):
    ...
    def setUp(self):
        ...
        self.hacks.add_discover_hack(self.IDENTITY, re.compile("/v2.0/?$"), "/")
    ...
    def test_ignored_non_service_type(self):
        self.assertEqual(self.V2_URL, self.hacks.get_discover_hack("other", self.V2_URL))
...
class DiscoverUtils(utils.TestCase):
    ...
    def test_version_args(self):
        ...
        def assert_min_max(in_ver, in_min, in_max, in_type, out_min, out_max):
            self.assertEqual(
                (out_min, out_max),
                discover._normalize_version_args(in_ver, in_min, in_max, service_type=in_type),
            )
...
class VersionDataTests(utils.TestCase):
    ...
    def test_endpoint_data_http_basic_override_no_discover(self):
        ...
        self.assertEqual(data.url, V3_URL)
        self.assertEqual(
            plugin.get_api_major_version(self.session, endpoint_override=V3_URL),
            (3, 0),
        )
        self.assertEqual(plugin.get_endpoint(self.session, endpoint_override=V3_URL), V3_URL)
    ...
    def test_keystone_version_data(self):
        ...
        valid_v3_versions = (
            ...
            disc.versioned_data_for(min_version="v3.0", max_version="v3.latest"),
            ...
            disc.versioned_data_for(min_version="3.latest", max_version="latest"),
            disc.versioned_data_for(min_version="latest", max_version="latest"),
        ...
        valid_v2_versions = (
            ...
            disc.versioned_data_for(min_version=2, max_version=(2, discover.LATEST)),
            disc.versioned_data_for(min_version="2.latest", max_version="2.latest"),
        )
    ...
    def test_cinder_version_data(self):
        ...
        for version in (
            disc.data_for("v2.0"),
            disc.versioned_data_for(min_version="v2.0", max_version="v2.latest"),
        ):
            self.assertEqual((2, 0), version["version"])
        ...
        for version in (
            disc.data_for(1),
            disc.versioned_data_for(min_version=(1,), max_version=(1, discover.LATEST)),
        ):
    ...
    def test_glance_version_data(self):
        ...
        for ver in (2, 2.1, 2.2):
            for version in (
                disc.data_for(ver),
                disc.versioned_data_for(min_version=ver, max_version=(2, discover.LATEST)),
            ):
        ...
        for ver in (1, 1.1):
            for version in (
                disc.data_for(ver),
                disc.versioned_data_for(min_version=ver, max_version=(1, discover.LATEST)),
            ):
