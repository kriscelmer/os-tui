...
class TestBetamaxHooks(testtools.TestCase):
    def test_pre_record_hook_v3(self):
        ...
        self.assertEqual(response_content["token"]["expires_at"], "9999-12-31T23:59:59Z")
        self.assertEqual(response_content["token"]["project"]["domain"]["id"], "dummy")
        self.assertEqual(response_content["token"]["user"]["domain"]["id"], "dummy")
        self.assertEqual(response_content["token"]["user"]["name"], "dummy")

        request_content = json.loads(interaction.data["request"]["body"]["string"])
        self.assertEqual(
            request_content["auth"]["identity"]["password"]["user"]["domain"]["id"],
            "dummy",
        )
        self.assertEqual(
            request_content["auth"]["identity"]["password"]["user"]["password"],
            "********",
        )
    ...
    def test_pre_record_hook_v2(self):
        ...
        self.assertEqual(response_content["access"]["token"]["tenant"]["name"], "dummy")
        ...
        self.assertEqual(request_content["auth"]["passwordCredentials"]["username"], "dummy")
