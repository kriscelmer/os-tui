...
class ServiceCatalogTest(utils.TestCase):
    def setUp(self):
        ...
        self.AUTH_RESPONSE_BODY = fixture.V3Token(audit_chain_id=uuid.uuid4().hex)
    ...
    def test_service_catalog_alias_find_exact_match(self):
        ...
        public_ep = sc.get_endpoints(service_type="volumev3", interface="public")
    ...
    def test_service_catalog_get_endpoints_region_names(self):
        ...
        for endpoint in endpoints["image"]:
            self.assertEqual(endpoint["url"], self.north_endpoints[endpoint["interface"]])

        endpoints = sc.get_endpoints(service_type="image", region_name="South")
        self.assertEqual(len(endpoints), 1)
        for endpoint in endpoints["image"]:
            self.assertEqual(endpoint["url"], self.south_endpoints[endpoint["interface"]])
    ...
    def test_service_catalog_service_name(self):
        ...
        urls = sc.get_urls(service_type="image", service_name="glance", interface="public")
