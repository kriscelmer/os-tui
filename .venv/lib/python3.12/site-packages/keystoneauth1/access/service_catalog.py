...
class ServiceCatalog(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def is_interface_match(self, endpoint: dict[str, str], interface: str) -> bool:
    ...
    def _normalize_endpoints(self, endpoints: list[dict[str, ty.Any]]) -> list[dict[str, ty.Any]]:
    ...
    def _denormalize_endpoints(self, endpoints: list[discover.EndpointData]) -> list[str | None]:
    ...
    def endpoint_data_for(
        ...
        if service_name and region_name:
            ...
            msg = f"{interface} endpoint for {service_type} service named {service_name} not found"
        elif region_name:
            msg = (
                f"{interface} endpoint for {service_type} service in {region_name} region not found"
            )
        else:
