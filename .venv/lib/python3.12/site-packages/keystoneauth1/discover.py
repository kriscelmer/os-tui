...
def get_version_data(
    ...
    try:
        ...
        # available versions.
        try:
            return ty.cast(list[dict[str, ty.Any]], body_resp["versions"]["values"])
    ...
    raise exceptions.DiscoveryFailure(f"Invalid Response - Bad version data returned: {err_text}")
...
def version_between(
    ...
    if normalized_min_version and normalized_candidate < normalized_min_version:
        return False

    # If the candidate is higher than the max_version, it's not a match.
    # No max_version means no upper bound.
    if normalized_max_version and normalized_candidate > normalized_max_version:
        return False

    return True


def version_match(required: _PARSED_VERSION_T, candidate: _PARSED_VERSION_T) -> bool:
...
def _latest_soft_match(required: _PARSED_VERSION_T | None, candidate: _PARSED_VERSION_T) -> bool:
    if not required:
        return False
...
class Discover:
    ...
    def __init__(
        ...
        self._url = url
        self._data = get_version_data(session, url, authenticated=authenticated)
    ...
    def raw_version_data(
        ...
        for v in self._data:
            try:
                ...
                _LOGGER.warning(
                    "Skipping over invalid version data. No stability status in version."
                )
                continue
    ...
    def versioned_data_for(
        self,
        url: str | None = None,
        min_version: str | int | float | ty.Iterable[str | int | float] | None = None,
        max_version: str | int | float | ty.Iterable[str | int | float] | None = None,
        *,
        allow_experimental: bool = False,
        allow_deprecated: bool = True,
        allow_unknown: bool = False,
        ...
        :rtype: dict
        """
        normalized_min_version, normalized_max_version = _normalize_version_args(
        ...
        if normalized_max_version == (LATEST, LATEST) and (
            not normalized_min_version or normalized_min_version == (LATEST, LATEST)
        ):
...
class EndpointData:
    ...
    def _infer_version_data(self, project_id: str | None = None) -> list["VersionData"]:
    ...
    def _get_discovery_url_choices(
        ...
        try:
            ...
            exact_match = bool(is_between and max_version and max_version[0] == url_version[0])
            high_match = (
                is_between
                and max_version
...
class _VersionHacks:
    ...
    def add_discover_hack(self, service_type: str, old: re.Pattern[str], new: str = "") -> None:
...
def add_catalog_discover_hack(service_type: str, old: re.Pattern[str], new: str) -> None:
