...
_PreparedRequestT = ty.TypeVar("_PreparedRequestT", bound=requests.PreparedRequest)
...
class _SamlAuth(requests.auth.AuthBase):
    ...
    * HTTP POST request again to the Service Provider. The body of the
    ...
    def _handle_response(self, response: requests.Response, **kwargs: ty.Any) -> requests.Response:
        if response.status_code == 200 and response.headers.get("Content-Type") == _PAOS_HEADER:
            response = self._ecp_retry(response, **kwargs)

        return response

    def _ecp_retry(self, sp_response: requests.Response, **kwargs: ty.Any) -> requests.Response:
        history = [sp_response]
        ...
        relay_state = authn_request.xpath(_XPATH_SP_RELAY_STATE, namespaces=_XML_NAMESPACES)[0]
