...
class Token(loading.BaseV3Loader[identity.V3Token]):
    ...
    def get_options(self) -> list[opts.Opt]:
        ...
        options.extend([loading.Opt("token", secret=True, help="Token to authenticate with")])
...
class OpenIDConnectClientCredentials(_OpenIDConnectBase[identity.V3OidcClientCredentials]):
    ...
    def get_options(self) -> list[opts.Opt]:
        ...
        options.extend(
            [
                ...
                loading.Opt("password", secret=True, help="Password", required=True),
                loading.Opt(
                    "idp_otp_key",
...
class OpenIDConnectAuthorizationCode(_OpenIDConnectBase[identity.V3OidcAuthorizationCode]):
...
class OpenIDConnectAccessToken(loading.BaseFederationLoader[identity.V3OidcAccessToken]):
...
class OpenIDConnectDeviceAuthorization(_OpenIDConnectBase[identity.V3OidcDeviceAuthorization]):
...
class TokenlessAuth(loading.BaseLoader[identity.V3TokenlessAuth]):
    ...
    def get_options(self) -> list[opts.Opt]:
        ...
        options.extend(
            [
                loading.Opt("auth-url", required=True, help="Authentication URL"),
                ...
                loading.Opt("project-domain-id", help="Domain ID containing project"),
                loading.Opt(
                    "project-domain-name",
    ...
    def load_from_options(self, **kwargs: ty.Any) -> identity.V3TokenlessAuth:
        if (
            ...
            or (
                ...
                and not (kwargs.get("project_domain_name") or kwargs.get("project_domain_id"))
            )
        ):
            m = (
...
class ApplicationCredential(loading.BaseV3Loader[identity.V3ApplicationCredential]):
    ...
    def load_from_options(self, **kwargs: ty.Any) -> identity.V3ApplicationCredential:
        _assert_identity_options(kwargs)
...
class OAuth2ClientCredential(loading.BaseV3Loader[identity.V3OAuth2ClientCredential]):
    ...
    def load_from_options(self, **kwargs: ty.Any) -> identity.V3OAuth2ClientCredential:
...
class OAuth2mTlsClientCredential(loading.BaseV3Loader[identity.V3OAuth2mTlsClientCredential]):
        ...
        options.extend(
            [
                ...
                loading.Opt(
                    "oauth2-client-id",
                    required=True,
                    help="Client credential ID for OAuth2.0 Mutual-TLS Authorization",
                ),
            ]
        )
        return options

    def load_from_options(self, **kwargs: ty.Any) -> identity.V3OAuth2mTlsClientCredential:
        if not kwargs.get("oauth2_endpoint"):
