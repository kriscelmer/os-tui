    ...
    def get_conf_options(
        ...
        opts = [
            ...
            cfg.ListOpt(
                ...
                help="List of interfaces, in order of preference, for endpoint URL.",
            ),
            cfg.StrOpt(
                "region-name",
...
def process_conf_options(confgrp: "cfg.OptGroup", kwargs: dict[str, ty.Any]) -> None:
    ...
    if confgrp.valid_interfaces and getattr(confgrp, "interface", None):
        raise TypeError(
            "interface and valid_interfaces are mutually exclusive. Please use valid_interfaces."
        )
...
def register_service_argparse_arguments(parser: argparse.ArgumentParser, service_type: str) -> None:
    return adapter.register_service_adapter_argparse_arguments(parser, service_type)
...
def load_from_conf_options(conf: "cfg.ConfigOpts", group: str, **kwargs: ty.Any) -> adapter.Adapter:
