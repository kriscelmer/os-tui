...
V3FederationPluginT = ty.TypeVar("V3FederationPluginT", bound="v3_plugins.FederationBaseAuth")
GenericPluginT = ty.TypeVar("GenericPluginT", bound="generic_plugins.BaseGenericPlugin")
    ...
    def get_options(self) -> list[opts.Opt]:
        ...
        options.extend(
            [
                ...
                opts.Opt("trust-id", help="ID of the trust to use as a trustee use"),
            ]
        )
...
class BaseV3Loader(BaseIdentityLoader[V3PluginT]):
    ...
    def load_from_options(self, **kwargs: ty.Any) -> V3PluginT:
        if kwargs.get("project_name") and not (
            kwargs.get("project_domain_name") or kwargs.get("project_domain_id")
        ):
            m = (
