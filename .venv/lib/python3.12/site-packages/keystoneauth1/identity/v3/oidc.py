...
class _OidcBase(federation.FederationBaseAuth, metaclass=abc.ABCMeta):
    ...
    def _get_discovery_document(self, session: ks_session.Session) -> dict[str, object]:
    ...
    def _get_access_token(self, session: ks_session.Session, payload: dict[str, str | None]) -> str:
        ...
        if _logger.isEnabledFor(logging.DEBUG):
            ...
            _logger.debug(
                "Making OpenID-Connect authentication request to %s with data %s",
    ...
    def get_unscoped_auth_ref(self, session: ks_session.Session) -> access.AccessInfoV3:
        ...
        discovery = self._get_discovery_document(session)
        grant_types = ty.cast(list[str] | None, discovery.get("grant_types_supported"))
    ...
    @abc.abstractmethod
    def get_payload(self, session: ks_session.Session) -> dict[str, str | None]:
...
class OidcPassword(_OidcBase):
    ...
    def manage_otp_from_session_or_request_to_the_user(
        ...
        if otp_from_session:
            ...
            payload[self.idp_otp_key] = input("Please, enter the generated OTP code: ")
...
class OidcDeviceAuthorization(_OidcBase):
    ...
    def _get_device_authorization_endpoint(self, session: ks_session.Session) -> str | None:
    ...
    def get_payload(self, session: ks_session.Session) -> dict[str, str | None]:
        ...
        self.verification_uri_complete = op_response.json().get("verification_uri_complete")
    ...
    def _get_access_token(self, session: ks_session.Session, payload: dict[str, str | None]) -> str:
        ...
        if self.verification_uri_complete:
            _logger.warning(f"To authenticate please go to: {self.verification_uri_complete}")
        else:
            _logger.warning(
