...
class OAuth2mTlsClientCredential(base.BaseAuth, metaclass=abc.ABCMeta):
    ...
    def get_auth_ref(self, session: ks_session.Session) -> access.AccessInfoV3:
        ...
        resp = session.get(url=token_url, authenticated=False, headers=headers, log=False)
        try:
            resp_data = resp.json()
        except ValueError:
        ...
        return access.AccessInfoV3(auth_token=self.oauth2_access_token, body=resp_data)
