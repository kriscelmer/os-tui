...
class _StringFormatter:
    ...
    def __init__(self, session: "Session", auth: ty.Optional["plugin.BaseAuthPlugin"]):
    ...
    def __getitem__(self, item: str) -> ty.Any:
        ...
        if not value:
            raise ValueError(
                f"This type of authentication does not provide a {item} that can be substituted"
            )
...
def _determine_calling_package() -> str:
    ...
    mod_lookup = {m.__file__: n for n, m in sys.modules.copy().items() if hasattr(m, "__file__")}
...
class RequestTiming:
    ...
    def __init__(self, method: str | None, url: str | None, elapsed: datetime.timedelta):
        self.method = method
        self.url = url
...
class Session:
    ...
    def __init__(
        ...
        additional_headers: collections.abc.MutableMapping[str, str] | None = None,
    ...
    def mount(self, scheme: str, adapter: requests.adapters.BaseAdapter) -> None:
    ...
    def _http_log_request(
        ...
        if headers:
            ...
            for header in sorted(headers.items()):
                string_parts.append('-H "{}: {}"'.format(*self._process_header(header)))
        if json:
    ...
    def _http_log_response(
        ...
        *,
        ...
        if headers:
            # Sort headers so that testing can work consistently.
            for header in sorted(headers.items()):
                string_parts.append("{}: {}".format(*self._process_header(header)))
    ...
    def _set_microversion_headers(
        ...
        if parsed_microversion[0] != discover.LATEST and discover.LATEST in parsed_microversion[1:]:
        ...
        headers.setdefault("OpenStack-API-Version", f"{service_type} {microversion}")
    ...
    def request(
        ...
        **kwargs: ty.Any,
        ...
        if kwargs.get("headers"):
            kwargs["headers"] = requests.structures.CaseInsensitiveDict(kwargs["headers"])
        else:
        ...
        retriable_status_codes = retriable_status_codes or _RETRIABLE_STATUS_CODES
        ...
        if not urllib.parse.urlparse(url).netloc:
            ...
            if endpoint_override:
                ...
                base_url = self.get_endpoint(auth, allow=allow, **endpoint_filter)
        ...
        if self.original_ip:
            headers.setdefault("Forwarded", f"for={self.original_ip};by={user_agent}")
        ...
        if log:
            ...
            request_id = resp.headers.get("x-openstack-request-id") or resp.headers.get(
            ...
            if request_id:
                ...
                if service_name:
                    ...
                    id_logger.debug(
                        "%(method)s call to %(url)s used request id %(response_request_id)s",
                        {
                            "method": resp.request.method,
                            "url": resp.url,
                            "response_request_id": request_id,
    ...
    def _send_request(
        ...
        **kwargs: ty.Any,
        ...
        try:
            ...
            logger.warning(
                "Failure: %(e)s. Retrying in %(delay).1fs. %(retries)s retries left",
        ...
        if log:
            self._http_log_response(response=resp, logger=logger, split_loggers=split_loggers)
        ...
        if resp.status_code in self._REDIRECT_STATUSES:
            ...
            try:
                ...
                logger.warning(
                    "Failed to redirect request to %s as new location was not provided.",
                    resp.url,
                )
            else:
                ...
                resp = new_resp
        elif resp.status_code in retriable_status_codes and status_code_retries > 0:
    ...
    def get_token(self, auth: ty.Optional["plugin.BaseAuthPlugin"] = None) -> str | None:
    ...
    def invalidate(self, auth: ty.Optional["plugin.BaseAuthPlugin"] = None) -> bool:
    ...
    def get_user_id(self, auth: ty.Optional["plugin.BaseAuthPlugin"] = None) -> str | None:
    ...
    def get_project_id(self, auth: ty.Optional["plugin.BaseAuthPlugin"] = None) -> str | None:
...
class TCPKeepAliveAdapter(requests.adapters.HTTPAdapter):
    ...
    def init_poolmanager(self, *args: ty.Any, **kwargs: ty.Any) -> None:
        if "socket_options" not in kwargs:
            ...
            if hasattr(socket, "TCP_KEEPCNT") and not utils.is_windows_linux_subsystem:
                socket_options += [
