...
def swap_memory():
    ...
    try:
        ...
        msg = f"'sin' and 'sout' swap memory stats couldn't be determined and were set to 0 ({err})"
        warnings.warn(msg, RuntimeWarning, stacklevel=2)
        sin = sout = 0
    else:
        with f:
        ...
        with open_text(file) as f:
            ...
            for lineno, line in enumerate(f, 1):
                try:
                    ...
                    msg = f"error while parsing {file}; malformed line {lineno} {line!r}"
    ...
    if os.path.exists(f"/proc/{os.getpid()}/io"):
        ...
        def io_counters(self):
            ...
            try:
                ...
                msg = f"{err.args[0]!r} field was not found in {fname}; found fields are {fields!r}"
                ...
                if isinstance(err, ValueError) or err.errno == errno.EINVAL:
                    ...
                    for cpu in cpus:
                        if cpu not in all_cpus:
                            msg = f"invalid CPU {cpu!r}; choose between {eligible_cpus!r}"
                            raise ValueError(msg) from None
                        if cpu not in eligible_cpus:
                            msg = (
                                f"CPU number {cpu} is not eligible; choose between {eligible_cpus}"
                            )
        ...
        def rlimit(self, resource_, limits=None):
            ...
            try:
                if limits is None:
                    ...
                    # set
                    if len(limits) != 2:
                        msg = f"second argument must be a (soft, hard) tuple, got {limits!r}"
                        raise ValueError(msg)
