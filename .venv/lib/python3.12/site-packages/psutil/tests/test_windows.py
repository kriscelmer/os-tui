    ...
    def test_nic_names(self):
        ...
        for nic in nics:
            if "pseudo-interface" in nic.replace(" ", "-").lower():
                continue
            if nic not in out:
                return pytest.fail(f"{nic!r} nic wasn't found in 'ipconfig /all' output")
            ...
            for wmi_part in wmi_parts:
                if ps_part.device.replace("\\", "") == wmi_part.DeviceID:
                    ...
                    if abs(usage.free - wmi_free) > 10 * 1024 * 1024:
                        return pytest.fail(f"psutil={usage.free}, wmi={wmi_free}")
                    break
            else:
...
class RemoteProcessTestCase(PsutilTestCase):
    ...
    def setUp(self):
        ...
        if other_python is None:
            return pytest.skip("could not find interpreter with opposite bitness")
        if IS_64BIT:
