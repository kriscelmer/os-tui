...
def all_projects():
    ...
    if not yielded:
        raise Exception("no projects found: is PROJECTS set, and do the paths exist")
...
class TestIntegration(base.BaseTestCase):
    ...
    def test_integration(self):
        ...
        project_name = packaging.utils.canonicalize_name(self.short_name).lower()
        ...
        if os.path.exists(setup_cfg):
            ...
            if config.has_section("metadata"):
                ...
                project_name = packaging.utils.canonicalize_name(raw_name).lower()
        ...
        with open(constraints, "r") as src:
            with open(tmp_constraints, "w") as dest:
                for line in src:
                    ...
                    constraint = packaging.utils.canonicalize_name(constraint).lower()
        ...
        python = venv.python
        self.useFixture(
            pbr_fixtures.CapturedSubprocess("sdist", [python, "setup.py", "sdist"], cwd=path)
        )
        ...
        self.useFixture(pbr_fixtures.CapturedSubprocess("tarball", [python] + pip_cmd + [filename]))
        ...
        python = venv.python
        self.useFixture(
            pbr_fixtures.CapturedSubprocess("install-editable", [python] + pip_cmd + ["-e", path])
        )
...
class TestInstallWithoutPbr(base.BaseTestCase):
    ...
    def test_install_without_pbr(self):
        ...
        venv = self.useFixture(pbr_fixtures.Venv("nopbr", ["pip", "wheel", "setuptools<80"]))
        python = venv.python
        ...
        self.useFixture(
            pbr_fixtures.CapturedSubprocess("nopbr", [python] + ["-m", "pkgReq"], cwd=test_pkg_dir)
        )
        pbr_cmd = os.path.join(venv.path, "bin", "pbr")
        self.useFixture(
            pbr_fixtures.CapturedSubprocess("nopbr", [pbr_cmd] + ["freeze"], cwd=test_pkg_dir)
        )
...
class TestLTSSupport(base.BaseTestCase):
    ...
    def test_lts_venv_default_versions(self):
        venv = self.useFixture(pbr_fixtures.Venv("setuptools", modules=self.modules))
        bin_python = venv.python
