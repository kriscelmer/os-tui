...
class CacheRegion:
    ...
    def configure_from_config(self, config_dict: dict[str, Any], prefix: str) -> Self:
        ...
        return self.configure(
            ...
            replace_existing_backend=config_dict.get("%sreplace_existing_backend" % prefix, False),
        )
    ...
    def get_or_create_multi(
        ...
        for orig_key, mangled_key in orig_to_mangled.items():
            with Lock(
                ...
                expiration_time,
                async_creator=lambda mutex: async_creator(mutexes, orig_key, mutex),
            ):
                pass
        try:
            if mutexes:
                ...
                values_w_created = {
                    orig_to_mangled[k]: self._value(v) for k, v in zip(keys_to_get, new_values)
                }
                ...
                if not should_cache_fn:
                    ...
                    self._set_multi_cached_value_to_backend(
                        {k: v for k, v in values_w_created.items() if should_cache_fn(v.payload)}
                    )
    ...
    def _value(self, value: Any, metadata: MetaDataType | None = None) -> CachedValue:
    ...
    def _get_multi_from_backend(self, keys: Iterable[KeyType]) -> Sequence[CacheReturnType]:
        if self.deserializer:
            return [
    ...
    def _set_multi_cached_value_to_backend(self, mapping: Mapping[KeyType, CachedValue]) -> None:
        ...
        if self.serializer:
            self.backend.set_serialized_multi(
                {k: self._serialized_cached_value(v) for k, v in mapping.items()}
            )
        else:
    ...
    def set_multi(self, mapping: Mapping[KeyType, ValuePayload]) -> None:
        ...
        if self.serializer:
            if self.key_mangler:
                mapping = {
                    self.key_mangler(k): self._serialized_payload(v, metadata=metadata)
                    for k, v in mapping.items()
                }
            else:
                mapping = {
                    k: self._serialized_payload(v, metadata=metadata) for k, v in mapping.items()
                }
    ...
    def cache_multi_on_arguments(
        ...
        def cache_decorator(user_func):
            ...
            def set_(mapping):
                ...
                self.set_multi(
                    dict((gen_key, mapping[key]) for gen_key, key in zip(gen_keys, keys))
                )
