...
def kwarg_function_key_generator(
    ...
    args_with_defaults = dict(
        (argspec.args[(idx * -1)], default) for idx, default in enumerate(default_list, 1)
    )
...
def sha1_mangle_key(key: str | bytes) -> str:
    ...
    return sha1(key.encode("utf-8") if isinstance(key, str) else key).hexdigest()


def length_conditional_mangler(length: int, mangler: Callable[[str], str]) -> Callable[[str], str]:
