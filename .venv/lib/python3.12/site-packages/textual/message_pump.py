...
class _MessagePumpMeta(type):
    ...
    def __new__(
        ...
        **kwargs: Any,
        ...
        handlers: dict[type[Message], list[tuple[Callable, dict[str, tuple[SelectorSet, ...]]]]] = (
        ...
        for value in class_dict.values():
            if callable(value) and hasattr(value, "_textual_on"):
                textual_on: list[tuple[type[Message], dict[str, tuple[SelectorSet, ...]]]] = (
...
class MessagePump(metaclass=_MessagePumpMeta):
    ...
    async def wait_for_refresh(self) -> bool:
        ...
        """
        assert self._task is not None, "Node must be running before calling wait_for_refresh"
        ...
        while not self._closed:
            ...
            try:
                ...
                if self._message_queue.empty() or (
                    self._max_idle is not None and current_time - self._last_idle > self._max_idle
                ):
