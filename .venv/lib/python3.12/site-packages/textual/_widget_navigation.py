...
def find_next_enabled(
    ...
    if anchor is None:
        if candidates:
            return (
                find_first_enabled(candidates) if direction == 1 else find_last_enabled(candidates)
            )
        return None
...
def find_next_enabled_no_wrap(
    ...
    valid_candidates = candidates[start:] if direction == 1 else reversed(candidates[: start + 1])
