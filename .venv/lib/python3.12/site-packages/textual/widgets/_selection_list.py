...
class SelectionList(Generic[SelectionType], OptionList):
    ...
    class SelectionMessage(Generic[MessageSelectionType], Message):
        ...
        def __init__(self, selection_list: SelectionList[MessageSelectionType], index: int) -> None:
            ...
            self.selection: Selection[MessageSelectionType] = selection_list.get_option_at_index(
                index
            )
    ...
    def _message_toggled(self, option_index: int) -> None:
        ...
        """
        if self._send_messages:
            self.post_message(self.SelectionToggled(self, option_index).set_sender(self))
    ...
    def _apply_to_all(self, state_change: Callable[[SelectionType], bool]) -> Self:
        ...
        with self.prevent(self.SelectedChanged):
            for selection in self._options:
                changed = state_change(cast(Selection[SelectionType], selection).value) or changed
    ...
    def select(self, selection: Selection[SelectionType] | SelectionType) -> Self:
        ...
        """
        if self._select(
            selection.value if isinstance(selection, Selection) else cast(SelectionType, selection)
        ):
            self.refresh()
        return self
    ...
    def _make_selection(
        ...
        if isinstance(selection, tuple):
            if len(selection) == 2:
                selection = cast("tuple[ContentText, SelectionType, bool]", (*selection, False))
    ...
    def _get_left_gutter_width(self) -> int:
        ...
        """
        return len(
            ToggleButton.BUTTON_LEFT + ToggleButton.BUTTON_INNER + ToggleButton.BUTTON_RIGHT + " "
        )
