...
class ListView(VerticalScroll, can_focus=True, can_focus_children=False):
    ...
    def remove_items(self, indices: Iterable[int]) -> AwaitComplete:
        ...
        normalized_indices = set(index if index >= 0 else index + len(self) for index in indices)
    ...
    def action_cursor_up(self) -> None:
        ...
        if self.index is None:
            ...
            for index, item in loop_from_index(self._nodes, self.index, direction=-1, wrap=False):
                if not item.disabled:
                    self.index = index
                    break
