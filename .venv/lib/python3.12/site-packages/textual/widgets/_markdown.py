...
class MarkdownBlock(Static):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(*args, name=token.type, classes=f"level-{token.level}", **kwargs)
...
class MarkdownTableContent(Widget):
    ...
    def compose(self) -> ComposeResult:
        for header in self.headers:
            yield MarkdownTableCellContents(header, classes="header").with_tooltip(header)
        for row_index, row in enumerate(self.rows, 1):
            for cell in row:
                yield MarkdownTableCellContents(cell, classes=f"row{row_index} cell").with_tooltip(
                    cell.plain
                )
    ...
    async def _update_rows(self, updated_rows: list[list[Content]]) -> None:
        ...
        for row_index, row in enumerate(updated_rows, self.last_row):
            for cell in row:
                new_cells.append(Static(cell, classes=f"row{row_index} cell").with_tooltip(cell))
        self.last_row = row_index
...
class Markdown(Widget):
    ...
    async def _on_mount(self, _: Mount) -> None:
        ...
        if initial_markdown is None:
            self.post_message(
                Markdown.TableOfContentsUpdated(self, self._table_of_contents).set_sender(self)
            )
    ...
    def _parse_markdown(self, tokens: Iterable[Token]) -> Iterable[MarkdownBlock]:
        ...
        for token in tokens:
            ...
            if token_type == "heading_open":
                ...
                if token.info:
                    stack_append(get_block_class("list_item_ordered_open")(self, token, token.info))
                else:
                    item_count = sum(
                ...
                if token.type == "heading_close":
                    block.id = f"heading-{slug_for_tcss_id(block._content.plain)}-{id(block)}"
                if stack:
    ...
    def append(self, markdown: str) -> AwaitComplete:
        ...
        updated_source = "".join(self._markdown.splitlines(keepends=True)[self._last_parsed_line :])
        ...
        async def await_append() -> None:
            ...
            async with self.lock:
                ...
                any_headers = any(isinstance(block, MarkdownHeader) for block in new_blocks)
                ...
                if any_headers:
                    self._table_of_contents = None
                    self.post_message(
                        Markdown.TableOfContentsUpdated(self, self.table_of_contents).set_sender(
                            self
                        )
                    )
...
class MarkdownViewer(VerticalScroll, can_focus=False, can_focus_children=True):
    ...
    def compose(self) -> ComposeResult:
        markdown = Markdown(parser_factory=self._parser_factory, open_links=self._open_links)
