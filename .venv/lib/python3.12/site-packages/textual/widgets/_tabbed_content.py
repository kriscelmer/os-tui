...
class ContentTab(Tab):
    ...
    def __init__(self, label: ContentType, content_id: str, disabled: bool = False) -> None:
...
class TabbedContent(Widget):
    ...
    def remove_pane(self, pane_id: str) -> AwaitComplete:
        ...
        removal_awaitables: list[Awaitable] = [
            self.get_child_by_type(ContentTabs).remove_tab(ContentTab.add_prefix(pane_id))
        ]
        try:
        ...
        if self._is_associated_tabs(event.tabs):
            ...
            self.post_message(
                TabbedContent.TabActivated(
                    tabbed_content=self,
                    tab=self.get_child_by_type(ContentTabs).get_content_tab(self.active),
                )
            )
    ...
    def _watch_active(self, active: str) -> None:
        ...
        if active:
            ...
            self.post_message(TabbedContent.Cleared(tabbed_content=self).set_sender(self))
    ...
    def get_pane(self, pane_id: str | ContentTab) -> TabPane:
        ...
        if isinstance(pane_id, ContentTab):
            target_id = pane_id.id if pane_id.id is None else ContentTab.sans_prefix(pane_id.id)
        else:
            target_id = pane_id
        if target_id:
            ...
            return pane
        raise ValueError("'pane_id' must be a non-empty string or a ContentTab with an id.")
