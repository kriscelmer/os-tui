...
class _Template(Validator):
    ...
    def __init__(self, input: Input, template_str: str) -> None:
        ...
        while template_chars:
            ...
            if escaped:
                ...
                if pattern:
                    char_flags = _CharFlags.REQUIRED if required_flag else _CharFlags.NONE
                    char_definition = self.CharDefinition(re.compile(pattern), char_flags)
                else:
                    char_definition = self.CharDefinition(
                        re.compile(re.escape(char)), _CharFlags.SEPARATOR, char
                    )
        ...
        if all(
            (_CharFlags.SEPARATOR in char_definition.flags) for char_definition in self.template
        ):
            raise ValueError("Template must contain at least one non-separator character")
                ...
                if char == self.next_separator(cursor_position):
                    ...
                    if (cursor_position > 0) and (prev_position != cursor_position - 1):
                        ...
                        while cursor_position < next_position + 1:
                            if _CharFlags.SEPARATOR in self.template[cursor_position].flags:
    ...
    def delete_at_position(self, position: int | None = None) -> None:
        ...
        while pos > 0:
            ...
            if (_CharFlags.SEPARATOR not in char_definition.flags) and (value[pos - 1] != " "):
                break
            pos -= 1
        value = value[:pos]
    ...
    def action_delete_left_all(self) -> None:
        ...
        if self.cursor_position > 0:
            ...
            if cursor_position >= len(self.value):
                ...
                self.value = (
                    self._template.empty_mask[:cursor_position] + self.value[cursor_position:]
                )
