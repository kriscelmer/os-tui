...
class Underline(Widget):
    ...
    def _highlight_range(self) -> tuple[int, int]:
        ...
        return (self.highlight_start, self.highlight_end) if self.show_highlight else (0, 0)
...
class Tabs(Widget, can_focus=True):
        ...
        *,
        ...
        if isinstance(before, str):
            try:
                ...
                raise self.TabError(f"There is no tab with ID '{before}' to mount before")
        ...
        mount_await = self.query_one("#tabs-list").mount(tab_widget, before=before, after=after)
    ...
    def _scroll_active_tab(self) -> None:
        ...
        if self.active_tab:
            try:
                self.query_one("#tabs-scroll").scroll_to_center(self.active_tab, force=True)
            except NoMatches:
                pass
    ...
    def disable(self, tab_id: str) -> Tab:
        ...
        try:
            ...
            raise self.TabError(f"There is no tab with ID {tab_id!r} to disable.") from None
    ...
    def enable(self, tab_id: str) -> Tab:
        ...
        try:
            ...
            raise self.TabError(f"There is no tab with ID {tab_id!r} to enable.") from None
