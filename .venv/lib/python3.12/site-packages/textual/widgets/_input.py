...
class Input(ScrollView):
    ...
    def _watch_value(self, value: str) -> None:
        ...
        validation_result = self.validate(value) if "changed" in self.validate_on else None
    ...
    def render_line(self, y: int) -> Strip:
        ...
        if not self.value:
            ...
            strip = Strip(
                console.render(placeholder, console_options.update_width(max_content_width + 1))
            )
        else:
            result = self._value
    ...
    def _on_blur(self, event: Blur) -> None:
        self._pause_blink()
        validation_result = self.validate(self.value) if "blur" in self.validate_on else None
    ...
    async def _on_mouse_move(self, event: events.MouseMove) -> None:
        if self._selecting:
            ...
            self.selection = Selection(selection_start, self._cell_offset_to_index(offset.x))
    ...
    async def action_submit(self) -> None:
        ...
        """
        validation_result = self.validate(self.value) if "submitted" in self.validate_on else None
