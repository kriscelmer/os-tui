...
class _Styled:
    ...
    def __rich_console__(self, console: "Console", options: "ConsoleOptions") -> "RichRenderResult":
        ...
        if style:
            apply = style.__add__
            result_segments = (
                _Segment(text, apply(_style), None) for text, _style, control in result_segments
            )
...
class Widget(DOMNode):
    ...
    def __init__(
        ...
        *children: Widget,
        ...
        self._arrangement_cache: FIFOCache[tuple[Size, int, bool], DockArrangeResult] = FIFOCache(4)
    ...
    def visible_siblings(self) -> list[Widget]:
        ...
        Returns:
            List of siblings.
        """
        siblings = [widget for widget in self.siblings if widget.visible and widget.display]
        return siblings
    ...
    def _allow_scroll(self) -> bool:
        ...
        """
        return self.is_scrollable and (self.allow_horizontal_scroll or self.allow_vertical_scroll)
    ...
    def _check_anchor(self) -> None:
        ...
        if self._anchored and self._anchor_released and self.scroll_y >= self.max_scroll_y:
    ...
    def get_visual_style(self, *component_classes: str, partial: bool = False) -> VisualStyle:
        ...
        if (visual_style := self._visual_style_cache.get(cache_key, None)) is None:
            ...
            for styles in iter_styles():
                ...
                if has_rule("background"):
                    text_background = background + styles.background.tint(styles.background_tint)
                    if partial:
    ...
    def mount(
        ...
        *widgets: Widget,
        ...
        ids_to_mount = [widget_id for widget in widgets if (widget_id := widget.id) is not None]
        ...
        if before is not None and after is not None:
            raise MountError("Only one of `before` or `after` can be handled -- not both")
        ...
        mounted = self.app._register(parent, *widgets, before=insert_before, after=insert_after)
    ...
    def _get_box_model(
        ...
        if styles_width is None:
            ...
            if content_width < content_container.width and self._has_relative_children_width:
        ...
        model = BoxModel(content_width + gutter.width, content_height + gutter.height, margin)
    ...
    def max_scroll_x(self) -> int:
        ...
        return max(
            0,
            self.virtual_size.width - (self.container_size.width - self.scrollbar_size_vertical),
        )
    ...
    def _refresh_scrollbars(self) -> None:
        ...
        if not (
            ...
            if overflow_x == "auto" and show_vertical and not show_horizontal:
                show_horizontal = self.virtual_size.width > (width - styles.scrollbar_size_vertical)
    ...
    def scrollbar_gutter(self) -> Spacing:
        ...
        """
        return Spacing(0, self.scrollbar_size_vertical, self.scrollbar_size_horizontal, 0)
    ...
    def scrollable_content_region(self) -> Region:
        ...
        Returns:
            Screen region that contains a widget's content.
        """
        content_region = self.region.shrink(self.styles.gutter).shrink(self.scrollbar_gutter)
        return content_region
    ...
    def _has_relative_children_width(self) -> bool:
        ...
        for child in self.children:
            ...
            if width is None:
                continue
            if styles.is_relative_width or (width.is_auto and child._has_relative_children_width):
                return True
        return False
    ...
    def _resolve_extrema(
        ...
        if styles.min_width is not None:
            min_width = styles.min_width.resolve(container, viewport, width_fraction) - gutter_width

        if styles.max_width is not None:
            max_width = styles.max_width.resolve(container, viewport, width_fraction) - gutter_width
        if styles.min_height is not None:
            min_height = (
                styles.min_height.resolve(container, viewport, height_fraction) - gutter_height
            )

        if styles.max_height is not None:
            max_height = (
                styles.max_height.resolve(container, viewport, height_fraction) - gutter_height
            )
    ...
    def is_scrolling(self) -> bool:
        ...
        for node in self.ancestors:
            if not isinstance(node, Widget):
                break
            if node.scroll_x != node.scroll_target_x or node.scroll_y != node.scroll_target_y:
                return True
        ...
        *,
        ...
        if center:
            ...
            if origin_visible and (region.offset not in window.translate(delta)):
                delta = clamp_delta(Region.get_scroll_to_visible(window, region, top=True))
        else:
            delta = clamp_delta(
    ...
    def __init_subclass__(
        ...
        if issubclass(base, Widget):
            ...
            cls.can_focus_children = (
                base.can_focus_children if can_focus_children is None else can_focus_children
            )
    ...
    def _get_scrollable_region(self, region: Region) -> Region:
        ...
        show_vertical_scrollbar = bool(show_vertical_scrollbar and scrollbar_size_vertical)
        show_horizontal_scrollbar = bool(show_horizontal_scrollbar and scrollbar_size_horizontal)
    ...
    def _scroll_update(self, virtual_size: Size) -> None:
        ...
        if self.show_vertical_scrollbar:
            ...
            self.vertical_scrollbar.window_size = height - self.scrollbar_size_horizontal
    ...
    def remove_children(
        ...
        await_remove = self.app._prune(*children_to_remove, parent=cast(DOMNode, self._parent))
        return await_remove
    ...
    def check_message_enabled(self, message: Message) -> bool:
        ...
        if not super().check_message_enabled(message) or self._is_prevented(type(message)):
            return False
