...
def resolve(
    ...
    *,
    ...
    resolved: list[tuple[Scalar, Fraction | None]] = [
        ((scalar, None) if scalar.is_fraction else (scalar, scalar.resolve(size, viewport)))
    ...
    offsets = [0] + [
        ...
        for fraction in accumulate(
            value for fraction in resolved_fractions for value in (fraction, fraction_gutter)
        )
    ]
    results = [
        (offset1, offset2 - offset1) for offset1, offset2 in zip(offsets[::2], offsets[1::2])
    ]
...
def resolve_fraction_unit(
    ...
    return (
        Fraction(remaining_space, remaining_fraction) if remaining_fraction > 0 else initial_space
    )
    ...
    box_models: list[BoxModel | None] = [
        (
            ...
            if _dimension is not None and _dimension.is_fraction
                ...
                size,
                viewport_size,
                (fraction_zero if (_width := fraction_width - margin_width) < 0 else _width),
                (fraction_zero if (_height := fraction_height - margin_height) < 0 else _height),
                greedy=greedy,
            )
        )
        for (_dimension, widget, (margin_width, margin_height)) in zip(dimensions, widgets, margins)
    ]
