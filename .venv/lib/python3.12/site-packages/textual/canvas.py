...
class HorizontalLine(Primitive):
    ...
    def render(self, canvas: Canvas) -> None:
        ...
        if right in x_range:
            box_line[right] = _combine_quads(box_line[right], (0, 0, 0, line_type_index))
...
class Canvas:
    ...
    def __init__(self, width: int, height: int) -> None:
        ...
        self.lines: list[array[str]] = [array(array_type_code, blank_line) for _ in range(height)]
    ...
    def render(self, primitives: Sequence[Primitive], base_style: Style) -> StripRenderable:
        ...
        for raw_spans, line in zip(self.spans, self.lines):
            ...
            if raw_spans:
                ...
                spans = [
                    (0, False, 0),
                    *((span.start, False, index) for index, span in enumerate(raw_spans, 1)),
                    *((span.end, True, index) for index, span in enumerate(raw_spans, 1)),
                    (width, True, 0),
                ]
                    ...
                    if next_offset > offset:
                        segments.append(
                            _Segment(
                                ...
                                base_style
                                + Style.from_color(
                                    colors[max(color_indices) if color_indices else 0].rich_color
                                ),
                            )
                        )
