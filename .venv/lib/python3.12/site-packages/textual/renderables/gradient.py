    ...
    def __rich_console__(self, console: Console, options: ConsoleOptions) -> RenderResult:
        ...
        for y in range(height):
            line_color = from_color(
                default_color,
                (blend(color2, y / (height - 1)).rich_color if height > 1 else rich_color1),
            )
...
class LinearGradient:
    ...
    def __init__(self, angle: float, stops: Sequence[tuple[float, Color | str]]) -> None:
        self.angle = angle
        self._stops = [
            (stop, Color.parse(color) if isinstance(color, str) else color) for stop, color in stops
        ]
    ...
    def __rich_console__(self, console: Console, options: ConsoleOptions) -> RenderResult:
        ...
        for line_y in range(height):
            ...
            x2 = (center_x + (point_x * cos_angle - (point_y + 1.0) * sin_angle)) / width
