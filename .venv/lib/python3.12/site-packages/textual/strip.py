...
class Strip:
    ...
    def __init__(self, segments: Iterable[Segment], cell_length: int | None = None) -> None:
    ...
    def link_ids(self) -> set[str]:
        ...
        if self._link_ids is None:
            self._link_ids = {style._link_id for _, style, _ in self._segments if style is not None}
    ...
    @classmethod
    def from_lines(cls, lines: list[list[Segment]], cell_length: int | None = None) -> list[Strip]:
    ...
    def join(cls, strips: Iterable[Strip | None]) -> Strip:
        ...
        """
        join_strips = [strip for strip in strips if strip is not None and strip.cell_count]
        ...
        if all(strip._render_cache is not None for strip in join_strips):
            joined_strip._render_cache = "".join([strip._render_cache for strip in join_strips])
        return joined_strip
    ...
    def __getitem__(self, index: int | slice) -> Strip:
        ...
        return self.crop(index.start, self.cell_count if index.stop is None else index.stop)
    ...
    def adjust_cell_length(self, cell_length: int, style: Style | None = None) -> Strip:
        ...
        if current_cell_length < cell_length:
            ...
            new_line = line + [_Segment(" " * (cell_length - current_cell_length), style)]
    ...
    def apply_filter(self, filter: LineFilter, background: Color) -> Strip:
        ...
        if cached_strip is None:
            cached_strip = Strip(filter.apply(self._segments, background), self._cell_length)
    ...
    def apply_style(self, style: Style) -> Strip:
        ...
        if cached is not None:
            return cached
        styled_strip = Strip(Segment.apply_style(self._segments, style), self.cell_length)
    ...
    def render_style(cls, style: Style, text: str, color_system: ColorSystem) -> str:
        ...
        if style._link:
            output = f"\x1b]8;id={style._link_id};{style._link}\x1b\\{output}\x1b]8;;\x1b\\"
        return output
    ...
    def render(self, console: Console) -> str:
        ...
        if self._render_cache is None:
            ...
            self._render_cache = "".join(
                [
                    (text if style is None else render(style, text, color_system=color_system))
