    ...
    def arrange(
        ...
        if min_column_width is not None:
            ...
            table_size_columns = max(
                1,
                (container_width + gutter_horizontal) // (min_column_width + gutter_horizontal),
            )
        ...
        row_scalars = repeat_scalars(row_scalars, table_size_rows if table_size_rows else row + 1)
        ...
        if self.auto_minimum and self.shrink:
            ...
            for column_index in range(table_size_columns):
                for row_index in range(len(row_scalars)):
                    if (cell_info := cell_map.get((column_index, row_index))) is not None:
                        widget = cell_info[0]
                        column_minimums[column_index] = max(
                            visualize(widget, widget.render()).get_minimal_width(widget.styles)
                            + widget.styles.gutter.width,
        ...
        for widget, (column, row, column_span, row_span) in cell_size_map.items():
            ...
            region = (
                Region(x, y, int(box_width + margin.width), int(box_height + margin.height))
                .crop_size(cell_size)
                .shrink(margin)
            ) + offset
            ...
            absolute = widget_styles.has_rule("position") and styles.position == "absolute"
            add_placement(
                _WidgetPlacement(
                    region,
                    placement_offset,
                    (margin if gutter_spacing is None else margin.grow_maximum(gutter_spacing)),
                    widget,
                    absolute,
                )
            )
