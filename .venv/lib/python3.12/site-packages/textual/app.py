...
class App(Generic[ReturnType], DOMNode):
    ...
    *:disabled:can-focus {
    ...
    def __init__(
        ...
        ansi_theme = self.ansi_theme_dark if self.current_theme.dark else self.ansi_theme_light
        self.set_reactive(App.ansi_color, ansi_color)
        self._filters: list[LineFilter] = [ANSIToTruecolor(ansi_theme, enabled=not ansi_color)]
        ...
        self.title = self.TITLE if self.TITLE is not None else f"{self.__class__.__name__}"
        ...
        css_paths = [
            ...
            for css_path in (_css_path_type_as_list(css_path) if css_path is not None else [])
        ]
        ...
        self.css_monitor = (
            FileMonitor(self.css_path, self._on_css_change) if watch_css or self.debug else None
        )
        ...
        for variable_name, screen_collection in (
            ...
            for screen_name, screen_object in screen_collection.items():
                if not (isinstance(screen_object, str) or callable(screen_object)):
                    ...
                    raise TypeError(f"expected a callable or string, got {screen_object!r}")
    ...
    def get_system_commands(self, screen: Screen) -> Iterable[SystemCommand]:
        ...
        if screen.maximized is not None:
            ...
            yield SystemCommand("Maximize", "Maximize the focused widget", screen.action_maximize)
    ...
    def ansi_theme(self) -> TerminalTheme:
        ...
        """
        return self.ansi_theme_dark if self.current_theme.dark else self.ansi_theme_light
        ...
        *,
        ...
        async def app_ready() -> None:
            ...
            if auto_pilot is not None:
                ...
                async def run_auto_pilot(auto_pilot: AutopilotCallbackType, pilot: Pilot) -> None:
                    with self._context():
                        try:
                ...
                pilot = Pilot(app)
                auto_pilot_task = create_task(run_auto_pilot(auto_pilot, pilot), name=repr(pilot))
    ...
    async def _on_css_change(self) -> None:
        ...
        css_paths = self.css_monitor._paths if self.css_monitor is not None else self.css_path
        if css_paths:
            try:
                time = perf_counter()
    ...
    def get_screen(self, screen: Screen | str, screen_class: Type[Screen] | None = None) -> Screen:
        ...
        if screen_class is not None and not isinstance(next_screen, screen_class):
            raise TypeError(f"Expected a screen of type {screen_class}, got {type(next_screen)}")
        return next_screen
    ...
    def push_screen(
        ...
        if not isinstance(screen, (Screen, str)):
            raise TypeError(f"push_screen requires a Screen instance or str; not {screen!r}")
    ...
    if TYPE_CHECKING:
        ...
        @overload
        async def push_screen_wait(self, screen: Screen[ScreenResultType]) -> ScreenResultType: ...
    ...
    def switch_screen(self, screen: Screen | str) -> AwaitComplete:
        ...
        """
        if not isinstance(screen, (Screen, str)):
            raise TypeError(f"switch_screen requires a Screen instance or str; not {screen!r}")
    ...
    def install_screen(self, screen: Screen, name: str) -> None:
        ...
        if screen in self._installed_screens.values():
            raise ScreenError(f"Can't install screen; {screen!r} has already been installed")
    ...
    def _update_mouse_over(self, screen: Screen) -> None:
        ...
        async def check_mouse() -> None:
            ...
            try:
                ...
                if mouse_over is not self.mouse_over or hover_over is not self.hover_over:
    ...
    def _fatal_error(self) -> None:
        ...
        self.bell()
        traceback = Traceback(show_locals=True, width=None, locals_max_length=5, suppress=[rich])
            ...
            with self.batch_update():
                try:
                    try:
                        ...
                        await self._dispatch_message(events.Resize.from_dimensions(self.size, None))
                ...
                if not self._exit:
                    ...
                    try:
                        ...
                        if self._driver.is_inline:
                            ...
                            self._driver.write(Control.move(-cursor_x, -cursor_y).segment.text)
                            self._driver.flush()
    ...
    async def _ready(self) -> None:
        ...
        if constants.SCREENSHOT_DELAY >= 0:
            self.set_timer(constants.SCREENSHOT_DELAY, take_screenshot, name="screenshot timer")
                    ...
                    try:
                        if isinstance(renderable, CompositorUpdate):
                            ...
                            if self._driver.is_inline:
                                ...
                                terminal_sequence += Control.move(*cursor_position).segment.text
                            else:
                                terminal_sequence = renderable.render_segments(console)
                        ...
                        if WINDOWS:
                            ...
                            for chunk in (
                                ...
                                for offset in range(0, len(terminal_sequence), CHUNK_SIZE)
                            ):
                                write(chunk)
                        else:
    ...
    def _binding_chain(self) -> list[tuple[DOMNode, BindingsMap]]:
        ...
        if focused is None:
            ...
            namespace_bindings = [(node, node._bindings) for node in focused.ancestors_with_self]
    ...
    def _normalize_keymap(cls, keymap: Keymap) -> Keymap:
        ...
        return {binding_id: _normalize_key_list(keys) for binding_id, keys in keymap.items()}
        ...
        if isinstance(event, events.Compose):
            ...
            if isinstance(event, events.MouseEvent):
                ...
                if isinstance(event, events.MouseDown):
                    try:
                        self._mouse_down_widget, _ = self.get_widget_at(event.x, event.y)
                    except NoWidget:
                ...
                if isinstance(event, events.MouseUp) and self._mouse_down_widget is not None:
                    try:
    ...
    def _parse_action(
        ...
        action_target: DOMNode | None = None if namespaces is None else namespaces.get(destination)
    ...
    def _check_action_state(self, action: str, default_namespace: DOMNode) -> bool | None:
        ...
        Returns:
            State of an action.
        """
        action_target, action_name, parameters = self._parse_action(action, default_namespace)
    ...
    async def _dispatch_action(self, namespace: DOMNode, action_name: str, params: Any) -> bool:
    ...
    def action_notify(self, message: str, title: str = "", severity: str = "information") -> None:
        """Show a notification."""
    ...
    def suspend(self) -> Iterator[None]:
        ...
        if self._driver.can_suspend:
            ...
            raise SuspendNotSupported("App.suspend is not supported in this environment.")
    ...
    def _deliver_binary(
        ...
        *,
        ...
        save_directory = user_downloads_path() if save_directory is None else Path(save_directory)
    ...
    def _on_delivery_failed(self, event: events.DeliveryComplete) -> None:
        ...
        if event.name == "screenshot":
            self.notify("Failed to save screenshot", title="Screenshot", severity="error")
