    ...
    def make_bindings(cls, bindings: Iterable[BindingType]) -> Iterable[Binding]:
        ...
        for binding in bindings:
            ...
            for key in binding.key.split(","):
                key = key.strip()
                if not key:
                    raise InvalidBinding(f"Can not bind empty string in {binding.key!r}")
                if len(key) == 1:
            ...
            for binding in bindings:
                ...
                if keymap_key_string := keymap.get(binding_id):
                    ...
                    for keymap_key in keymap_keys:
                        if keymap_key in self.key_to_bindings or keymap_key in new_bindings:
                            ...
                            for clashed_binding in clashing_bindings:
                                ...
                                if not (
                                    clashed_binding.id
                                    and keymap.get(clashed_binding.id) != clashed_binding.key
                                ):
