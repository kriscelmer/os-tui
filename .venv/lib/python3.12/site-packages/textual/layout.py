...
class WidgetPlacement(NamedTuple):
    ...
    def process_offset(self, constrain_region: Region, absolute_offset: Offset) -> WidgetPlacement:
        ...
        if not widget.absolute_offset and not styles.has_any_rules("constrain_x", "constrain_y"):
            # Bail early if there is nothing to do
            return self
        region = self.region
        margin = self.margin
        if widget.absolute_offset is not None:
            region = region.at_offset(widget.absolute_offset + margin.top_left - absolute_offset)
...
class Layout(ABC):
    ...
    def render_keyline(self, container: Widget) -> StripRenderable:
        ...
        primitives = [
            get_rectangle(widget.region) for widget in container.children if widget.visible
        ]
