...
class Worker(Generic[ResultType]):
    ...
    def __init__(
        ...
        *,
        ...
        self.name = name
        self.group = group
        self.description = description if len(description) <= 1000 else description[:1000] + "..."
    ...
    def update(self, completed_steps: int | None = None, total_steps: int | None = -1) -> None:
    ...
    async def run(self) -> ResultType:
        ...
        Returns:
            Return value of the work.
        """
        return await (self._run_threaded() if self._thread_worker else self._run_async())
    ...
    def _start(self, app: App, done_callback: Callable[[Worker], None] | None = None) -> None:
        """Start the worker.
    ...
    async def wait(self) -> ResultType:
        ...
        try:
            if active_worker.get() is self:
                raise DeadlockError("Can't call worker.wait from within the worker function!")
        except LookupError:
            # Not in a worker
            pass
