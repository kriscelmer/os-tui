...
class SimpleAnimation(Animation):
    ...
    def __call__(self, time: float, app_animation_level: AnimationLevel = "full") -> bool:
        if (
            self.duration == 0
        ...
        if factor == 1.0:
            ...
            assert isinstance(self.start_value, (int, float)), (
                f"`start_value` must be float, not {self.start_value!r}"
            )
            assert isinstance(self.end_value, (int, float)), (
        ...
        *,
        ...
        if animation is None:
            ...
            animation = SimpleAnimation(
                ...
                easing=easing_function,
                on_complete=(
                    partial(self.app.call_later, on_complete) if on_complete is not None else None
                ),
                level=level,
            )
