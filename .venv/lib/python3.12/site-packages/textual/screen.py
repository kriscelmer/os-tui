...
class Screen(Generic[ScreenResultType], Widget):
    ...
    def __init__(
        ...
        self.screen_layout_refresh_signal: Signal[Screen] = Signal(self, "layout-refresh")
        ...
        self.text_selection_started_signal: Signal[Screen] = Signal(self, "selection_started")
        ...
        for namespace, bindings in self._modal_binding_chain:
            for key, binding in bindings:
                ...
                if existing_key_and_binding := bindings_map.get(key):
                    ...
                    if binding.priority and not existing_key_and_binding.binding.priority:
                        bindings_map[key] = ActiveBinding(
                            namespace, binding, enabled, binding.tooltip
                        )
                else:
                    # New binding
                    bindings_map[key] = ActiveBinding(namespace, binding, enabled, binding.tooltip)
    ...
    def arrange(self, size: Size, _optimal: bool = False) -> DockArrangeResult:
        ...
        arrangement = self._arrangement_cache[cache_key] = arrange(
            self,
            (get_maximize_widgets(self.maximized) if self.maximized is not None else self._nodes),
            size,
            self.size,
            False,
        )
        ...
        while node_stack:
            ...
            if node is None:
                ...
                if node.is_container and node.allow_focus_children():
                    sorted_displayed_children = sorted(node.displayed_children, key=focus_sorter)
    ...
    def minimize(self) -> None:
        ...
        if self.focused is not None:
            self.call_after_refresh(self.scroll_to_widget, self.focused, animate=False, center=True)
    ...
    def _on_mount(self, event: events.Mount) -> None:
        ...
        self.screen_layout_refresh_signal.subscribe(self, self._maybe_clear_tooltip, immediate=True)
    ...
    def _compositor_refresh(self) -> None:
        ...
        if app.is_inline:
            ...
            if self is app.screen:
                # Top screen
                update = self._compositor.render_update(screen_stack=app._background_screens)
    ...
    def _set_breakpoints(self, dimension: int, breakpoints: list[tuple[int, str]]) -> None:
    ...
    def _forward_event(self, event: events.Event) -> None:
        ...
        if isinstance(event, (events.Enter, events.Leave)):
            ...
            if self._selecting:
                ...
                select_widget, select_offset = self.get_widget_and_offset_at(event.x, event.y)
                if (
    ...
    def _watch__select_end(self, select_end: tuple[Widget, Offset, Offset] | None) -> None:
        ...
        if start_widget is end_widget:
            ...
            self.selections = {start_widget: Selection.from_offsets(start_offset, end_offset)}
            return
        ...
        for region in select_regions:
            ...
            covered_widgets = [
                widget for widget in covered_widgets if region.overlaps(widget.content_region)
            ]
