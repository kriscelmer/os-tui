...
def invoke_watcher(
    ...
    with watcher_object._context():
        if param_count == 2:
            watch_result = cast(WatchCallbackBothValuesType, watch_function)(old_value, value)
        elif param_count == 1:
...
class Reactive(Generic[ReactiveType]):
    ...
    def _check_watchers(cls, obj: Reactable, name: str, old_value: Any) -> None:
        ...
        if watchers:
            watchers[:] = [
                (reactable, callback) for reactable, callback in watchers if not reactable._closing
            ]
