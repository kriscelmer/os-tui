...
_CONTROL_STRIP_TRANSLATE: Final = {_codepoint: None for _codepoint in _STRIP_CONTROL_CODES}
...
class Content(Visual):
    ...
    def __init__(
        ...
        self._divide_cache: FIFOCache[Sequence[int], list[tuple[Span, int, int]]] | None = None
        self._split_cache: FIFOCache[tuple[str, bool, bool], list[Content]] | None = None
    ...
    @classmethod
    def from_text(cls, markup_content_or_text: ContentText, markup: bool = True) -> Content:
        ...
        if isinstance(markup_content_or_text, Content):
            ...
            raise TypeError("This method expects a str, a Text instance, or a Content instance")
    ...
    @classmethod
    def from_rich_text(cls, text: str | Text, console: Console | None = None) -> Content:
    ...
    @classmethod
    def assemble(cls, *parts: str | Content | tuple[str, str | Style], end: str = "") -> Content:
    ...
    def _wrap_and_format(
        ...
        for y, line in enumerate(self.split(allow_blank=True)):
            ...
            if no_wrap:
                ...
                content_line = _FormattedLine(get_style, line, width, y=y, align=align)
                offsets = divide_line(line.plain, width - line_pad * 2, fold=overflow == "fold")
    ...
    def append(self, content: Content | str) -> Content:
        ...
        if isinstance(content, str):
            return Content(
                ...
                self._spans,
                (None if self._cell_length is None else self._cell_length + cell_len(content)),
            )
    ...
    def pad_left(self, count: int, character: str = " ") -> Content:
        ...
        if count:
            ...
            _Span = Span
            spans = [_Span(start + count, end + count, style) for start, end, style in self._spans]
            content = Content(
                text,
                spans,
    ...
    def extend_right(self, count: int, character: str = " ") -> Content:
        ...
        if count:
            ...
            return Content(
                ...
                [(span.extend(count) if span.end == plain_len else span) for span in self._spans],
                None if self._cell_length is None else self._cell_length + count,
            )
        return self
    ...
    def pad(self, left: int, right: int, character: str = " ") -> Content:
        ...
        if left or right:
            ...
            if left:
                spans = [
                    _Span(start + left, end + left, style) for start, end, style in self._spans
                ]
            else:
                spans = self._spans
            content = Content(
    ...
    def stylize(self, style: Style | str, start: int = 0, end: int | None = None) -> Content:
    ...
    def render(
        ...
        style_map = {
            index: (get_style(span.style) if isinstance(span.style, str) else span.style)
    ...
    def render_segments(self, base_style: Style = Style.null(), end: str = "") -> list[Segment]:
    ...
    def split(
        ...
        *,
        ...
        if include_separator:
            ...
            lines = [line for line in self.divide(list(flatten_spans())) if line.plain != separator]
    ...
    def extend_style(self, spaces: int) -> Content:
        ...
        if spans:
            end_offset = len(self)
            spans = [span.extend(spaces) if span.end >= end_offset else span for span in spans]
    ...
    def highlight_regex(
        ...
        *,
        ...
        for match in re_highlight.finditer(plain):
            ...
            if maximum_highlights is not None and (count := count + 1) >= maximum_highlights:
                break
...
class _FormattedLine:
    ...
    def to_strip(self, style: Style) -> tuple[list[Segment], int]:
        ...
        segments = [Segment(" " * pad_left, style.background_style.rich_style)] if pad_left else []
        add_segment = segments.append
    ...
    def _apply_link_style(self, link_style: RichStyle, segments: list[Segment]) -> list[Segment]:
        _Segment = Segment
        segments = [
            _Segment(
                text,
