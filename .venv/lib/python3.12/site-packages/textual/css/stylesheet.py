...
class StylesheetErrors:
    ...
    def __rich_console__(self, console: Console, options: ConsoleOptions) -> RenderResult:
        error_count = 0
        errors = list(dict.fromkeys(chain.from_iterable(_rule.errors for _rule in self.rules)))
        ...
        for token, message in errors:
            ...
            title = Text.assemble("Error at ", path_string, widget_string, style="bold red")
            yield ""
            yield Panel(
                self._get_snippet(
...
class Stylesheet:
    ...
    def apply(
        ...
        *,
        ...
        limit_rules = {
            rule for name in rules_map.keys() & node._selector_names for rule in rules_map[name]
        }
        ...
        node._has_odd_or_even = "odd" in all_pseudo_classes or "even" in all_pseudo_classes
        ...
        if cache is not None and all_pseudo_classes.isdisjoint(
            ...
            cache_key = (
                node._parent,
                (None if node._id is None else (node._id if f"#{node._id}" in rules_map else None)),
                node.classes,
        ...
        if rule_attributes:
            ...
            for initial_rule_name in initial:
                ...
                if node_rules[initial_rule_name] is None:  # type: ignore[literal-required]
                    ...
                    default_rules = [
                        rule for rule in rule_attributes[initial_rule_name] if not rule[0][0]
                    ]
                    if default_rules:
    ...
    def _process_component_classes(self, node: DOMNode) -> None:
        ...
        if component_classes:
            ...
            for component in sorted(component_classes):
                ...
                if not refresh_node and old_component_styles.get(component) != virtual_node.styles:
        ...
        if animate:
            ...
            for key in modified_rule_keys:
                ...
                if is_animatable(key) and (
                    new_render_value != old_render_value or animator.is_being_animated(base, key)
                ):
