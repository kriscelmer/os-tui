...
class StylesBase:
    ...
    scrollbar_visibility = StringEnumProperty(VALID_SCROLLBAR_VISIBILITY, "visible", layout=True)
    ...
    grid_gutter_horizontal = IntegerProperty(default=0, layout=True, refresh_children=True)
    grid_gutter_vertical = IntegerProperty(default=0, layout=True, refresh_children=True)

    row_span = IntegerProperty(default=1, layout=True)
    column_span = IntegerProperty(default=1, layout=True)

    text_align: StringEnumProperty[TextAlign] = StringEnumProperty(VALID_TEXT_ALIGN, "start")
    ...
    overlay = StringEnumProperty(VALID_OVERLAY, "none", layout=True, refresh_parent=True)
    constrain_x: StringEnumProperty[Constrain] = StringEnumProperty(VALID_CONSTRAIN, "none")
    constrain_y: StringEnumProperty[Constrain] = StringEnumProperty(VALID_CONSTRAIN, "none")
    text_wrap: StringEnumProperty[TextWrap] = StringEnumProperty(VALID_TEXT_WRAP, "wrap")
    ...
    @property
    def is_dynamic_height(self, _dynamic_units={Unit.AUTO, Unit.FRACTION, Unit.PERCENT}) -> bool:
    ...
    @classmethod
    def parse(cls, css: str, read_from: CSSLocation, *, node: DOMNode | None = None) -> Styles:
    ...
    def partial_rich_style(self) -> Style:
        ...
        """
        style = Style(
            color=(self.color.rich_color if self.has_rule("color") and self.color.a > 0 else None),
            bgcolor=(
...
class Styles(StylesBase):
    ...
    def _get_border_css_lines(self, rules: RulesMap, name: str) -> Iterable[tuple[str, str]]:
    ...
    def css_lines(self) -> list[str]:
        ...
        if "scrollbar_color_active" in rules:
            append_declaration("scrollbar-color-active", self.scrollbar_color_active.css)

        if "scrollbar_corner_color" in rules:
            append_declaration("scrollbar-corner-color", self.scrollbar_corner_color.css)
        ...
        if "scrollbar_background_hover" in rules:
            append_declaration("scrollbar-background-hover", self.scrollbar_background_hover.css)
        if "scrollbar_background_active" in rules:
            append_declaration("scrollbar-background-active", self.scrollbar_background_active.css)
        ...
        if "scrollbar_size" in rules:
            ...
            if "scrollbar_size_horizontal" in rules:
                append_declaration("scrollbar-size-horizontal", str(self.scrollbar_size_horizontal))
            if "scrollbar_size_vertical" in rules:
                append_declaration("scrollbar-size-vertical", str(self.scrollbar_size_vertical))
        ...
        if "transitions" in rules:
            append_declaration(
                "transition",
                ", ".join(f"{name} {transition}" for name, transition in self.transitions.items()),
            )

        if "align_horizontal" in rules and "align_vertical" in rules:
            append_declaration("align", f"{self.align_horizontal} {self.align_vertical}")
        ...
        if "content_align_horizontal" in rules and "content_align_vertical" in rules:
            ...
            append_declaration("content-align-horizontal", self.content_align_horizontal)
        ...
        if "grid_gutter_horizontal" in rules:
            append_declaration("grid-gutter-horizontal", str(self.grid_gutter_horizontal))
        ...
        if "border_subtitle_background" in rules:
            append_declaration("subtitle-background", self.border_subtitle_background.css)
        ...
        if "constrain_x" in rules and "constrain_y" in rules:
            if self.constrain_x == self.constrain_y:
                ...
                append_declaration("constrain", f"{self.constrain_x} {self.constrain_y}")
...
class RenderStyles(StylesBase):
    ...
    def has_rule(self, rule_name: str) -> bool:
        ...
        return self._inline_styles.has_rule(rule_name) or self._base_styles.has_rule(rule_name)
