...
def _add_specificity(specificity1: Specificity3, specificity2: Specificity3) -> Specificity3:
...
def parse_selectors(css_selectors: str) -> tuple[SelectorSet, ...]:
    ...
    while True:
        ...
        if token_name == "pseudo_class":
            ...
            _selector, specificity = get_selector(token_name, (SelectorType.TYPE, (0, 0, 0)))
            selectors.append(
                Selector(
...
def parse_rule_set(
    ...
    while True:
        if token.name == "pseudo_class":
            ...
            _selector, specificity = get_selector(token.name, (SelectorType.TYPE, (0, 0, 0)))
            selectors.append(
                Selector(
                    name=token.value.lstrip(".#"),
                    combinator=combinator or CombinatorType.DESCENDENT,
    ...
    if selectors:
        if scope and selectors[0].name != scope:
            scope_selector, scope_specificity = get_selector(scope, (SelectorType.TYPE, (0, 0, 0)))
            selectors.insert(
                0,
                Selector(
                    name=scope,
            ...
            def combine_selectors(
                ...
                if selectors2 and selectors2[0].type == SelectorType.NESTED:
                    ...
                    merged_selector = dataclasses.replace(
                        final_selector,
                        pseudo_classes=(
                            final_selector.pseudo_classes | nested_selector.pseudo_classes
                        ),
            ...
            for rule_selector in rule_selectors:
                for rule_set in recursive_parse:
                    nested_rule_set = RuleSet(
                        [
                            SelectorSet(
                                combine_selectors(rule_selector, recursive_selectors.selectors)
                            )._total_specificity()
                    ...
                    if ref_name in variables:
                        ...
                        for _token in reference_tokens:
                            yield _token.with_reference(
                                ReferencedBy(ref_name, ref_location, ref_length, token.code)
                            )
                    else:
