...
def _resolve_cells(value: float, size: Size, viewport: Size, fraction_unit: Fraction) -> Fraction:
...
def _resolve_width(value: float, size: Size, viewport: Size, fraction_unit: Fraction) -> Fraction:
...
def _resolve_height(value: float, size: Size, viewport: Size, fraction_unit: Fraction) -> Fraction:
...
class Scalar(NamedTuple):
    ...
    def resolve(
        ...
        if unit == Unit.PERCENT:
            unit = percent_unit
        try:
            dimension = RESOLVE_MAP[unit](value, size, viewport, fraction_unit or _FRACTION_ONE)
        except KeyError:
