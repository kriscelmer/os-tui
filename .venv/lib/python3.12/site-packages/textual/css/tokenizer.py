...
class Tokenizer:
    ...
    def get_token(self, expect: Expect) -> Token:
        ...
        if match is None:
            ...
            error_message = f"{expect.description} (found {error_line.split(';')[0]!r})."
            if expect._expect_semicolon and not error_line.endswith(";"):
                error_message += "; Did you forget a semicolon at the end of a line?"
            raise TokenError(self.read_from, self.code, (line_no + 1, col_no + 1), error_message)
        ...
        if token.name == "pseudo_class" and token.value.strip(":") not in VALID_PSEUDO_CLASSES:
