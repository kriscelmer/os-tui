...
class _ClassesDescriptor:
    ...
    def __get__(self, obj: DOMNode, objtype: type[DOMNode] | None = None) -> frozenset[str]:
...
class DOMNode(MessagePump):
    ...
    def __init__(
        ...
        *,
        ...
        self.styles: RenderStyles = RenderStyles(self, self._css_styles, self._inline_styles)
        ...
        self._bindings = (
            BindingsMap() if self._merged_bindings is None else self._merged_bindings.copy()
        )
        ...
        self._reactive_connect: dict[str, tuple[MessagePump, Reactive[object] | object]] | None = (
            None
        )
        self._pruning = False
    ...
    def data_bind(
        ...
        **bind_vars: Reactive[Any] | object,
        ...
        for name, reactive in bind_vars.items():
            if name not in self._reactives:
                raise ReactiveError(f"Unable to bind non-reactive attribute {name!r} on {self}")
            if isinstance(reactive, Reactive) and not isinstance(parent, reactive.owner):
                raise ReactiveError(
    ...
    def text_style(self) -> Style:
        ...
        Returns:
            A Rich Style.
        """
        return Style.combine(node.styles.text_style for node in reversed(self.ancestors_with_self))
    ...
    def _get_title_style_information(self, background: Color) -> tuple[Color, Color, VisualStyle]:
    ...
    def background_colors(self) -> tuple[Color, Color]:
        ...
        for node in reversed(self.ancestors_with_self):
            ...
            opacity *= styles.opacity
            background += styles.background.tint(styles.background_tint).multiply_alpha(opacity)
    ...
    def get_pseudo_classes(self) -> set[str]:
        ...
        return {name for name, check_class in self._PSEUDO_CLASSES.items() if check_class(self)}
    ...
    def query_one(
        ...
        if is_id_selector(query_selector):
            ...
            if (
                node := walk_breadth_search_id(base_node, query_selector[1:], with_root=False)
            ) is not None:
        ...
        for node in iter_children:
            ...
            for later_node in iter_children:
                if match(selector_set, later_node):
                    raise TooManyMatches("Call to query_one resulted in more than one matched node")
            if cache_key is not None:
                base_node._query_one_cache[cache_key] = node
            return node
