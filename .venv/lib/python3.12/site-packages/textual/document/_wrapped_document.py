...
class WrappedDocument:
    ...
    def wrap_range(
        ...
        start_line_index = clamp(start_line_index, 0, min((old_max_index, new_max_index)))
        old_end_line_index = clamp(old_end_line_index, 0, old_max_index)
        new_end_line_index = clamp(new_end_line_index, 0, new_max_index)

        top_line_index, old_bottom_line_index = sorted((start_line_index, old_end_line_index))
        ...
        for line_index, line in enumerate(new_lines, top_line_index):
            tab_sections = get_tab_widths(line, tab_width)
            wrap_offsets = (
                compute_wrap_offsets(line, width, tab_width, precomputed_tab_sections=tab_sections)
                if width
                else []
            )
        ...
        self._offset_to_line_info[top_y_offset : old_bottom_y_offset + 1] = new_offset_to_line_info

        self._line_index_to_offsets[top_line_index : old_bottom_line_index + 1] = (
            new_line_index_to_offsets
        )

        self._tab_width_cache[top_line_index : old_bottom_line_index + 1] = new_tab_widths
        ...
        self._wrap_offsets[top_line_index : old_bottom_line_index + 1] = new_wrap_offsets
    ...
    def offset_to_location(self, offset: Offset) -> Location:
        ...
        if offset_data is not None:
            ...
            location = len(self._wrap_offsets) - 1, get_target_document_column(-1, x, -1)
    ...
    def location_to_offset(self, location: Location) -> Offset:
        ...
        x_offset = cell_len(expand_tabs_inline(section[:section_column_index], self._tab_width))
    ...
    def get_target_document_column(
        ...
        # document line)
        target_section_start = sum(len(wrapped_section) for wrapped_section in sections[:y_offset])
