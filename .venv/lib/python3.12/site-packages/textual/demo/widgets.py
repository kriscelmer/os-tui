...
"""
    ...
    def compose(self) -> ComposeResult:
        ...
        with containers.Grid():
            ...
            yield Label("Number")
            yield Input(type="number", placeholder="Type a number here", valid_empty=True)
...
'''
    ...
    def update_rich_log(self) -> None:
        ...
        if self.is_scrolling:
            return
        if not self.app.screen.can_view_entire(rich_log) and not rich_log.is_in_maximized_view:
            return
...
"""
    ...
    def compose(self) -> ComposeResult:
        ...
        with containers.VerticalScroll(id="container", can_focus=True, can_maximize=True):
